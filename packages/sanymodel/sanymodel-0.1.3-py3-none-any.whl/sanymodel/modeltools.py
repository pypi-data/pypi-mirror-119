...
class Result(object):
    ...
    def add_fig(self, local_fig_path: str, upload_fig_path: str = None, fig_name: str = None, fig_data: str = '',
        ...
        if rename:
            name_parts = [self.wind_farm, self.turbine_num, self.data_start_time, self.data_end_time, fig_name, str(uuid.uuid1())]
    ...
    def add_file(self, local_file_path: str, upload_file_path: str = None, rename=True):
        ...
        if rename:
            name_parts = [self.wind_farm, self.turbine_num, self.data_start_time, self.data_end_time, file_name, str(uuid.uuid1())]
            file_name = '_'.join(name_parts)
    ...
    def upload(self, stub: str = None):
        ...
        if len(user_comment):
            self.comment = self.description = ';'.join(user_comment)
...
class Figure(object):
    ...
    @staticmethod
    def format_fig(fig, template='plotly_dark') -> go.Figure:
        """
        ...
        is_matplotlib = isinstance(fig, mpl.figure.Figure)
        ...
        new_layout = go.Layout(showlegend=True,
                               ...
                               titlefont=go.layout.Font(size=15),
                               legend=go.layout.Legend(font=go.layout.Font(size=13))
                               )
