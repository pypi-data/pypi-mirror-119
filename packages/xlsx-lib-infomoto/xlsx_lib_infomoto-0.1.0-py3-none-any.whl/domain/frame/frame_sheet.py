...
from domain.frame.element_part import ElementPart
from domain.frame.frame_cell_value_types import FrameCellValueTypes
from domain.frame.frame_element import FrameElement
from domain.xlsx_elements.sheet import Sheet



class ModelEndError(EOFError):
    pass
...
class FrameSheet(Sheet):
    frame_elements: list[FrameElement] = list()
    ...
    def process_cell(self, cell: Cell, col_index: int, row_index: int):
        if cell.value is None:
            if self.sheet_reader.read_cell_value(col_index, row_index + 1) is None \
                    ...
                    and len(self.frame_elements[-1].element_parts) == 0 and \
                    self.frame_elements[-1].observations is None:
    ...
    def process_frame_element(
        ...
        self.frame_elements.append(FrameElement(
            name=values[0],
            value=values[1],
    ...
    def process_element_part(
        ...
        self.frame_elements[-1].element_parts.append(ElementPart(
            name=values[0],
            value=values[1],
            observations=values[2]
        ))
