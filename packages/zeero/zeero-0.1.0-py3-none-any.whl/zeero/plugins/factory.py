...
def set_status(status: Status, source: str, message: str, progress: int, path: str):
    """set_status writes the factory status update event to the event stream.
    """

    with open(path, "a+", encoding="utf-8") as event_stream:
...
class ManifestBuilder():
    ...
    def clone_operator(url: str, debug: bool = False):
        ...
        repo_dir = tempfile.TemporaryDirectory() # pylint: disable=consider-using-with
        _, err = run_command(
            sh.git, # pylint: disable=no-member
            ("clone", url, repo_dir.name),
