...
import stat
import sys
from salvia.util.config import load_config, traverse_dict
from salvia.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `salvia init --fix-ssl-permissions` to fix issues.")
