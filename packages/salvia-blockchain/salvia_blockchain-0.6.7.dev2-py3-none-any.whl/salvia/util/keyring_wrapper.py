...
from salvia.util.misc import prompt_yes_no
...
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ salvia passphrase set # all the cool kids are doing it!"
...
class KeyringWrapper:
    ...
    def set_master_passphrase(
        ...
        from salvia.util.keychain import KeyringCurrentPassphaseIsInvalid, KeyringRequiresMigration
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'salvia passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase? (y/n) ")
            if response:
                from salvia.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'salvia passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from salvia.util.keychain import Keychain, MAX_KEYS
    ...
    def cleanup_legacy_keyring(self, migration_results: MigrationResults):
        ...
        # [salvia_2Duser_2Dsalvia_2D1_2E8] is left behind
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from salvia.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
