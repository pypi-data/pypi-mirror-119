...
from salvia.cmds.init_funcs import check_keys, salvia_init
from salvia.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from salvia.daemon.keychain_server import KeychainServer, keychain_commands
from salvia.daemon.windows_signal import kill
from salvia.server.server import ssl_context_for_root, ssl_context_for_server
...
from salvia.util.service_groups import validate_service
from salvia.util.setproctitle import setproctitle
from salvia.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Salvia.")
    quit()
...
service_plotter = "salvia plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "salvia": "salvia",
        "salvia_wallet": "start_wallet",
        "salvia_full_node": "start_full_node",
        "salvia_harvester": "start_harvester",
        "salvia_farmer": "start_farmer",
        "salvia_introducer": "start_introducer",
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("salvia daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SALVIA_ROOT
    os.environ["SALVIA_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with SALVIA_ROOT: {os.environ['SALVIA_ROOT']}")
    ...
    if service_command == "salvia_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in salvia_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    salvia_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("salvia_daemon")
