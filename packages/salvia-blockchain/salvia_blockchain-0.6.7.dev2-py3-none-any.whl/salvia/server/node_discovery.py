...
from salvia.protocols import full_node_protocol, introducer_protocol
from salvia.protocols.protocol_message_types import ProtocolMessageTypes
from salvia.server.address_manager import AddressManager, ExtendedPeerInfo
from salvia.server.address_manager_store import AddressManagerStore
from salvia.server.outbound_message import NodeType, make_msg
from salvia.server.server import SalviaServer
from salvia.types.peer_info import PeerInfo, TimestampedPeerInfo
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: SalviaServer = server
    ...
    async def on_connect(self, peer: ws.WSSalviaConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSSalviaConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSSalviaConnection):
