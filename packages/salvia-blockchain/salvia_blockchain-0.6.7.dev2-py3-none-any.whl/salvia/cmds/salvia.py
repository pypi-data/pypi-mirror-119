...
from salvia.cmds.configure import configure_cmd
from salvia.cmds.farm import farm_cmd
from salvia.cmds.init import init_cmd
from salvia.cmds.keys import keys_cmd
from salvia.cmds.netspace import netspace_cmd
from salvia.cmds.passphrase import passphrase_cmd
from salvia.cmds.plots import plots_cmd
from salvia.cmds.show import show_cmd
from salvia.cmds.start import start_cmd
...
@click.group(
    help=f"\n  Manage salvia blockchain infrastructure ({__version__})\n",
    epilog="Try 'salvia start node', 'salvia netspace -d 192', or 'salvia show -s'",
...
@cli.command("version", short_help="Show salvia version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs salvia daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by salvia start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from salvia.daemon.server import async_run_daemon
