...
from salvia.consensus.block_rewards import calculate_base_farmer_reward
from salvia.pools.pool_wallet import PoolWallet
from salvia.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
...
from salvia.simulator.simulator_protocol import FarmNewBlockProtocol
...
from salvia.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
from salvia.util.path import path_from_root
...
from salvia.wallet.derive_keys import master_sk_to_singleton_owner_sk
...
from salvia.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from salvia.wallet.util.trade_utils import trade_record_to_dict
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "salvia_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "salvia_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xslv_target_address")
        pool_target = config["pool"].get("xslv_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from salvia.pools.pool_wallet_info import initial_pool_state_from_dict
