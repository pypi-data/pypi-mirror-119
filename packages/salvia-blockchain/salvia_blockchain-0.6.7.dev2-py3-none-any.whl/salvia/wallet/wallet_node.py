...
from salvia.daemon.keychain_proxy import (
    KeychainProxy,
...
from salvia.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from salvia.protocols import wallet_protocol
from salvia.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from salvia.protocols.protocol_message_types import ProtocolMessageTypes
from salvia.protocols.wallet_protocol import (
...
from salvia.server.node_discovery import WalletPeers
...
from salvia.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from salvia.wallet.wallet_blockchain import ReceiveBlockResult
from salvia.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'salvia keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSSalviaConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSSalviaConnection):
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSSalviaConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSSalviaConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSSalviaConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSSalviaConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
