...
def app_wrapper(args, parser, func):
    ...
    vars = argparse.Namespace(**args)
    writeInputsOptionsLog(vars.outdir, parser, vars, debug)

    _pm_try_except(func, SESSION, JOB, standalone, debug, vars)

    if not standalone:
        ### set the complete status of the job
        setAsCompleted(vars.outdir)
...
def pmws_wrapper(args):
    ...
    try:
        session = Session(args.token)
        job = session.get_job(execid=args.exec_id)
    except Exception as e:
    ...
    try:
        ...
        appname = None
        for param in params:
            if param["name"] == "appname":
                appname = param["value"]

        _download_datacenter_args(params, indir, session)

        cmd = [args.pythonexe, "/logic/app.py", "-token", args.token]
