...
def _run(list_of_contents_for_A: list, list_of_contents_for_R: list, list_of_contents_for_value: list):
    ...
    practice_factor, erodability, slope_length, pcorr, p_ef_c1, ef_p_c2 = list_of_contents_for_A
    A = calculate_A(R, practice_factor, erodability, slope_length, pcorr, p_ef_c1, ef_p_c2)

    nla_environment, P_content = list_of_contents_for_value
    logger.debug('model=%s, term=%s, R=%s, A=%s, nla_environment=%s, P_content=%s',
                 MODEL, TERM_ID, R, A, nla_environment, P_content)
...
def _should_run(cycle: dict):
    ...
    soil_phosphorus_content = _get_measurement_content('phosphorusPerKgSoil')
    ...
    list_of_contents_for_A = [
        practice_factor, erodability, slope_length,
