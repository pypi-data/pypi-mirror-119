...
class API:
    """`API` is a library for interacting with Robocorp public API endpoints."""
    ...
    def api_base_url(self):
        return f"{self.robocorp_api_server}/workspaces/{self.workspace_id}/processes"
    ...
    def start_process(
        ...
        response = self.http.session_less_post(
            url=f"{self.api_base_url}/{pid}/{endpoint}",
    ...
    def start_configured_process(
        ...
        self.logger.warning(request_data)
        response = self.http.session_less_post(
            url=f"{self.api_base_url}/{pid}/run-request",
            headers=self.headers,
    ...
    def create_input_work_item(
        ...
        pid = process_id or self.process_id
        response = self.http.session_less_post(
            url=f"{self.api_base_url}/{pid}/work-items",
            headers=self.headers,
            json={"payload": payload or {}},
        )
        return response.status_code, response.json()

    @keyword(tags=["process", "post"])
    def register_file_upload(
        ...
        pid = process_id or self.process_id
        response = self.http.session_less_post(
            url=f"{self.api_base_url}/{pid}/work-items/{workitem_id}/files/upload",
            headers=self.headers,
            data=json.dumps(
    ...
    def get_process_run_status(self, run_id: str):
        pass
    ...
    def list_process_workitems(self, process_id: str, include_data: bool = False):
        pid = process_id or self.process_id
        response = self.http.session_less_get(
            url=f"{self.api_base_url}/{pid}/work-items",
            headers=self.headers,
    ...
    def get_workitem(
        ...
        pid = process_id or self.process_id
        response = self.http.session_less_get(
            url=f"{self.api_base_url}/{pid}/work-items/{workitem_id}",
            headers=self.headers,
            params={"includeData": str(include_data).lower()},
        )
        return response.status_code, response.json()
