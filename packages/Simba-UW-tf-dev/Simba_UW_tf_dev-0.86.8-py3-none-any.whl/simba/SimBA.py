...
class project_config:
    ...
    def bpTable(self,master):
        ...
        if currentPlatform == 'Windows':
            if self.noAnimals > 1:
                frame = LabelFrame(master,text='Bodyparts\' name                       Animal ID number')
            else:
...
class unsupervisedInterface:
    def __init__(self,inifile):
        ...
        tabControl.add(tab1, text=' [Save Project Folder] ')
        tabControl.add(tab2, text=' [Create Dataset] ')
        ...
        saveProjectFolder = LabelFrame(tab1, text="1.Save Project Folder", font=("Helvetica", 12, 'bold'), padx=5, pady=5, fg='black')
        btnCreateFolder = Button(saveProjectFolder, text="Create folder", command= self.unsupervisedCreate)

        # create dataset
        createDataset = LabelFrame(tab2, text="2.Create Dataset", font=("Helvetica", 12, 'bold'), padx=5, pady=5, fg='black')
        ...
        btnVisualizeDR = Button(self.performDR, text="Save visualizations",
                                command=print('test'))  # change name to update with algorithm selection
        ...
        createDataset.grid(row=0,sticky=W)
        ...
        btnVisualizeDR.grid(row=4,pady=5)
    ...
    def unsupervisedCreate(self):
        folder_list = ['dimensionality_reduction','create_dataset','clustering','train_model','visualize_clusters']
    ...
    def generate_dataset(self):
        ...
        outputPath = os.path.join(self.unsupervisedfolderpath, 'create_dataset')
    ...
    def save_permimportances(self):
        ...
        for cluster in range(len(clustersList)):
            ...
            # for CurrUnderSample in samplingList:
            #     if CurrUnderSample != 'None':
            #         print('Performing undersample...' + str(CurrUnderSample))
            #         nontargetFrameRowsSize = int(len(currentTargetDf) * CurrUnderSample)
            #         nonTargetFrameRows = currentNonTargetDf.sample(n=nontargetFrameRowsSize, replace=False)
            #         nonTargetFrameRows['Cluster'] = 0
            #         targetFrameRows['Cluster'] = 1
            #         trainDf = pd.concat([targetFrameRows, nonTargetFrameRows])
            #         trainDf.sample(frac=1)
            #         target_train = trainDf.pop('Cluster_target').values
            #         data_train = trainDf
            #
            #
            #     visualizer = ClassificationReport(clf, classes=['Not_target_cluster', 'target_cluster'], support=True)
            #     visualizer.score(data_test, target_test)
            #     visualizerPath = os.path.join(outputPath, 'Classification_report_target_cluster_' + str(cluster) + '_downsample_' + str(CurrUnderSample) + '.png')
            #     g = visualizer.poof(outpath=visualizerPath, clear_figure=True)
            #
            #
            #     #PERFORM FEATURE PERM IMPORTANCE
            ...
            #     fname = 'Permutation_importance_target_cluster_' + str(cluster) + '_nonTarget_cluster_' + '_downsample_' + str(CurrUnderSample) + '_SMOTEEN_1.csv'
