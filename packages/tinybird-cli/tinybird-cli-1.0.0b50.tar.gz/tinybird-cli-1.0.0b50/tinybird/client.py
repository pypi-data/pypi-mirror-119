...
class TinyB(object):
    ...
    async def datasource_analyze(self, url, type_guessing=True):
        params = {
            'dry': 'true',
            'url': url,
            'type_guessing': 'true' if type_guessing else 'false',
        }
        return await self._req(f"/v0/datasources?{urlencode(params)}", method='POST', data=b'')
    ...
    async def pipes(self, branch=None, dependencies=False):
        response = await self._req(f"/v0/pipes?dependencies={'true' if dependencies else 'false'}")
