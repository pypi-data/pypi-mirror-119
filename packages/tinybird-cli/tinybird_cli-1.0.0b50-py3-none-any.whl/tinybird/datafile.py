...
def parse(s, default_node=None, basepath='.'):  # noqa: C901
    ...
    def include(*args, **kwargs):
        ...
        with open(p / f) as file:
            lines[lineno:lineno + 1] = [''] + list(StringIO(Template(file.read()).substitute(attrs), newline=None))
    ...
    cmds = {
        ...
        'kafka_bootstrap_servers': assign_single_var('kafka_bootstrap_servers'),
        'kafka_key': assign_single_var('kafka_key'),
        'kafka_secret': assign_single_var('kafka_secret'),
        'kafka_schema_registry_url': assign_single_var('kafka_schema_registry_url'),
...
async def process_file(filename, tb_client, tag='', resource_versions=None, verbose=False, skip_connectors=False):  # noqa: C901
...
def find_file_by_name(folder, name, verbose=False, is_raw=False):
    f = Path(folder)
    ds = name + ".datasource"
    if os.path.isfile(os.path.join(folder, ds)):
        return ds
    ...
    if not is_raw:
        return find_file_by_name(folder, get_dep_from_raw_tables(name), verbose=verbose, is_raw=True)
...
async def build_graph(filenames, tb_client, tag='', resource_versions=None, process_dependencies=False, verbose=False, skip_connectors=False):  # noqa: C901
    """process files"""
    to_run = {}
    deps = []
    dep_map = {}
    ...
    async def process(filename, deps, dep_map, to_run):
    ...
    for filename in filenames:
        ...
        name = await process(filename, deps, dep_map, to_run)
        processed.add(name)
        ...
        while len(deps) > 0:
            ...
            if dep not in processed:
                ...
                if f:
                    ...
                    await process(str(f), deps, dep_map, to_run)
...
async def folder_push(tb_client: TinyB, tag='', filenames=None, dry_run=False, check=False, push_deps=False,  # noqa: C901
    ...
    resources, dep_map = await build_graph(filenames, tb_client, tag=tag, process_dependencies=True, skip_connectors=True)
