...
def prepare_docker():
	return prepare_docker_image(f"{IMAGE_NAME}:{IMAGE_TAG}", IMAGE_NAME, IMAGE_TAG)
...
def prepare_docker_image(image, name, tag):
	...
	if len(images):
		...
		print(f'Unable to find {image} in local storage. Pulling the image...')
...
class DojoCLI(click.MultiCommand):
	...
	def ls(reverse):		
		...
		environment = {
			"KAEN_DOJO_LS_REVERSE": "" if reverse else "-r"
		}
	...
	def inspect(dojo):	
		...
		logs = container.attach(stream = False, logs = True, stdout = True, stderr = True)
...
class JobCLI(click.MultiCommand):
	...
	def ls(reverse):		
		...
		env = {
			"KAEN_JOB_LS_REVERSE": "" if reverse else "-r"
		}
...
class IdeCLI(click.MultiCommand):	
	...
	def jupyter():
		...
		container = IdeCLI._run(image_name, image_tag, [('8888', port)], [('JPY_TKN', token)])
		logs = container.attach(stream = True, logs = True)
		for s in logs:
			s = s.decode('utf-8').strip()
