...
def nuccell_zprojlabel_imgs(
    ...
    es_slidepxscene_nuccellmatche = set()
    s_ipath = s_format_segdir_cellpose.format(s_segdir, s_slide)
    for s_file in sorted(os.listdir(s_ipath)):
        ...
        # plot
        if not (o_match is None) and not (s_slidepxscene in es_slidepxscene_nuccellmatche):
            ...
            ax[0,0].imshow(io.imread(s_ipath + config.d_nconv['s_format_png_nucprojection'].format(s_slidepxscene, i_nuc_diam)))
            ax[0,1].set_title('nuc label')
            ax[0,1].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_nuc'].format(s_slidepxscene, i_nuc_diam)))
            ax[0,2].axis('off')
            ax[0,3].axis('off')
            # cell
            ax[1,0].set_title('cell z projection')
            try:
                ax[1,0].imshow(io.imread(s_ipath + config.d_nconv['s_format_png_cellprojection'].format(s_slidepxscene, s_seg_markers, i_cell_diam)))  # bue: s_seg_markers == s_z_label
            except FileNotFoundError:
                #ax[1,0].axis('off')
                pass
            ax[1,1].set_title('cell label - original')
            try:
                ax[1,1].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_cell'].format(s_slidepxscene, s_seg_markers, i_cell_diam)))
            except FileNotFoundError:
                #ax[1,1].axis('off')
                pass
            ax[1,2].set_title('cell label - nuc ok')
            try:
                ax[1,2].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_nuccellmatched'].format(s_slidepxscene, s_seg_markers, i_nuc_diam, i_cell_diam)))
            except FileNotFoundError:
                #ax[1,2].axis('off')
                pass
            ...
            try:
                ax[1,3].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_nuccellmatchedfeat'].format(s_slidepxscene, s_seg_markers, i_nuc_diam, i_cell_diam)))
            except FileNotFoundError:
                #ax[1,3].axis('off')
                pass
            ...
            os.makedirs(s_opath, exist_ok=True)
            fig.savefig(s_opath + f'{s_slidepxscene}_nuccell_zprojection_labelbasin_img.png', facecolor='white')
            plt.close()

            # update es_slidepxscene_nuccellmatche
            if (b_nuccellmatched):
                es_slidepxscene_nuccellmatche.add(s_slidepxscene)
