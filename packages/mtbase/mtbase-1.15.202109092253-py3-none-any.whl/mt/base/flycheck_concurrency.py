...
async def run_asyn_works_in_new_context(progress_queue: _mp.Queue, context_id: int, func, func_args: list = [], func_kwargs: dict = {}, work_id_list: list = [], max_concurrency: int = 1024, asyn: bool = True, profile = None):
    ...
    progress_queue: multiprocessing.Queue
        ...
        notes below.
    context_id : int
        the context id to be assigned to the new context
    func : function
...
def run_asyn_works(progress_queue: _mp.Queue, func, func_args: list = [], func_kwargs: dict = {}, num_processes = None, num_works: int = 0, max_concurrency: int = 1024, profile = None):
    ...
    def worker_process(progress_queue: _mp.Queue, context_id: int, func, func_args: list = [], func_kwargs: dict = {}, work_id_list: list = [], max_concurrency: int = 1024, profile = None):
        import asyncio
        asyncio.run(run_asyn_works_in_new_context(
            progress_queue, context_id, func,
            func_args=func_args,
    ...
    for context_id in range(num_buckets):
        p = _mp.Process(
            target=worker_process,
            args=(progress_queue, context_id, func),
            kwargs={
                'func_args': func_args,
                'func_kwargs': func_kwargs,
