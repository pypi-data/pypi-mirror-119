...
from pymasmovil.models.order_item_status import OrderItemStatus
...
class Account:
    ...
    def get_order_item_by_ICC(self, session, ICC, status_list=[]):
        """
        ...
        :param: session: MM login session instance,
                ...
                status_list: list of order-item status to filter
        :return: most recently created  OrderItem object with corresponding status and ICC
        """
        params = {"rowsPerPage": 30, "actualPage": 1}

        if status_list and any(
                status not in OrderItemStatus.available_status for status in status_list):
            raise OrderItemStatusNotFound(status_list)
        ...
        icc_order_items = [order_item for order_item in order_item_list
                           if order_item.get("simAttributes").get("ICCID") == str(ICC)]

        if status_list:
            icc_order_items = list(
                filter(lambda item: item.get("status") in status_list, icc_order_items)
            )
    ...
    def _get_newest_order_item(self, order_items):
        ...
        `createdDate`.
        """
        sorted_order_items = sorted(order_items,
