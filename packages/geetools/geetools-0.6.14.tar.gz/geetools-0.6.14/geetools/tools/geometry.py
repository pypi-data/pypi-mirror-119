...
def GeometryCollection_to_MultiPolygon(geom):
    ...
    def over_geom(geomdict, polygons):
        ...
        return ee.List(ee.Algorithms.If(isPolygon.Or(isRing), polygons.add(coords),
               ee.List(ee.Algorithms.If(isMulti, polygons.cat(coords),
               polygons))))
...
def GeometryCollection_to_MultiLineString(geom):
    ...
    def over_geom(geomdict, strings):
        ...
        return ee.List(ee.Algorithms.If(isLineString, strings.add(coords),
               ee.List(ee.Algorithms.If(isMulti,strings.cat(coords),
               strings))))
    geomlist = ee.List(geometries.iterate(over_geom, ee.List([])))
...
def GeometryCollection_to_MultiPoint(geom):
    ...
    def over_geom(geomdict, points):
        ...
        return ee.List(ee.Algorithms.If(isPoint, points.add(coords),
               ee.List(ee.Algorithms.If(isMulti, points.cat(coords),
               points))))
    geomlist = ee.List(geometries.iterate(over_geom, ee.List([])))
