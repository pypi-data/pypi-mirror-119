...
class SimulateModel(WindowedPlugin): 
    ...
    def create_window(self, dialog):
        ...
        self.dialog = dialog
        self.window = wx.ScrolledWindow(dialog, size=(350, 700))
...
#        self.main_sizer.Add(self.values_labels)
    ...
    def continuous_mode(self, evt):
        self.run_cont = self.is_continuous.GetValue()
        if self.is_continuous.GetValue():
            self.sim_end_input.Disable()
            self.sim_points_input.Disable()
        else:
            self.sim_end_input.Enable()
            self.sim_points_input.Enable()
    ...
    def go(self, evt):
        ...
        if self.run_cont:
            self.run_without_end()
        else:
            self.run_with_end()


    def run_without_end(self):
        self.go_btn.Disable()
        self.stop_btn.Show()
        ...
        self.r = te.loada(ant)
        #sim = r.simulate(start=int(self.sim_start_input.GetValue()), end=round(float(self.sim_step_time.GetValue()))*100, points=100)
        #self.update_node_info(sim)
    ...
    def do_one_step(self, evt):
        ...
        api.update_canvas()
            # will retrieve values here
            #jr.getFloatingSpeciesConcentration returns numpy array
            #    getFloatingSpeciesConcentrationsNamedArray
            #    order same as ids
        
        '''
        while not self.chose_stop_sim:
            # we will have to simulate again eventually
            if self.current_point == 100:
                sim = r.simulate()
                self.update_node_info(sim)
        '''
        '''
        # update the canvas
        self.timer.Stop() # TODO this is redundant
        self.current_point = self.current_point % 100
    ...
    def update_current_point(self, evt):
        if self.run_cont or self.current_point < self.sim_points - 1:
    ...
    def update_network(self, net_index):
        ...
        with api.group_action():
            for node_id in self.nodeinfo:
                ...
                final_conc = self.nodeinfo[node_id]["points"][self.current_point]
        ...
        sbml = exportSBML.ExportSBML.NetworkToSBML(self)
        self.model = simplesbml.loadSBMLStr(sbml)
        # update the text controls TODO DEFINITELY should change this
        ...
        for p in self.param_iv_tc:
            p.SetValue(api.get_reaction_by_index(0, self.reacinfo[p.GetName()]["index"]).concentration) # never mind
        '''
        for p in self.param_iv_tc:
