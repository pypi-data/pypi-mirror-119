...
def _resolve_array_lengths(
    ...
    if len(resolve) == 0:
        # TODO: Do we return lengths for arrays if they aren't accessed internally?
...
def _len(args: q.Args, *rest) -> int:
    """
    Calculates the length of a queryable data structure if applicable.
    :param args: Arguments (should be a single expression) for the #len function.
    :return: The length of the evaluated single argument.
    """
    return len(evaluate_no_validate(args[0], *rest))
