...
to a file.
"""
from bwmd.tools import load_vectors, save_vectors
...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self, reduced_dimensions: int, compression: str) -> None:
        ...
        self.output_layer = tf.keras.layers.Dense(
            units=reduced_dimensions, activation=tf.nn.sigmoid
        )
...
class Decoder(tf.keras.layers.Layer):
    ...
    def __init__(self, reduced_dimensions: int, original_dimensions: int = 300) -> None:
        ...
        self.output_layer = tf.keras.layers.Dense(
            units=original_dimensions, activation=tf.nn.sigmoid
        )
...
class AutoEncoder(tf.keras.Model):
    ...
    def __init__(
        self,
        reduced_dimensions: int = 30,
        original_dimensions: int = 300,
...
def spearman_correlation(y_true, y_pred):
    ...
    sp = tf.map_fn(
        lambda x: sp_rank(x[0], x[1]), (y_true, y_pred_rank), dtype=tf.float32
    )
...
class Compressor:
    ...
    def __init__(
        self,
        original_dimensions: int = 300,
        reduced_dimensions: int = 60,
        compression: str = "bool_",
    ) -> None:
        """
        ...
        self.autoencoder = AutoEncoder(
            reduced_dimensions, original_dimensions, compression
        )
    ...
    def prepare_vectors(vectors: list, batch_size: int, shuffle=True) -> "tf.Tensor":
        ...
        if shuffle:
            # Shuffle.
            prepared_vectors = prepared_vectors.shuffle(
                vectors.shape[0], reshuffle_each_iteration=True
            )
