...
def get_date_range(oid: int, month_range, start_day, end_day, second):
    for month in month_range:
        url = 'https://api.bilibili.com/x/v2/dm/history/index?type=1&oid={}&month={}'.format(oid, month)
        date_list = requests.get(url=url, headers=head.headers, cookies=head.cookie()).json()
        date_range = []
        if date_list['data'] is not None:
            date_range = date_list['data']
        time.sleep(second)
        date_range = before_remove(date_range, start_day)
        date_range = after_remove(date_range, end_day)
        get_danmaku(oid, date_range, second)


def get_danmaku(oid: int, date_range, second):
    if date_range:
        for date in date_range:
            url = 'https://api.bilibili.com/x/v2/dm/web/history/seg.so?type=1&oid={}&date={}'.format(oid, date)
            response = requests.get(url=url, headers=head.headers, cookies=head.cookie()).content
...
def main(start_day, end_day, oid, second):
    if not os.path.exists(str(oid)):
        os.mkdir(str(oid))
    month_range = get_month_list(start_day, end_day)
    print('开始获取弹幕')
    try:
        get_date_range(oid, month_range, start_day, end_day, second)
        print('开始合并弹幕')
