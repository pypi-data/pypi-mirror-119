...
from tecton_spark.time_utils import convert_proto_duration_for_version
...
def _get_all_partial_aggregation_tiles_df(
    ...
    if not use_materialized_data:
        ...
        plan = materialization_plan.get_batch_materialization_plan(
            ...
            schedule_interval=fpov.batch_materialization_schedule,
        )
...
def construct_spine_with_all_join_keys_for_TAFP(
    ...
    max_aggregation_window = convert_proto_duration_for_version(
        fpov.max_aggregation_window, version=fpov.get_feature_store_format_version
    )
...
def construct_full_tafp_df(
    ...
    if not spine_df:
        ...
        partial_aggregations_df = all_partial_aggregations_df.withColumn(
            ...
            + convert_timedelta_for_version(fpov.get_tile_interval, fpov.get_feature_store_format_version),
        )
    else:
    ...
    for feature in time_aggregation.features:
        ...
        upper_range = -(convert_timedelta_for_version(window_duration, version)) + 1
        window_spec = (
