...
def is_spark3():
    return SparkContext._active_spark_context.version.startswith("3.")
...
def get_query_plan_info(spark, sql: Optional[str], transformation: Optional[Transformation]) -> QueryPlanInfo:
    if sql:
        plan = spark._jsparkSession.sessionState().sqlParser().parsePlan(sql)
    elif transformation:
        # Get logical plan
        plan = transformation.dataframe(spark)._jdf.queryExecution().logical()
    else:
        raise ValueError("Neither SQL not Transformation provided")
...
def get_table_names(spark, sql: str):
    """Get all the table names of the given the SQL."""
    return get_query_plan_info(spark, sql, None).table_names
