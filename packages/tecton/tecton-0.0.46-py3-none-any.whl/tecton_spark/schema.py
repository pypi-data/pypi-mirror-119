...
TECTON_TO_SPARK_TYPE = {v: k for k, v in SPARK_TO_TECTON_TYPE.items()}
...
class Schema:
    ...
    def from_spark(cls, spark_schema):
        ...
        for field in spark_dict["fields"]:
            ...
            if not isinstance(column_type, dict):
                tecton_type = SPARK_TO_TECTON_TYPE.get(column_type, None)
            else:
                if column_type["type"] == "array" and column_type["elementType"] == "string":
                    tecton_type = column_type_pb2.COLUMN_TYPE_STRING_ARRAY
                    column_type = "string_array"
                else:
                    tecton_type = None
            ...
            if tecton_type:
                ...
                # Timestamp is an exception because it is required in temporal
                # package's view_sql but does not have a ColumnType because it
                # is not written in materialized data.
                pass
            else:
    ...
    def to_spark(self):
        ...
        ret = StructType()
        for col_name, col_type in self.column_name_types():
            ret.add(col_name, col_type)
        return ret
