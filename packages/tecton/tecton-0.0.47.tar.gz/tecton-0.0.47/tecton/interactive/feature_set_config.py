from typing import List
from typing import Optional
from typing import Tuple
from typing import Union

import attr
import pandas
import pyspark

from tecton._internals.sdk_decorators import sdk_public_method
from tecton.interactive.data_frame import DataFrame
from tecton.interactive.feature_package import FeaturePackage
from tecton_proto.data.feature_service_pb2 import FeatureSetItem
from tecton_spark.feature_package_view import FeaturePackageOrView
from tecton_spark.feature_package_view import FPBackedFeaturePackageOrView
from tecton_spark.logger import get_logger

logger = get_logger("FeatureSetConfig")


@attr.s(frozen=True, auto_attribs=True)
class _Key(object):
    name: str
    namespace: str


@attr.s(frozen=True, auto_attribs=True)
class FeatureDefinitionAndJoinConfig(object):
    """
    A FeaturePackage and its associated join configuration.

    :param package: The Feature Package.
    :param join_keys: The mapping from FeatureService's join keys to FeaturePackage's join keys.
    :param namespace: The namespace.
    """

    definition: FeaturePackageOrView
    name: str
    join_keys: List[Tuple[str, str]]
    namespace: str
    features: List[str]

    @classmethod
    def _from_proto(cls, feature_set_item: FeatureSetItem):
        join_keys = [(i.spine_column_name, i.package_column_name) for i in feature_set_item.join_configuration_items]
        if feature_set_item.enrichments.HasField("feature_view"):
            definition = FeaturePackageOrView.from_fv(feature_set_item.enrichments.feature_view)
        else:
            definition = FeaturePackageOrView.from_fp(feature_set_item.enrichments.feature_package)
        return FeatureDefinitionAndJoinConfig(
            definition=definition,
            name=definition.name,
            join_keys=join_keys,
            namespace=feature_set_item.namespace,
            features=list(feature_set_item.feature_columns),
        )

    def _key(self) -> _Key:
        return _Key(namespace=self.namespace or "", name=self.name)


class FeatureSetConfig(object):
    """
    Config used to create a :class:`FeatureService`.
    """

    _definitions_and_configs: List[FeatureDefinitionAndJoinConfig]

    def __init__(self):
        """
        Initialize a new FeatureSetConfig.
        """

        self._definitions_and_configs = []

    def _add(
        self,
        fpov: FeaturePackageOrView,
        namespace: Optional[str] = None,
    ):
        if namespace is None:
            namespace = fpov.name

        package_and_config = FeatureSetConfig._make_definition_and_config(fpov, namespace=namespace)
        self._definitions_and_configs.append(package_and_config)

    @property  # type: ignore
    @sdk_public_method
    def feature_packages(self) -> List[FeaturePackageOrView]:
        """
        Returns the Feature Packages enclosed in this FeatureSetConfig.
        """
        return [config.definition for config in self._get_feature_definitions_and_join_configs()]

    @property  # type: ignore
    @sdk_public_method
    def features(self) -> List[str]:
        """
        Returns the features generated by the enclosed feature packages.
        """
        return [
            features
            for config in self._get_feature_definitions_and_join_configs()
            for features in FeatureSetConfig._get_full_feature_names(config)
        ]

    @staticmethod
    def _get_full_feature_names(config: FeatureDefinitionAndJoinConfig):
        return [config.namespace + "." + feature if config.namespace else feature for feature in config.features]

    @staticmethod
    def _make_package_and_config(feature_package: FeaturePackage, namespace: str) -> FeatureDefinitionAndJoinConfig:
        join_keys = [(join_key, join_key) for join_key in feature_package.join_keys]
        features = feature_package.features

        return FeatureDefinitionAndJoinConfig(
            definition=FPBackedFeaturePackageOrView(feature_package),
            name=feature_package.name,
            join_keys=join_keys,
            namespace=namespace,
            features=features,
        )

    @staticmethod
    def _make_definition_and_config(fpov: FeaturePackageOrView, namespace: str) -> FeatureDefinitionAndJoinConfig:
        join_keys = [(join_key, join_key) for join_key in fpov.join_keys]
        features = fpov.features

        return FeatureDefinitionAndJoinConfig(
            definition=fpov, name=fpov.name, join_keys=join_keys, namespace=namespace, features=features
        )

    def _get_feature_definitions_and_join_configs(self) -> List[FeatureDefinitionAndJoinConfig]:
        """
        Returns a list of all FeaturePackages and their configs in the insertion order.
        """
        return self._definitions_and_configs

    @sdk_public_method
    def get_feature_dataframe(
        self,
        spine: Union[str, dict, "pyspark.sql.DataFrame", list, "pandas.DataFrame", "pyspark.RDD", None],
        timestamp_key: str,
        include_feature_package_timestamp_columns=False,
        use_materialized_data: bool = True,
    ) -> DataFrame:
        """
        Creates a feature vector dataframe for the specified entities, including the entities' features.

        :param spine: SQL string that fetches the entity ids for which the features will be generated or a dataframe.
        :param timestamp_key: Name of the time column in spine. The column must be of type Spark timestamp.
        :param include_feature_package_timestamp_columns: (Optional) Include timestamp columns for every individual FeaturePackage.
        :param use_materialized_data: (Optional) Use materialized data if materialization is enabled.
        :return: A Tecton DataFrame.
        """
        from tecton.tecton_context import TectonContext

        tc = TectonContext.get_instance()
        return tc.execute(
            spine,
            feature_set_config=self,
            timestamp_key=timestamp_key,
            include_feature_package_timestamp_columns=include_feature_package_timestamp_columns,
            use_materialized_data=use_materialized_data,
        )

    # TODO: Change this to a @sdk_public_method once it's fully ready.
    def get_feature_sql_str(
        self,
        spine: str,
        timestamp_key: str,
        include_feature_package_timestamp_columns=False,
        use_materialized_data: bool = True,
    ) -> DataFrame:
        """
        Creates a sql str to fetch feature vector for the specified entities, including the entities' features.

        :param spine: SQL string that fetches the entity ids for which the features will be generated or a dataframe.
        :param timestamp_key: Name of the time column in spine. The column must be of type Spark timestamp.
        :param include_feature_package_timestamp_columns: (Optional) Include timestamp columns for every individual FeaturePackage.
        :param use_materialized_data: (Optional) Use materialized data if materialization is enabled.
        :return: A Tecton DataFrame.
        """
        # TODO: Generate sql str to return here
        return ""

    @classmethod
    def _from_protos(cls, feature_set_items):
        config = FeatureSetConfig()
        config._definitions_and_configs = [FeatureDefinitionAndJoinConfig._from_proto(p) for p in feature_set_items]
        return config
