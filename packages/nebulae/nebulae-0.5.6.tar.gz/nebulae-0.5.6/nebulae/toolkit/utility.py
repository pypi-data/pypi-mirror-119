...
class GPUtil():
    def __init__(self):
        ...
        with subps.Popen(['nvidia-smi', 'query-gpu=name', '--format=csv'], stdout=subps.PIPE) as p:
            ...
            for line in p.stdout.readlines():
                self.gpus.append(line.decode('utf-8'))
    ...
    def monitor(self):
        ...
        self.process = subps.Popen(['timeout', '3600', 'nvidia-smi',
    ...
    def status(self):
        ...
        with open('./temp_gpu_stat.csv', 'r') as f:
            ...
            while True:
                ...
                if len(line)!=4 or line[-1][-3:]!='MiB':
                    continue
                t[gpu_id].append(int(line[1]))
                u[gpu_id].append(int(line[2][:-2]))
                m[gpu_id].append(int(line[3][:-4]))
                gpu_id = (gpu_id + 1) % n
        t = [sum(T) / len(T) for T in t]
        u = [sum(U) / len(U) for U in u]
        m = [sum(M) / len(M) for M in m]
        stat = '+' + 72 * '-' + '+'
        stat += '\n| GPU | ' + 10 * ' ' + 'Name' + 11 * ' ' + ' | T.Celsius | VUtil |   Memory   |'
        for i, g in enumerate(self.gpus):
            stat += '\n| {:^3d} | {:<25s} |  {:<2.1f}ºC   | {:2.1f}% | {:<5.1f}MiB |'.format(
                i, g, t[i], u[i], m[i])
        stat += '\n+' + 72 * '-' + '+'
        return stat