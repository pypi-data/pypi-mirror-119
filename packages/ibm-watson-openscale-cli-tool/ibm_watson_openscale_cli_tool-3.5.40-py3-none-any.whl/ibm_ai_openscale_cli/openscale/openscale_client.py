...
from ibm_ai_openscale_cli.utility_classes.constants import DRIFT_ARCHIVE_ENV_MAPPING
...
class OpenScaleClient(OpenScaleReset):
    ...
    def set_environment(self) -> None:
        """
        Sets the environment based on the version from fairness heartbeat.

        :returns: None
        """

        # Calling the fairness heartbeat API
        fairness_heartbeat_response = self.get_fairness_heartbeat()

        # Getting the fairness version
        fairness_version = fairness_heartbeat_response.get("version")

        if fairness_version.startswith("1"):
            self.env = Environment.PUBLIC_CLOUD.value
        elif fairness_version.startswith("3"):
            self.env = Environment.CPD_3_X.value
        elif fairness_version.startswith("4.0.0"):
            self.env = Environment.CPD_4_0_0.value
        elif fairness_version.startswith("4.0.1"):
            self.env = Environment.CPD_4_0_1.value
        elif fairness_version.startswith("4.0.2"):
            self.env = Environment.CPD_4_0_2_PLUS.value
        else:
            self.env = Environment.CPD_4_0_2_PLUS.value
    ...
    def configure_drift(self, params):
        if params:
            ...
            scikit_folder = DRIFT_ARCHIVE_ENV_MAPPING[self.env]

            filename = "drift_archives/{}/{}".format(scikit_folder, Model.DRIFT_MODEL_FILENAME)
    ...
    def get_fairness_heartbeat(self) -> dict:
        """
        ...
        url = "{}/v1/fairness_monitoring/heartbeat".format(self._credentials["url"])
