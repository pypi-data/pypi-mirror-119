...
class FiltrationData(BaseModel):
    ...
    def __get_sort_spec(self) -> list[ModelSortSpec]:
        if self.order_by is None:
            raise ValueError("order_by is None, can not get a sort spec.")

        res = _split_field(self.order_by) | {"direction": self.sort_by}
        if "model" not in res and self.default_model is not None:
            res |= {"model": self.default_model}
        return [res]  # type: ignore

    def apply(self, query: Query) -> DataTable:
        if self.default_model is not None:
            self.to_filter = self.to_filter.set_default_model(self.default_model)
