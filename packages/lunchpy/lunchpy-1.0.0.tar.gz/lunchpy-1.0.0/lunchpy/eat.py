...
class Eat:
    ...
    def __init__(self, api_key: str = '', endpoint: str = "", version: str = 'v1'):
        ...
        """
        if endpoint:
            self.endpoint = f"{endpoint.rstrip('/')}/{version.rstrip('/').lstrip('/')}/"
        else:
            self.endpoint = f'https://dev.lunchmoney.app/{version}/'
        if api_key == '':
            key = os.getenv("LUNCH_MONEY_API_KEY")
            if not key:
                raise NoEnvApiKeyError
            api_key = key
        self.headers: dict[str, str] = {'content-type': 'application/json',
                                        'Authorization': f'Bearer {api_key}'}

    @staticmethod
    def _objectify(res: dict, ident: str = '', cl=LunchPyResultObject) -> list[LunchPyResultObject]:
        """Turn a dict of dicts into a list of LunchPyResultObjects (or whatever class is supplied as cl)"""
        if ident != '':
            try:
                res = res[ident]
            except KeyError:
    ...
    def _query(self, endpoint: str, method: str = 'GET', extra_headers: dict = None,
    ...
    def categories(self, **kwargs) -> list[LunchPyResultObject]:
    ...
    def transactions(self, **kwargs) -> list[LunchPyResultObject]:
    ...
    def transaction(self, tid: int, debit_as_negative: bool = False, **kwargs) -> LunchPyResultObject:
        ...
        Raises:
            RequestError: An error direct from Lunch Money. It will contain details.
        """
        resp = self._query(f'transactions/{tid}', params={'debit_as_negative': str(debit_as_negative)} | kwargs)
        return LunchPyResultObject(resp)

    def tags(self, **kwargs) -> list[LunchPyResultObject]:
    ...
    def recurring_expenses(self, **kwargs) -> list[LunchPyResultObject]:
    ...
    def budgets(self, start_date: str = None, end_date: str = None, **kwargs) -> list[LunchPyResultObject]:
    ...
    def assets(self, **kwargs) -> list[LunchPyResultObject]:
    ...
    def plaid_accounts(self, **kwargs) -> list[LunchPyResultObject]:
    ...
    def crypto(self, **kwargs) -> list[LunchPyResultObject]:
        """
    ...
    def del_group(self, tx_id: int) -> list[int]:
    ...
    def create_transactions(self, transactions: list[dict], **kwargs) -> list[int]:
    ...
    def create_transaction_group(self, date: str, payee: str, transactions: list[int], **kwargs):
    ...
    def update_transaction(self, transaction: dict, tx_id: int = None, split: dict = None, **kwargs) \
            -> tuple[bool, list[int]]:
