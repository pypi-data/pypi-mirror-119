...
class TeacherCourseInline(BaseModelTabularInline):
    model = Course.teacher_obj.through
    fields = ['teacher_obj', 'priority', ]
    raw_id_fields = ('teacher_obj',)
    extra = 1
    exclude = []

    def __init__(self, *args, **kwargs):
        Klass = TeacherCourseInline
...
class CourseAdmin(BaseCourseAdmin):
    ...
    list_filter = [
        ParentFilter,
        DependencyFilter,
    ]# 'publish_month', 'publish_week', 'publish_day']
    raw_id_fields = []
    exclude = []
    dynamic_raw_id_fields = [
        'category_obj',
