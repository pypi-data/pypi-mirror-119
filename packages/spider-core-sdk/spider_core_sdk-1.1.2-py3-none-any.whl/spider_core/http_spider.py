...
class HttpSpider(Spider):
    def __init__(self, instance_id, spider_id=None, **kwargs):
        ...
        self.api.init_setting(self.TEST_ENV)
        self.redis_key = ''
        self.idle_count = 0
        self.item_list = []
    ...
    def fetch_data(self):
        ...
        for i in range(1):
            ...
            if data and data['code'] == 200:
                ...
                self.logger.debug(f'suc fetch_data: {data} instance_id: {self.instance_id}')
            else:
                self.redis_key = ''
                self.logger.debug(f'error fetch_data: {data} instance_id: {self.instance_id}')
                break
        return datas
