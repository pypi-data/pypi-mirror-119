...
def est_aberr(test_ims, test_aberrs, pix_size_um=0.08, num_aperture=0.004, stig_rot_deg=0,
    ...
    if cropping_valid:
        ...
        [Image.fromarray(tt).save("E:/autofocus/hessian_evolution/exp2/im" + str(crop_ref) + "_" + str(iim) + ".tif")
...
def test_installation():
    im1 = np.array(Image.open(
        requests.get('https://gitlab.com/rangolisaxena90/mapfost/-/raw/main/src/mapfost/example/test_im1.png',
                     stream=True).raw))
    im2 = np.array(Image.open(
        requests.get('https://gitlab.com/rangolisaxena90/mapfost/-/raw/main/src/mapfost/example/test_im2.png',
                     stream=True).raw))
