...
class InferPytorch:
    ...
    def __init__(self, model_dir: str):
        """
        :param model_dir: directory from where to load model config
        :paramtype model_dir: str
        """
    ...
    def infer(self, bgr_img, verbose: bool = True) -> str:
        ...
        :param verbose: if True print all details such as time taken for inference, probabilities of all classes and predicted class
        :type verbose: bool
        ...
        if verbose:
            ...
            print("{:15s} | {:15s}".format("Predictions", "Probabilities"))
            print("-" * 30)
            ...
            print(
                "Top-1 Prediction: {} | Probability: {:.2f}%".format(
                    self.class_mapping[str(argmax)], probs[argmax] * 100
                )
            )
