# Adapted from: https://github.com/PRBonn/bonnetal/blob/master/train/common/trtCalibINT8.py
import numpy as np
import cv2
import os
...
class EntropyCalibrator(trt.IInt8EntropyCalibrator2):
    """
    Prepare callibartion dataset for int8 callibration.

    :param model_path: Path to the model
    :type model_path: str
    :param calibration_files: callibaration image paths
    :type calibration_files: list
    :param batch_size: batch size
    :type batch_size: int
    :param h: input size height
    :type h: int
    :param w: input size width
    :type w: int
    :param means: means
    :type means: float
    ...
    def __init__(self, model_path, calibration_files, batch_size, h, w, means, stds):
        """
        :param model_path: directory path where model will be loaded from
        :type model_path: str
        :param calibration_files: path to calibration image files
        :type calibration_files: str
        ...
        def load_batches():
            ...
            """
            for i in range(
                0, len(self.calib_image_paths) - self.batch_size + 1, self.batch_size
            ):
    ...
    def read_batch_file(self, filenames):
        ...
        :returntype: np.ascontiguousarray
        """
        tensors = []
    ...
    def get_batch(self, names):
        ...
        :returns: List[batch from self.batches iter]
        :returntype: List
        """
        try:
    ...
    def read_calibration_cache(self):
        """
        reads calibration cache

        :returns: file content from self.cache_file
    ...
    def write_calibration_cache(self, cache):
        """
        writes cache to self.cache_file
