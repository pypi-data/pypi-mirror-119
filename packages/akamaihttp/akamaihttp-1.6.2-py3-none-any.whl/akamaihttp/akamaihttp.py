...
class EdgeGridHttpCaller():
    ...
    def getResult(self, endpoint,params=None):
        """ Executes a GET API call and returns the JSON output """
        path = endpoint
        endpoint_result = self.session.get(parse.urljoin(self.baseurl,path), params=params)
...
def AkamaiHTTPHandler(edgercLocation):
    section = 'default'
    debug = False
    verbose = False
    edgerc = EdgeRc(edgercLocation)
    host = edgerc.get(section, 'host')
    baseurl_prd = 'https://' + host
