...
class Agent(Insight, Channel, Catalog, Dataset):
    ...
    def get_dataset_by_slug(self, agent_slug, slug: str) -> Dataset:
        s = requests.Session()
        s.keep_alive = False
        retries = Retry(total=5, backoff_factor=1, status_forcelist=[502, 503, 504])
        s.mount('https://', HTTPAdapter(max_retries=retries))
...
class Askdata(SignUp):
    ...
    def get(self, query, workspace='', dataset='', lang="en"):

        if self._env == 'dev':
            base_url = url_list['BASE_URL_ASKDATA_DEV']
        elif self._env == 'qa':
            base_url = url_list['BASE_URL_ASKDATA_QA']
        elif self._env == 'prod':
            base_url = url_list['BASE_URL_ASKDATA_PROD']
        ...
        if(workspace == '' and dataset==''):
            ...
            try:
                ...
                authentication_url = base_url+ '/smartbot/agents/agentslug/' + workspace
            ...
            try:
                ...
                authentication_url = base_url+'/smartbot/agents/agentslug/'+workspace + '/datasetslug/' + slug
        ...
        url = base_url + "/smartbot/agents/"+agent_id+"/ask"
        ...
        #print("{} from the dataset: {} / {}".format(self.extract_fields(result[0]["attachment"]["body"][0]["details"]["columns"]), workspace, result[0]["attachment"]["body"][0]["details"]["datasetName"]))
        display(HTML("<p>{} from the dataset: {} / {}</p>".format(self.extract_fields(result[0]["attachment"]["body"][0]["details"]["columns"]), workspace, result[0]["attachment"]["body"][0]["details"]["datasetName"])))
        if(len(result[0]["attachment"]["body"])>1):
            print("Filtered by: {}".format(result[0]["attachment"]["body"][1]["details"]["text"]))
        if(len(result)>1):
            print("{} Alternatives:".format(len(result)-1))
            for r in result[1:]:
                print("     {} from the dataset: {} / {}".format(self.extract_fields(r["attachment"]["body"][0]["details"]["columns"]), workspace, r["attachment"]["body"][0]["details"]["datasetName"]))

        df = pd.DataFrame(data=result[0]["attachment"]["body"][0]["details"]["rows"], columns=result[0]["attachment"]["body"][0]["details"]["columns"])
        return df
