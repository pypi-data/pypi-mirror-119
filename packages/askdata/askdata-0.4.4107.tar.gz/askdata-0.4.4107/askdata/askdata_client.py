...
class Askdata(SignUp):
    ...
    def get(self, query, workspace_slug, dataset_slug, lang="en"):
        ...
        get_agent_url = base_url + "/agents/agentslug/"+workspace_slug
        s = requests.Session()
        s.keep_alive = False
        retries = Retry(total=5, backoff_factor=1, status_forcelist=[502, 503, 504])
        s.mount('https://', HTTPAdapter(max_retries=retries))

        response = s.get(url=get_agent_url, headers=self._headers)
        response.raise_for_status()
