...
class Agent(Insight, Channel, Catalog, Dataset):
    ...
    def query(self, dataset_slug="", fields:list=[], filters=None, sorted_by=None,  dataset_id="", limit=50):
        ...
        if(dataset_id=="" and dataset_slug!=""):
            ...
            url = self._base_url_askdata + '/smartbot/agents/agentslug/' + self.slug + '/datasetslug/' + dataset_slug
    ...
    def load_dataset(self, dataset_slug, aliases:list=[], filters:list=[]):
        ...
        dataset_id = self.get_dataset_by_slug(self._agentId, dataset_slug)
    ...
    def create_or_replace_dataset(self, dataframe: pd.DataFrame, dataset_name: str, slug: str, icon_url=None,
        ...
        dataset_id = self.get_dataset_by_slug(self.slug, slug)
...
class Askdata(SignUp):
    ...
    def get(self, query, workspace, dataset, lang="en"):
        if self._env == 'dev':
            base_url = url_list['BASE_URL_INSIGHT_DEV']
        elif self._env == 'qa':
            base_url = url_list['BASE_URL_INSIGHT_QA']
        elif self._env == 'prod':
            base_url = url_list['BASE_URL_INSIGHT_PROD']

        url = base_url + "/data/nl/result"
