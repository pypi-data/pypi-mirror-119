...
class figure:
    ...
    def __init__(self, dark=False, despine=0):
        raise Exception(f"ERRoR {style}", style)
        # Member variables
        self._dark = dark
        ...
        if self._dark:
            ...
            pass
        reload(plt)
        self._base = plt.figure(facecolor=theme.base if self._dark else None)
    ...
    def logo(self, alpha=1.):
        ...
        inset.imshow(
            theme.images['normal' if self._dark else 'inverted'], interpolation='none', alpha=alpha)
        return
    ...
    def title(self, *args, **kwargs):
        ...
        defaults = {
            ...
            'x': 0.00,
            'y': 1.02,
            'fontdict': {'family': 'serif', 'weight': 'bold', 'size': 14, "color": 'white' if self._dark else theme.base}
        }
