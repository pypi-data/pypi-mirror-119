...
#SERVER_ADDRESS = 'dataops-api-server-snr3asztcq-uk.a.run.app'
PORT = 443
...
class _Client(object):
    def __init__(self, token="", billing_account=HARDCODED_BILLING_ACCOUNT):

        # TODO we should be able to find the users bacc out of this and remove the ingestion of the billing account
    ...
    def get_dimension(self, catalog_id: str, dimension_id: str):
        res = sync(self.dimension_operator_service.list_dimension(catalog_id=catalog_id))
...
class Client(object):
    def __new__(self, token="", billing_account=HARDCODED_BILLING_ACCOUNT, catalog='rwg'):

        return _Client(billing_account=billing_account, 
                            token=token
                            ).catalog(catalog)