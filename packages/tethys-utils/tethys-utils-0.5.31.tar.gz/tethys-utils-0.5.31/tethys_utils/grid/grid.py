...
class Processor(object):
    ...
    def save_grid_chunks(self, output_path, xy_size=None, xy_size_multiplier=10):
        ...
        ## Open grid
        grid1 = self.load_raw_grid(self.source_data_paths, self._lon_dim_name, self._lat_dim_name, self._time_dim_name, chunks=chunks, parallel=True, preprocessor=self._preprocessor, postprocessor=self._postprocessor, **self._kwargs)
    ...
    def determine_grid_block_size(self, min_size=1000, max_size=2000, block_size=10):
        ...
        for dataset in dataset_list:
            ...
            # calc obj sizes
            grid1 = self.load_raw_grid(self.source_data_paths, self._lon_dim_name, self._lat_dim_name, self._time_dim_name, chunks={self._time_dim_name: self._dims['time'], self._lon_dim_name: 1, self._lat_dim_name: 1}, parallel=True, preprocessor=self._preprocessor, postprocessor=self._postprocessor, **self._kwargs)
...
class Grid(Titan):
    ...
    def load_results(self, xy_size, results=None, dataset_id=None, sum_closed='right', other_closed='left', discrete=True, other_attrs=None, other_encoding=None, run_date=None, skip_resampling=True):
        """
