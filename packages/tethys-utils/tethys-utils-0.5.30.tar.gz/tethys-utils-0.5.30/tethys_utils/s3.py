...
import xarray as xr
import orjson
from tethys_utils.data_models import Dataset, S3ObjectKey, Station
...
def list_objects_s3(s3_client, bucket, prefix, start_after='', delimiter='', continuation_token=''):
    """
...
def put_remote_dataset(s3, bucket, dataset, stns, run_date=None, version=3):
    """
    ...
    if dataset['spatial_distribution'] == 'sparse':
    ...
    ds_key = key_patterns[version]['dataset'].format(dataset_id=dataset_id)
...
def get_remote_dataset(s3, bucket, dataset_id=None, ds_key=None, version=3):
    """
...
def put_remote_station(s3, bucket, station, run_date=None, version=3):
    """
...
def put_remote_agg_stations(s3, bucket, dataset_id, threads=30, version=3):
    """

    """
    stn_key = key_patterns[version]['station']
    ...
    if not list2.empty:
        ...
        stn_list = [[s3, bucket, None, None, k, version] for k in list2.Key]

        output = ThreadPool(threads).starmap(get_remote_station, stn_list)

        ## Separate the results obj keys from the stations
        results_obj_keys = [{'dataset_id': s['dataset_id'], 'station_id': s['station_id'], 'results_object_key': s['results_object_key']} for s in output]
        [s.update({'results_object_key': s['results_object_key'][-1]}) for s in output]
        ...
        agg_stn_key = key_patterns[version]['stations']
        ...
        # Results object keys
        ro_key = key_patterns[version]['results_object_keys'].format(dataset_id=dataset_id)

        ro_obj = write_json_zstd(results_obj_keys)

        s3.put_object(Body=ro_obj, Bucket=bucket, Key=ro_key, ContentType='application/json', Metadata={'run_date': run_date_key})
...
def put_remote_agg_datasets(s3, bucket, threads=30, version=3):
    """

    """
    ds_key = key_patterns[version]['dataset']
    ...
    ds_list = [[s3, bucket, None, k, version] for k in list2.Key]
    ...
    run_date_key = make_run_date_key()
    dss_key = key_patterns[version]['datasets']
    s3.put_object(Body=dss_obj, Bucket=bucket, Key=dss_key, ContentType='application/json', Metadata={'run_date': run_date_key})

    return output


def compare_datasets_from_s3(conn_config, bucket, new_data, add_old=False, last_run_date_key=None, public_url=None, version=3):
    """
    Parameters
    ----------
    conn_config : dict
    ...
    base_key_pattern = key_patterns[version]['results']
...
def update_results_s3(processing_code, data_dict, run_date_dict, remote, threads=30, public_url=None, version=3):
    ...
    for ds_id, results in data_dict.items():
        ...
        if len(results) > 0:
            ...
            prefix = key_patterns[version]['results'].split('{station_id}')[0].format(dataset_id=ds_id)
            ...
            def update_result(result):
                ...
                if isinstance(up1, xr.Dataset) and (len(up1[parameter].time) > 0):
                    ...
                    new_key = key_patterns[version]['results'].format(**key_dict)
...
def delete_result_objects_s3(conn_config, bucket, dataset_ids=None, keep_last=10, threads=50, version=3):
    ...
    prefix = key_patterns[version]['results'].split('{dataset_id}')[0]
...
def process_run_date(processing_code, dataset_list, connection_config, bucket, run_date=None, save_interval_hours=336, version=3):
    """
    ...
    for ds in dataset_list:
        ...
        try:
            key1 = key_patterns[version]['stations'].format(dataset_id=dataset_id)
...
def reprocess_datasets(dataset_id, conn_config, bucket, public_url, threads=20):
    """
    ...
    ds = tethys._datasets[dataset_id]

    bad_stns = [s for s in stns if 'time_range' not in s]

    prefix = key_patterns[2]['results'].split('{station_id}')[0].format(dataset_id=dataset_id)
    ...
    def reprocess_dataset(stn):
        ...
        # run_date = stn['results_object_key']['run_date']
            ...
            new_key = key_patterns[2]['results'].format(**key_dict)
    ...
    if len(bad_stns) > 0:
        ...
        if ds_stations is not None:
            ds_new = put_remote_dataset(s3, bucket, ds, ds_stations, version=2)
