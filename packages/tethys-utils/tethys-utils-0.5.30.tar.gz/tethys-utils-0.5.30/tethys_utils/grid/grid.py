...
class Processor(object):
    ...
    def determine_grid_block_size(self, dataset_id, min_size=1000, max_size=1600, samples=10):
        """
        ...
        ## get sample points
        pts_df = self._select_sample_points(samples)
        ...
        mean_size = int(sum(obj_sizes)/samples)
        ...
        if (sum(obj_sizes)/2 > min_size*1000) or (sum(obj_sizes) > min_size*1000):
            ...
            print('xy size estimate is ' + str(xy_size))
