...
def put_remote_agg_stations(s3, bucket, dataset_id, threads=30, version=3):
    ...
    if version == 2:
        ...
        list1 = list_objects_s3(s3, bucket, stn_prefix, delimeter='/')
    else:
...
def put_remote_agg_datasets(s3, bucket, threads=30, version=3):
    ...
    if version == 2:
        ...
        list1 = list_objects_s3(s3, bucket, ds_prefix, delimeter='/')
    else:
        raise ValueError('Wrong object structure version.')
...
def process_run_date(processing_code, dataset_list, remote, connection_config, bucket, run_date=None, save_interval_hours=336, version=3):
    """
    ...
    for ds in dataset_list:
        ...
        try:
            ...
            rok2 = pd.DataFrame([r['results_object_key'] for r in rok1])
