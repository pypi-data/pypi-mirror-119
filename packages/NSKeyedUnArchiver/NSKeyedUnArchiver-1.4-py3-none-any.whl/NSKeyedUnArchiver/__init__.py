...
#             print(f"cursor={k}")
            if isinstance(reassembled[k],plistlib.UID):
                ...
                reassembled[k]=_unserialize(reassembled[k], serialized, removeClassName=removeClassName, start=False)
                ...
                if isinstance(reassembled[k],dict) and '$class' in reassembled[k] and removeClassName:
