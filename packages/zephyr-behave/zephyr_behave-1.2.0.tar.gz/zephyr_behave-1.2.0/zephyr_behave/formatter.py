...
class ZephyrFormatter(Formatter):
    ...
    api_key = None
    test_cycle_key = None
    start_date_time = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.000+0000")
    url_base = None
    ...
    def __init__(self, stream_opener, config):
        ...
        api_key = self.config.userdata.get("ZEPHYR_API_KEY", None)
        if not api_key:  # I know... this is unfortuantely a hack for a short comming
            api_key = self.config.userdata.get("zephyr_api_key", "")

        self.api_key = api_key
        self.url_base = self.config.userdata.get("ZEPHYR_API_URL", "")
    ...
    def close(self):
        ...
        if self.api_key:
            self.upload_results()
            self.update_testCycle()
        else:
            print("No ZEPHYR_API_KEY provided.  No updates send to Zepker")
        self.file_cleanup()
    ...
    def upload_results(self):
        upload_results = self.config.userdata.get(
            "ZEPHYR_UPLOAD_RESULTS", "false"
        ).lower()
        ...
        create_test_cases = self.config.userdata.get(
            "ZEPHYR_AUTO_CREATE_TEST_CASES", "false"
        ).lower()
        url = f"{self.url_base}/v2/automations/executions/cucumber?projectKey={zephyr_project_key}&autoCreateTestCases={create_test_cases}"
        ...
        print(f"Test cycle key: {response.json()['testCycle']['key']}")
        self.test_cycle_key = response.json()["testCycle"]["key"]

    def update_testCycle(self):
        # Get test cycle data from Zepyr
        url = f"{self.url_base}/v2/testcycles/{self.test_cycle_key}"
        headers = {"Authorization": self.api_key, "Content-Type": "application/json"}
        response = requests.request("GET", url, headers=headers)
        response.raise_for_status()

        # Add detail to response before posting it back to Zephyr
        data = response.json()
        test_name = self.config.userdata.get(
            "ZEPHYR_TEST_CYCLE_NAME",
            f"Automation Build {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        )
        data["name"] = test_name
        data["plannedStartDate"] = self.start_date_time
        data["plannedEndDate"] = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.000+0000")
