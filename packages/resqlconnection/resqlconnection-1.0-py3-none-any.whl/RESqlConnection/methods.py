...
class Method:
    ...
    def get_table_columns(self, table_name, schema_name, sql_conn):
        ...
        """
        columns = sql_conn.read_to_df("SELECT COLUMN_NAME,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS " +
    ...
    def get_table_info(self, table_name, schema_name, sql_conn):
        ...
        number_of_rows = sql_conn.read_to_df(f"Select Count(*) FROM {schema_name}.{table_name}").values[0][0]
    ...
    def get_all_tables_info(self, sql_conn):
        ...
        """
        val = sql_conn.read_to_df("SELECT TABLE_SCHEMA, TABLE_NAME FROM INFORMATION_SCHEMA.TABLES")
    ...
    def push_data(self, table_name, schema_name, input_data, column_name_list, value_columns, crawldate_column, replace,
                  ignore_duplicates_check, sql_conn, recipients):
        """
        ...
        col_list = self.get_table_columns(table_name, schema_name,
                                     sql_conn).values.tolist()  # using this in case they are not using column_name_list
        if max(len(col_list), len(column_name_list)) <= len(value_columns):
            self.send_email(sql_conn, recipients, "Every column cannot be a value column")
        ...
        if any(isinstance(i, list) for i in input_data):  # list in list
            ...
            self.df_to_db(table_name, schema_name, df, value_columns, replace, schema_created, sql_conn, ignore_duplicates_check,
                     crawldate_column, recipients)
            ...
            self.df_to_db(table_name, schema_name, input_data, value_columns, replace, schema_created, sql_conn,
                     ignore_duplicates_check, crawldate_column, recipients)
            ...
            self.df_to_db(table_name, schema_name, df, value_columns, replace, schema_created, sql_conn,
                     ignore_duplicates_check, crawldate_column, recipients)
        else:
            body = f"Did not recognize data structure for insert into {schema_name}.{table_name} crawler. " \
    ...
    def df_to_db(self, table_name, schema_name, df, value_columns, replace, schema_created, sql_conn, ignore_duplicates_check,
                 crawldate_column, recipients):
        ...
        else:  # not necessary with if else
            if "CrawlDate" not in df:
        ...
        try:
            ...
            df.to_sql(table_name, schema=schema_name, con=engine, if_exists="append", index=False, dtype=varchar_cols,
                      chunksize=int(math.floor(2100 / len(df.columns))) - 1, method='multi')
        except:
            try:
                ...
                self.send_email(sql_conn, recipients,
                          f"Adding to {schema_name}.{table_name} failed. The following error occured: " + str(e))
                raise Exception(f"Adding to {schema_name}.{table_name} failed. The following error occured: " + str(e))
        if not ignore_duplicates_check:
            self.remove_duplicates(table_name, schema_name, replace, value_columns, sql_conn, crawldate_column)
        ...
        if len(col_list) != 0:
            print(
                f"{after - before} rows added to: {table_name}. Before: {before} rows, after: {after} rows. Also added following new columns: " + " ,".join(
                    col_list))
        else:
            print(f"{after - before} rows added to: {table_name}. Before: {before} rows, after: {after} rows.")


    def remove_duplicates(self, table_name, schema_name, replace, value_columns, sql_conn, crawldate_column='CrawlDate'):
        """
        ...
        if replace:
            ...
            sql_val = "Asc"
        col_list = self.get_table_columns(table_name, schema_name, sql_conn).values.tolist()
        ...
        for i in col_list:
            if i[0] in value_columns or i[0] == "CrawlDate":
                ...
                col_string += f"[{i[0]}], "
        params = f"BEGIN WITH CTE AS (SELECT *, ROW_NUMBER() OVER" \
    ...
    def drop_schema(self, schema_name, sql_conn):
        ...
        :param sql_conn: Instance of SQLConnection class
        """
        df = sql_conn.read_to_df(f"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES Where TABLE_SCHEMA = '{schema_name}'")
    ...
    def monitor_crawler(self, table_name, schema_name, nr_of_lines, successful, engine):
        ...
        df = pd.DataFrame([[schema_name, table_name, nr_of_lines, successful, pd.pandas.Timestamp.now()]],
                          columns=["Schema", "Table", "Nr_of_rows", "Upload successful", "Timestamp"])
        df.to_sql("MonitoringTable", schema="Info", con=engine, if_exists="append", index=False, dtype=col_dtypes)
    ...
    def get_row_count(self, table_name, schema_name, sql_conn):
        ...
        try:
            df = sql_conn.read_to_df(f"SELECT COUNT(*) FROM [{schema_name}].[{table_name}]")
            return int(df.values[0][0])
        except:
            return 0


    def send_email(self, sql_conn, recipients, body,subject):
        """
        :param sql_conn: Instance of SQLConnection class
        ...
        if recipients is not None:
            ...
            query = "Begin EXEC msdb.dbo.sp_send_dbmail " \
    ...
    def get_newest_date(self, table_name, schema_name, col_name, sql_conn):
        ...
        try:
            ...
            res_df = sql_conn.read_to_df(f"Select Top 1 {col_name} From {schema_name}.{table_name} Order By {col_name} Desc")
