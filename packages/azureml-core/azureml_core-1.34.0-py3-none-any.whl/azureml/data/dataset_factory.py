...
class DataType:
    ...
    @staticmethod
    def to_stream(workspace):
        """Configure conversion to stream."""
        dt = DataType()
        dt._set_type_conversion(dataprep().StreamInfoConverter(workspace))
        return dt
...
def _transform_and_validate(dataflow, partition_format, include_path, validate, infer_column_types=False):
    ...
    if validate:
        ...
        _validate_has_data(dataflow, 'Cannot load any data from the specified path. '
                                     'To disable the data accessing, infer_column_types also need to be set to False.')
    return dataflow
