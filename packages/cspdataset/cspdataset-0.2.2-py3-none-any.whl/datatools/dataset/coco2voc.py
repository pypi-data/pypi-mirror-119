...
def get_CK5(origin_base_dir, output_voc_dir, data_types=None):
    ...
    for dataType in data_types:
        origin_Images_dir = os.path.join(origin_base_dir, 'Images', dataType)
        ...
        # 生成 voc trainval.txt
        dst_ImageSets_txt_path = os.path.join(dst_ImageSets_txt_dir, "trainval" + '.txt')
        img_filenames = []
...
def main(origin_base_dir, output_voc_dir, data_types=None):
    if data_types is None:
        data_types = ["train"]
...
if __name__ == "__main__":
    args = get_args()
    main(args.origin_base_dir, args.output_voc_dir, args.data_types)
