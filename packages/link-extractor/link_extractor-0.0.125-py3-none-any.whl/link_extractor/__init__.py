...
from .matters import filterMatters
...
def yieldLinks(soup):
	...
	for item in soup.find_all('div'):
		yield item.get('data-link'), item

def formatRawLink(link_package, domain, site):
	if not link_package:
		return
	link, item = link_package
	if not link:
		return
	...
	for key, sub_config in config.items():
		if key in site:
			...
			if not_contain and matchKey(str(item), not_contain):
				return
...
def getLinks(site):
	...
	if site.endswith('matters.news'):
		return list(filterMatters(links))
	return links