...
class MainController(DefConfigMixin):
    ...
    def __init__(self, config, logQueue=None, logProcessFacQueue=None, nolog=False):
        ...
        self.requiredvars = {
            ...
            'sql': {
                'default': """SELECT value FROM fugluconfig WHERE `section`=:section AND `scope` IN ('$GLOBAL',CONCAT('%',:to_domain),:to_address) AND `option`=:option ORDER BY `scope` DESC LIMIT 1""",
                'section': 'databaseconfig',
            ...
            'sqlsection': {
                'default': """SELECT value, option, scope FROM fugluconfig WHERE `section`=:section AND `scope` IN ('$GLOBAL',CONCAT('%',:to_domain),:to_address) ORDER BY `scope` DESC""",
                'section': 'databaseconfig',
            ...
            'm_ivmsg': {
                'default': "fuglu.plugins.ivmsg.IVMSendgrid",
                'section': 'PluginAlias',
            },

            'ivmfeed': {
                'default': "fuglu.plugins.ivmsg.IVMFeed",
                'section': 'PluginAlias',
            },
    ...
    def load_plugins(self, propagate_plugin_config: bool=False):
        ...
        newplugins, loadok = self._load_all(self.config.getlist('main', 'plugins'))
        if not loadok:
            allOK = False

        newprependers, loadok = self._load_all(self.config.getlist('main', 'prependers'))
        if not loadok:
            allOK = False

        newappenders, loadok = self._load_all(self.config.getlist('main', 'appenders'))
        if not loadok:
            allOK = False

        try:
            milterplugins, loadok = self._load_all(self.config.getlist('main', 'mplugins'))
            if not loadok:
                allOK = False
        except (configparser.NoSectionError, configparser.NoOptionError):
            milterplugins = []
    ...
    def _load_all(self, plugins):
        ...
        for plug in plugins:
            ...
            try:
                ...
                self.logger.error('Could not load plugin %s : %s' % (structured_name, e))
                allOK = False
            except Exception as e:
                CrashStore.store_exception()
                self.logger.error('Could not load plugin %s : %s' % (structured_name, e))
