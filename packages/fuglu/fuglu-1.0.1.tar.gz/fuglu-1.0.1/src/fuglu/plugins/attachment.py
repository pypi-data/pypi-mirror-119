...
#
#
from fuglu.shared import ScannerPlugin, DELETE, DUNNO, string_to_actioncode, FileList, get_outgoing_helo
...
class AttHashAction(ScannerPlugin):
    def __init__(self, config, section=None):
        ...
        self.requiredvars = {
            'blocklistconfig':{
                'default':'${confdir}/atthash-rbl.conf',
    ...
    def examine(self, suspect):
        ...
        for attobj in suspect.att_mgr.get_objectlist(level=0):
            ...
            myhash = attobj.get_checksum('sha256', Mailattachment.HASHENC_B32, strip=True)
    ...
    def examine(self, suspect):
        ...
        for attobj in suspect.att_mgr.get_objectlist(level=0):
            ...
            if not self._check_filename(filename, len(filecontent)):
                self.logger.debug('Skipping attachment size %s with filename: %s' % (len(filecontent), filename))
                continue
            ...
            if virusname is not None:
                ...
                self.logger.debug('no matching hash found for %s with hash %s' % (filename, myhash))
        ...
        if self.redis_pool is not None:
            ...
            while attempts:
                ...
                try:
                    ...
                    msg = f"problem in {self.__class__.__name__} getting virusname for haxh {myhash}"
    ...
    def insert(self, myhash, messagebytes, age=0):
        ...
        try:
            ...
            if result and result[1] == filesize:
                ...
                redisconn.hmset(myhash, dict(filename=filename, filesize=filesize, virusname=virusname))
