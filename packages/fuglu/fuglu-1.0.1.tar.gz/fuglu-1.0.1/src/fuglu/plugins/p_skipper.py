...
class PluginSkipper(PrependerPlugin):
    ...
    def __init__(self, config, section=None):
        ...
        self.requiredvars = {
            'filterfile': {
                'default': '${confdir}/skipplugins.regex',
                'description': 'path to file containing scanner plugin skip regex rules'
            }
        }
    ...
    def pluginlist(self, suspect, pluginlist):
        ...
        argsext = self.filter.get_args(suspect, True)
        # each arg should be a comma separated list of classnames to skip

        skipplugindict = {}
        for fieldname, strval, arg, pattern in argsext:
            plugnames = arg.split(',')
            for plugname in plugnames:
                skipplugindict[plugname] = fieldname

        listcopy = pluginlist[:]
        for plug in pluginlist:
            name = plug.__class__.__name__
            if name in skipplugindict:
                listcopy.remove(plug)
                suspect.set_tag(f'{name}.skipreason', skipplugindict[name])
        return listcopy
...
class AppenderSkipper(PrependerPlugin):
    ...
    def __init__(self, config, section=None):
        ...
        self.requiredvars = {
            'filterfile': {
                'default': '${confdir}/skipappenders.regex',
                'description': 'path to file containing appender plugin skip regex rules'
            }
        }
        self.logger = self._logger()
