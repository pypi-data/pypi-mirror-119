...
from fuglu.shared import ScannerPlugin, AppenderPlugin, SuspectFilter, DUNNO, FileList, get_outgoing_helo
...
class FuzorMixin(object):
    def __init__(self):
        self.requiredvars = {
            ...
            'redis_password': {
                'default': '',
                'description': 'password to connect to redis database. leave empty for no password. deprecated, set password in redis_conn url',
            },
            'redis_ttl': {
                'default': '604800',
    ...
    def _init_backend_redis(self):
        ...
        redis_pool = RedisPooledConn(redis_url, password=password)
        ...
        if digest is not None:
            ...
            if self.ignorelist is not None:
                ...
                if digest in ignorelist:
                    self.logger.info('%s ignoring fuzor hash %s' % (suspect.id, digest))
                    return DUNNO
            ...
            if count > 0:
                ...
                suspect.write_sa_temp_header("%s-ID" % headername, digest)
                suspect.write_sa_temp_header("%s-Lvl" % headername, count)
                self.logger.info("%s digest %s seen %s times" % (suspect.id, digest, count))
            else:
                self.logger.debug("%s digest %s not seen before" % (suspect.id, digest))
        else:
...
class RedisBackend(FuzorBackend):
    def __init__(self, redispool=None):
        self.redis_pool = redispool or RedisPooledConn()
        self.ttl = 7 * 24 * 3600
    ...
    def get(self, digest):
        try:
            redisconn = self.redis_pool.get_conn()
            return int(redisconn.get(digest))
