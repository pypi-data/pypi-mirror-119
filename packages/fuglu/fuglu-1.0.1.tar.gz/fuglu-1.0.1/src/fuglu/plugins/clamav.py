...
"""
    ...
    def _check_skiplist(self, suspect, viruses):
        ...
        """
        self._init_skiplist()
        if self.skiplist and suspect.is_welcomelisted():
            skiplist = self.skiplist.get_list()
            virusnames = viruses.keys()
            if len(virusnames) == 1 and virusnames[0] in skiplist:
                self.logger.info('%s got hit on %s but is skiplisted' % (suspect.id, ','.join(virusnames)))
                return True
        return False
        ...
        while attempts:
            ...
            try:
                ...
                msg = f"{suspect.id} Problem encountered while contacting clamd " \
                      f"(try {max_attempts - attempts}/{max_attempts}): {str(e)}"
                if attempts:
    ...
    def scan_stream(self, content, suspectid ="(NA)"):
        ...
        if result.startswith('INSTREAM size limit exceeded'):
            raise Exception("%s: Clamd size limit exeeded. Make sure fuglu's clamd maxsize config is not larger than clamd's StreamMaxLength" % suspectid)
