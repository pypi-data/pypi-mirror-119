...
SSSP_acceptsyntax = re.compile(b"^ACC\s+(.*?)\s*$")
SSSP_optionsyntax = re.compile(b"^(\w+):\s*(.*?)\s*$")
SSSP_virussyntax = re.compile(b"^VIRUS\s+(\S+)\s+(.*)")
SSSP_typesyntax = re.compile(b"^TYPE\s+(\w+)")
SSSP_donesyntax = re.compile(b"^DONE\s+(\w+)\s+(\w+)\s+(.*?)\s*$")
SSSP_eventsyntax = re.compile(b"^([A-Z]+)\s+(\w+)")
SSSP_tmpdirsyntax = re.compile(u"(/tmp/savid_tmp[^/]+/)(.+)")
