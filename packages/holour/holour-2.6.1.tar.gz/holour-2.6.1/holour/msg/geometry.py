...
class Pose:
    ...
    @staticmethod
    def from_list(pose: List) -> 'Pose':
        return Pose(Vector3(pose[0], pose[1], pose[2]), Vector3(pose[3], pose[4], pose[5]))

    @staticmethod
    def pose_trans(p1: 'Pose', p2: 'Pose') -> 'Pose':
...
class AxisAngle:
    ...
    def get_rotation_vector(self) -> Vector3:
        return Vector3(self._x * self._angle, self._y * self._angle, self._z * self._angle)
