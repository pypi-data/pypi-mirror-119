...
class Crostab:
    ...
    # __slots__ = (SIDE, HEAD, ROWS, TITLE, '__dict__')
    ...
    def __getitem__(self, pos):
        if isinstance(pos, tuple) and len(pos) == 2:
            x, y = pos
            return self.rows[self.roin(x)][self.coin(y)]
        else: raise IndexError('Index must be a tuple of 2')

    def __setitem__(self, pos, value):
        if isinstance(pos, tuple) and len(pos) == 2:
            x, y = pos
            self.rows[self.roin(x)][self.coin(y)] = value
        else: raise IndexError('Index must be a tuple of 2')
