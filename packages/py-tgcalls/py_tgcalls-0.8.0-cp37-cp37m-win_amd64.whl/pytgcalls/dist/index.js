...
binding.on('request', async function (data) {
    binding_1.Binding.log('REQUEST: ' + JSON.stringify(data), binding_1.Binding.INFO);
    let connection = connections.get(data.chat_id);
    switch (data.action) {
        case 'join_call':
            if (!connection) {
                connection = new rtc_connection_1.RTCConnection(data.chat_id, binding, data.buffer_length, data.invite_hash, data.stream_audio, data.stream_video);
        ...
        case 'change_stream':
            if (connection) {
                try {
                    await connection.changeStream(data.stream_audio, data.stream_video);
                    await binding.sendUpdate({
                        action: 'update_request',
