...
class GoogleServiceAccountCredentials(Credentials):
    ...
    def __init__(self, key_file_path: Optional[pathlib.Path] = None):
        ...
        self._credentials = IDTokenCredentials.from_service_account_file(
            filename=key_file_path or settings.MYST_APPLICATION_CREDENTIALS, target_audience=settings.MYST_API_HOST
        )
    ...
    def token(self) -> str:
        """Validated Bearer token using these credentials."""
        expiry: Optional[datetime] = getattr(self._credentials, "expiry", None)
...
def authenticate_with_service_account(key_file_path: Optional[pathlib.Path] = None) -> None:
    ...
    """
    creds = GoogleServiceAccountCredentials(key_file_path=key_file_path)
    myst.get_client().authenticate(credentials=creds)
