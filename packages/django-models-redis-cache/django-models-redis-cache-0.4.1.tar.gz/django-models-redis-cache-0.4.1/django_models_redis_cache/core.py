        ...
        for django_model, cache_conf in django_models_to_cache.items():
            ...
            def get_or_create_redis_model_from_django_model(django_model, redis_root):
                ...
                if django_model.__name__ in registered_models_names:
                    existing_redis_model_index = redis_root.registered_models_names.index(django_model.__name__)
                    existing_redis_model = redis_root.registered_models[existing_redis_model_index]
                    new_redis_model = update_redis_model_from_redis_fields(existing_redis_model, redis_fields, redis_root)
                else:
                    new_redis_model = create_redis_model_from_redis_fields(redis_fields, redis_root)
            ...
            def create_redis_model_from_redis_fields(redis_fields, redis_root):
            ...
            def django_field_to_redis_field(django_field, redis_root):
                ...
                def get_allowed_params(django_field, redis_root):
                    allowed_params = ['null', 'default', 'choices', 'model']
                    ...
                    for django_field_param_name in django_field.__dict__.keys():
                        if django_field_param_name in allowed_params:
                            ...
                            print(django_field, django_field_param_name, value)
                            if django_field_param_name == 'choices':
                                ...
                                if value is not None:
                                    ...
                                    real_value = None
                                value = real_value
                            elif django_field_param_name == 'model':
                ...
                if redis_field:
                    ...
                    redis_field = redis_field_class(**get_allowed_params(django_field, redis_root))
                return redis_field            
            
            def update_redis_model_from_redis_fields(existing_redis_model, redis_fields, redis_root):
                new_redis_model = create_redis_model_from_redis_fields(redis_fields, redis_root)
            ...
            cache_func(django_model, self, get_or_create_redis_model_from_django_model)
...
class RedisModel:
    ...
    def _serialize_data(self, full=True):
        ...
        for field_name, field in fields.items():
            if not field_name.startswith('__'):
                try:
                    print(field_name, field, field.__class__)
                    cleaned_value = field.clean()
                    if full:
