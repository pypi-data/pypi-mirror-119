...
#         self.model = None
#         if not issubclass(model.__class__, django_models.ModelBase):
#             raise Exception(f'{model.__class__.__name__} class is not django.db.models Model')
#         self.model = model
#
#     def _get_id_from_instance(self):
#         if issubclass(self.value.__class__, django_models.ModelBase):
...
class RedisRoot:
        ...
        for django_model, cache_conf in django_models_to_cache.items():
            ...
            def django_field_to_redis_field(django_model, django_field):
                ...
                field_mapping = {
                    ...
                    django_models.DecimalFeild: RedisDecimal,
                    django_models.PostitiveSmallIntegerField: RedisNumber,
        ...
        if user_cache_conf is not None:
            if type(user_cache_conf) == dict:
                ...
                if 'django_model' in user_cache_conf.keys():
                    if issubclass(user_cache_conf['django_model'], django_models.ModelBase):
    ...
    def register_django_models(self, models_dict):
        for model, user_cache_conf in models_dict.items():
            if issubclass(model, django_models.ModelBase):
                ...
                raise Exception(f'{model.__name__} class is not django_models.ModelBase')
