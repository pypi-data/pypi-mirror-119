...
class RedisRoot:
    ...
    def check_cache(self):
        ...
        for django_model, cache_conf in django_models_to_cache.items():
            ...
            def django_field_to_redis_field(django_model, django_field, redis_root):

                def get_allowed_params(django_field, django_model, redis_root):
                    ...
                    for django_field_param_name in django_field.__dict__.keys():
                        if django_field_param_name in allowed_params:
                            ...
                            if django_field_param_name == 'choices':
                                ...
                                create_redis_model_from_django_model(django_model, redis_root)
                ...
                if redis_field:
                    ...
                    redis_field = redis_field_class(**get_allowed_params(django_field, django_model, redis_root))
                return redis_field

            def create_redis_model_from_django_model(django_model, redis_root):
                redis_fields = {}
                for django_field in django_model._meta.get_fields():
                    redis_field = django_field_to_redis_field(django_model, django_field, redis_root)
                    if redis_field:
