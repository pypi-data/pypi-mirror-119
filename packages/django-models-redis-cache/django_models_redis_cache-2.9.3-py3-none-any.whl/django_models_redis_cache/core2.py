...
class RedisRoot:
    ...
    def _get_field_instance_by_name(self, field_name, redis_model):
        field_instance = None
        try:
            if field_name == 'id':
                field_instance = getattr(redis_model, field_name)
            else:
                field_instance = getattr(redis_model, field_name)
        except BaseException as ex:
            if self.ignore_deserialization_errors:
                print(
                    f'{datetime.datetime.now()} - {redis_model.__name__} has no field {field_name}, ignoring deserialization')
                field_instance = field_name
            else:
                raise Exception(f'{redis_model.__name__} has no field {field_name}')
    ...
    def _get_all_redis_instances(self, model, filters=None):
    ...
    def _get_all_redis_model_instances(self, redis_model, filters=None):
        if filters is None:
            filters = {}
        instances_with_allowed = self._get_all_redis_instances(redis_model, filters)
    ...
    def get(self, django_model, return_dict=False, **filters):
        redis_model = self.get_or_create_redis_model_from_django_model(django_model)
        result = self.get_by_redis_model(redis_model, return_dict=False, **filters)
        return result
    ...
    def _update_by_key(self, redis_model, key, fields_to_update, renew_ttl, new_ttl):
        updated_data = None
    ...
    def update(self, django_model, instances=None, return_dict=False, renew_ttl=False, new_ttl=None, **fields_to_update):
        redis_model = self.get_or_create_redis_model_from_django_model(django_model)
        model_name = redis_model.__name__
        updated_datas = []
        if instances is None:
            for key in self.redis_instance.scan_iter(f'{self.prefix}:{model_name}:*'):
                updated_data = self._update_by_key(redis_model, key, fields_to_update, renew_ttl, new_ttl)
        ...
        for updated_data in updated_datas:
            ...
            if self.economy:
                ...
                updated_instance_qs = self.get(django_model, id=updated_id)
...
class RedisModel:
    ...
    def save(self):
        ...
        if self.__model_data__['redis_root'].economy:
            ...
            deserialized_instance = redis_root.get_by_redis_model(self.__class__, id=self.id.value)[0]
