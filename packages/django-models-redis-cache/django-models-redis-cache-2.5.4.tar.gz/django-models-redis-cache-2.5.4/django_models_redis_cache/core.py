...
class RedisRoot:
    ...
    def get(self, django_model, return_dict=False, **filters):
        ...
        return self.get_by_redis_model(redis_model, return_dict, **filters)
...
class RedisModel:
    ...
    def _serialize_data(self):
        ...
        for field_name, field in fields.items():
            if not field_name.startswith('__'):
                try:
                    if isinstance(field, RedisForeignKey):
                        id = get_ids_from_untyped_data(field.value)[0]
                        deserialized_value = redis_root.deserialize_value(id, name, field_name)
                    elif isinstance(field, RedisManyToMany)
                        ids = get_ids_from_untyped_data(field.value)
                        deserialized_value = redis_root.deserialize_value(ids, name, field_name)
                    else:
                        deserialized_value = redis_root.deserialize_value(field.value, name, field_name)
