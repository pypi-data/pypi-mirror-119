...
class RedisForeignKey(RedisNumber):
    ...
    def clean(self):
        self.value = self.check_value()
        if self.value not in [None, 'null']:
            print(get_ids_from_untyped_data(self.value)[0])
    ...
    def deserialize_value(self, value, redis_root):
        ...
        if value != 'null':
            ...
            for instance_id in isntances_ids:
                instance = redis_root._get_instances_by_key(f'{redis_root.prefix}:{model_name}:{instance_id}')
                instances_list.append(instances_list)
