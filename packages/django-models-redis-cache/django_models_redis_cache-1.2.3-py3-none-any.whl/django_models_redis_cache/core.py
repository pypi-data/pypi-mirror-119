...
class RedisRoot:
    ...
    def fast_get_keys_values(self, string):
        keys = self.redis_instance.scan_iter(string)
    ...
    def _get_all_stored_model_instances(self, model, filters=None):
        ...
        print(f'{self.prefix}:{model_name}:*')
        instances_with_allowed = {}
        fast_get_results = self.fast_get_keys_values(f'{self.prefix}:{model_name}:*')
        print(fast_get_results)
        for key, raw_value in fast_get_results:
            prefix, model_name, instance_id, field_name = key.split(':')
...
class RedisModel:
    ...
    def _set_fields(self):
        fields_to_write = self._serialize_data()

        async def write_fields(redis_root, fields_to_write):

            async def write_field(redis_root, key, field_data):
                redis_instance = redis_root.redis_instance
                redis_instance.set(key, field_data['value'], ex=field_data['ttl'])

            async_write_fields = [
                write_field(redis_root, key, field_data)
                for key, field_data in fields_to_write.items()
            ]

            await asyncio.gather(*async_write_fields)

        loop = asyncio.get_event_loop()
        loop.run_until_complete(write_fields(self.__model_data__['redis_root'], fields_to_write))
    ...
    def save(self):
        ...
        if self.__model_data__['redis_root'].economy:
            ...
            print(self.__class__, self.id.value)
            deserialized_instance = redis_root.get(self.__class__, id=self.id.value)
            print(deserialized_instance)
            deserialized_instance = deserialized_instance[0]
