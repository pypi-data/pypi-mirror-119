    ...
    async def run_cache_to_django(
        ...
        for redis_instance_id, redis_instance_data in redis_dicts.items():
            ...
            if len(async_tasks) == async_chunk_size or completed == to_complete:
                ...
                async_tasks = []
                print(
                    f'{datetime.datetime.now()} - Written {completed} {django_model.__name__} instances from cache to django')
        ...
        if cache_conf['delete']:
            ...
            all_django_instances = django_model.objects.all()
            async_tasks = []
            completed = 0
            ...
            for django_instance in all_django_instances:
                ...
                if len(async_tasks) == async_chunk_size or completed == to_complete:
                    await asyncio.gather(*async_tasks)
                    async_tasks = []
                    print(f'{datetime.datetime.now()} - Deleted {django_model.__name__} instances from django')
...
def sync_to_async_update_or_create_django_instance_from_redis_instance(
    ...
    if need_to_create:
        pass
        print(need_to_create, django_params, django_many_to_many_params)
    ...
    #     )
        ...
        for redis_field_name, redis_field_value in django_params.items():
            ...
            if django_instance_fields_value.__class__ == datetime.datetime:
                redis_field_value = redis_field_value
                print(django_instance_fields_value.isoformat(), redis_field_value.replace(microsecond=None).replace(tzinfo=None).isoformat())
                if django_instance_fields_value.isoformat() != redis_field_value.replace(microsecond=None).replace(tzinfo=None).isoformat():
        ...
        print(need_to_create, changed_fields_to_update, changed_many_to_many_fields_to_update)
...
@sync_to_async
def django_sync_to_async_func_and_update(
        django_func,
        params_to_use=None,
    ...
    if type(params_to_update) == dict:
        result = objects_qs.update(**params_to_update)
    else:
        result = objects_qs
...
@sync_to_async
def django_sync_to_async_create(
        django_model,
        params
):
    new_django_instance = django_model.objects.create(**params)
    new_django_instance_id = new_django_instance.id
    return new_django_instance_id
    ...
    async def run_django_to_cache(
        ...
        for django_instance in django_instances:
            ...
            if len(async_tasks) == async_chunk_size or completed == to_complete:
                ...
                async_tasks = []
                print(
                    f'{datetime.datetime.now()} - Written {completed} {django_model.__name__} instances from django to cache')
        ...
        print(f'{datetime.datetime.now()} - Deleted {django_model.__name__} instances from cache')
