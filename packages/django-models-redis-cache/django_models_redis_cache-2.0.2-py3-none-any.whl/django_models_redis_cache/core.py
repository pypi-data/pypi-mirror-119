...
class RedisRoot:
    ...
    def get_cache_conf(self, django_model):
        if django_model in self.registered_django_models.keys():
            cache_conf = self.registered_django_models[django_model]
            return cache_conf
        else:
    ...
    def get_or_create_redis_model_from_django_model(self, django_model):
        ...
        def django_fields_to_redis_fields(django_model, redis_root, save_related_models, exclude_fields):
            ...
            for django_field in django_model._meta.get_fields():
                ...
                allowed = True
                if exclude_fields:
                    allowed = (django_field.name in exclude_fields)
                if allowed:
        ...
        cache_conf = self.check_cache_conf(django_model)
    ...
    def _get_all_redis_model_instances(self, model, filters=None):
        if filters is None:
            filters = {}
    ...
    def _django_model_to_redis_model(self, django_model):
        if django_model in self.registered_django_models.keys():
            ...
            raise Exception(f'Django model {django_model} is not registered in redis cache')
    ...
    def update(self, django_model, instances=None, return_dict=False, renew_ttl=False, new_ttl=None, **fields_to_update):
        ...
        for updated_data in updated_datas:
            updated_model = updated_data['django_model']
    ...
    def create(self, django_model, **params):
        model = self._django_model_to_redis_model(django_model)
        redis_instance_id = model(redis_root=self, **params).save()
        if self.economy:
            redis_instances = {'id': redis_instance_id}
        else:
            redis_instance = self.get(django_model, id=redis_instance_id)
        return redis_instance
...
class RedisModel:
    ...
    def _fill_fields_values(self, field_values_dict):
        print('_fill_fields_values', field_values_dict)
    ...
    def _set_fields(self):
        ...
        for instance_key, fields_dict in data_to_write.items():
            ...
            if instance_id not in saved_instance.keys():
                saved_instance[instance_id] = fields_dict
