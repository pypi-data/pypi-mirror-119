...
class RedisRoot:
    ...
    def update_or_create_redis_instance_from_django_instance(
        ...
        redis_dict = self.django_instance_to_redis_params(
            django_instance,
            django_fields,
            cache_conf,
        )
            ...
            if not django_field.__class__.__name__.endswith('Rel'):
                ...
                if allowed:
                    if django_field.name not in ['id', 'pk']:
                        redis_param = self.django_instance_field_to_redis_value(
                            django_instance,
                            django_field,
                            cache_conf,
                        )
    ...
    def django_instance_field_to_redis_value(
        ...
        save_related_models = cache_conf['save_related_models']
        django_field_value = getattr(django_instance, django_field.name, None)
        ...
        if django_field.__class__ == django_models.ForeignKey:
            if django_field_value is None:
                ...
                if save_related_models:
                    if django_foreign_key_instance:
                        ...
                        redis_foreign_key_instance = self.get_or_create_redis_instance_from_django_instance(
            ...
            if save_related_models:
                if len(django_many_to_many_instances):
                    ...
                    redis_value = [
                        self.get_or_create_redis_instance_from_django_instance(
        ...
        for model, user_cache_conf in models_dict.items():
            if issubclass(model, django_models.Model):
                if model not in self.registered_django_models.keys():
                    ...
                    def save_redis_instance_from_django_instance(sender, instance, **kwargs):
                        django_fields = sender._meta.get_fields()
                        self.update_or_create_redis_instance_from_django_instance(
                            instance,
                            django_fields,
                            cache_conf
                        )
                        print('Called save_redis_instance_from_django_instance')

                    @receiver(pre_delete, sender=model)
                    def delete_redis_instance_from_django_instance(sender, instance, **kwargs):
                        redis_instances_qs = self.get(sender, django_id=instance.id)
                        if redis_instances_qs:
                            self.delete(sender, redis_instances_qs)
                        print('Called delete_redis_instance_from_django_instance')
...
class RedisQuerySet:
    ...
    def get(self, **filters):
        return self.model.redis_root.get(self.model, **filters)

    def update(self, instances, **params):
        return self.model.redis_root.update(self.model, instances, **params)

    def create(self, **params):
        return self.model.save(**params)

    def delete(self, instances):
        return self.model.redis_root.delete(self.model, instances)


class RedisObjectsManager(django_models.Manager):
    def get_queryset(self, **kwargs):
        return RedisQuerySet(self)


class DjangoRedisModel(django_models.Model):
    objects = RedisObjectsManager()
    django_objects = django_models.Manager()
    redis_root = settings.REDIS_ROOT

    def save(self, *args, **kwargs):
        existing_fields = {
            field.name: getattr(self, field.name)
            for field in self.__class__._meta.get_fields()  # Parents included
            if field.name not in ['objects', 'django_objects', 'redis_root']
        }
        return self.redis_root.create(self.__class__, **existing_fields)

    def django_save(self, *args, **kwargs):
