...
class RedisModel:
    ...
    def _serialize_data(self):
        ...
        for field_name, field in fields.items():
            if not field_name.startswith('__'):
                try:
                    deserialized_fields[field_name] = field.value
                    print(field_name, field.value, type(field.value))
                    cleaned_value = field.clean()
                    cleaned_fields[field_name] = cleaned_value
                except BaseException as ex:
                    raise Exception(f'{ex} ({name} -> {field_name})')
        return instance_key, deserialized_fields
    ...
    def _set_fields(self):
        instance_key, fields_dict = self._serialize_data()
