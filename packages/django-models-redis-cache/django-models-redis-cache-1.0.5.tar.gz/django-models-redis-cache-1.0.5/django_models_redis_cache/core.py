...
class RedisRoot:
    ...
    def check_cache(self):
        ...
        for django_model, cache_conf in django_models_to_cache.items():
            ...
            def get_or_create_redis_model_from_django_model(django_model, redis_root, save_related_models):
                registered_models_names = [model.__name__ for model in redis_root.registered_models]
                redis_fields = django_fields_to_redis_fields(django_model, redis_root, save_related_models)
            ...
            def django_fields_to_redis_fields(django_model, redis_root, save_related_models):
                redis_fields = {}
            ...
            def django_field_to_redis_field(django_field, redis_root, save_related_models):
                ...
                if redis_field:
                    ...
                    redis_field_params = django_field_params_to_redis_field_params(django_field, redis_root, save_related_models)
                    redis_field = redis_field_class(**redis_field_params)
                return redis_field

            def django_field_params_to_redis_field_params(django_field, redis_root, save_related_models):
                    ...
                    if django_field_param_name in allowed_params:
                        ...
                        if django_field_param_name == 'choices':
                            ...
                            if value is not None:
                                value = value.model
                                if value is not None:
                                    value = get_or_create_redis_model_from_django_model(value, redis_root, save_related_models)
                                    if value is not None:
                                        allowed_redis_params[redis_field_param_name] = value
            ...
            redis_model = get_or_create_redis_model_from_django_model(django_model, self, save_related_models)
