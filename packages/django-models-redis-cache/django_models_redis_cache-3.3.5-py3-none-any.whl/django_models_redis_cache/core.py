...
class RedisRoot:
    ...
    def get_cache_conf(self, django_model):
        if django_model in self.registered_django_models.keys():
            ...
            cached_cache_conf = self.redis_instance.get(f'{self.prefix}:{django_model.__name__}:cache_conf')
            if cached_cache_conf:
                cache_conf = cached_cache_conf
            else:
                cache_conf = {
                    'enabled': False,
                    'ttl': 60 * 5,
        ...
        for model, user_cache_conf in models_dict.items():
            if issubclass(model, django_models.Model):
                if model not in self.registered_django_models.keys():
                    ...
                    self.redis_instance.set(f'{self.prefix}:{model.__name__}:cache_conf', json.dumps(cache_conf))
...
class RedisQuerySet(django_models.query.QuerySet):
    ...
    def create(self, **kwargs):
        ...
        existing_fields = {
            field.name: getattr(create_result, field.name)
            for field in create_result.__class__._meta.get_fields()
        }
        redis_root = get_redis_root()
        redis_root.create(create_result.__class__, **existing_fields)
...
class RedisObjectsManager(django_models.Manager):
    def get_queryset(self, *args, **kwargs):
        return RedisQuerySet(self, *args, **kwargs)
...
class DjangoRedisModel(django_models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        existing_fields = {
            field.name: getattr(save_result, field.name)
            for field in save_result.__class__._meta.get_fields()
        }
        redis_root = get_redis_root()
