...
async def django_instance_to_redis_params(
    ...
    django_fields = django_instance.__class__._meta.get_fieds()
    if redis_root.performance['enabled']:
        ended_meta_get_fields = datetime.datetime.now()
        redis_root.performance['django_instance_to_redis_params meta_get_fields']['time'] += (started_meta_get_fields - ended_meta_get_fields).total_seconds()
