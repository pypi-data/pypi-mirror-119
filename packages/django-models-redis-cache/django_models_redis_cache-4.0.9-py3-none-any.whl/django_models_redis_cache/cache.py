...
def cache_to_django(
    ...
    async def run_cache_to_django(
        ...
        for redis_instance_id, redis_instance_data in redis_dicts.items():
            ...
            print('to completed:', to_complete)
            completed += 1
...
def sync_to_async_update_or_create_django_instance_from_redis_instance(
    ...
    if need_to_create:
        if cache_conf['write_to_django']:
            ...
            redis_instances = redis_root.get(django_model, id=redis_instance_id)
            redis_root.delete(django_model, redis_instances)
            redis_instances = redis_root.get(django_model, id=redis_instance_id)
            print(len('MUST BE 0: ', len(list(redis_instances))))
            ...
            redis_instances = redis_root.get(django_model, django_id=int(django_instance.id))
            print(len('MUST BE 1: ', len(list(redis_instances))))
    ...
    print(f'now django {django_model.__name__}: {len(django_model.objects.all())}\n'
          f'now redis {django_model.__name__}: {len(redis_root.get(django_model))}'
          '\n\n\n')
    return django_instance
