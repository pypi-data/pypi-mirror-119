...
def cache_to_django(
    ...
    if redis_root.performance['enabled']:
        if 'cache_to_django' not in redis_root.performance.keys():
            redis_root.performance['cache_to_django'] = {
                'requests': 0,
                'time': 0
            }
        redis_root.performance['cache_to_django']['requests'] += 1
    ...
    if redis_root.performance['enabled']:
        ...
        redis_root.performance['cache_to_django']['time'] += (ended_in - started_in).total_seconds()
...
def django_to_cache(
    ...
    if redis_root.performance['enabled']:
        if 'django_to_cache' not in redis_root.performance.keys():
            redis_root.performance['django_to_cache'] = {
                'requests': 0,
                'time': 0
            }
        redis_root.performance['django_to_cache']['requests'] += 1
        started_in = datetime.datetime.now()
    ...
    if redis_root.performance['enabled']:
        ended_in = datetime.datetime.now()
        redis_root.performance['django_to_cache']['time'] += (ended_in - started_in).total_seconds()
...
async def update_or_create_redis_instance_from_django_instance(
    ...
    if redis_root.performance['enabled']:
        if 'update_or_create_redis_instance_from_django_instance' not in redis_root.performance.keys():
            redis_root.performance['update_or_create_redis_instance_from_django_instance'] = {
                'requests': 0,
                'time': 0
            }
        redis_root.performance['update_or_create_redis_instance_from_django_instance']['requests'] += 1
        started_in = datetime.datetime.now()

    django_instance_model = django_instance.__class__
    old_redis_instance = get_redis_instance_by_django_id_from_all_redis_instances(
        django_instance.id,
        all_redis_instances
    )
    ...
    if redis_root.performance['enabled']:
        ended_in = datetime.datetime.now()
        redis_root.performance['update_or_create_redis_instance_from_django_instance']['time'] += (ended_in - started_in).total_seconds()
...
async def django_instance_field_to_redis_value(
    ...
    if redis_root.performance['enabled']:
        if 'django_instance_field_to_redis_value' not in redis_root.performance.keys():
            redis_root.performance['django_instance_field_to_redis_value'] = {
                'requests': 0,
                'time': 0
            }
        redis_root.performance['django_instance_field_to_redis_value']['requests'] += 1
        started_in = datetime.datetime.now()
        ...
        if django_field_value is None:
            ...
            if save_related_models:
                if django_foreign_key_instance:
                    ...
                    all_redis_instances = redis_root.get(django_instance_model)
    ...
    if redis_root.performance['enabled']:
        ended_in = datetime.datetime.now()
        redis_root.performance['django_instance_field_to_redis_value']['time'] += (ended_in - started_in).total_seconds()
...
async def django_instance_to_redis_params(
    ...
    if redis_root.performance['enabled']:
        if 'django_instance_to_redis_params' not in redis_root.performance.keys():
            redis_root.performance['django_instance_to_redis_params'] = {
                'requests': 0,
                'time': 0
            }
        redis_root.performance['django_instance_to_redis_params']['requests'] += 1
        started_in = datetime.datetime.now()

    redis_params = {}
    exclude_fields = cache_conf['exclude_fields']
    if redis_root.performance['enabled']:
        if 'django_instance_to_redis_params meta_get_fields' not in redis_root.performance.keys():
            redis_root.performance['django_instance_to_redis_params meta_get_fields'] = {
                'requests': 0,
                'time': 0
            }
        redis_root.performance['django_instance_to_redis_params meta_get_fields']['requests'] += 1

        started_meta_get_fields = datetime.datetime.now()
    django_fields = django_instance.__class__._meta.get_fields()
    if redis_root.performance['enabled']:
        ended_meta_get_fields = datetime.datetime.now()
        redis_root.performance['django_instance_to_redis_params meta_get_fields']['time'] += (ended_meta_get_fields - started_meta_get_fields).total_seconds()
    ...
    if redis_root.performance['enabled']:
        ended_in = datetime.datetime.now()
        redis_root.performance['django_instance_to_redis_params']['time'] += (ended_in - started_in).total_seconds()

    return redis_params


def get_redis_instance_by_django_id_from_all_redis_instances(
        django_id,
        all_redis_instances
):
    all_django_ids = []
    for redis_instance in all_redis_instances:
        ...
        if 'django_id' in redis_instance.keys():
            django_id = redis_instance['django_id']
        all_django_ids.append(django_id)
    try:
        index_of_redis_instance = all_django_ids.index(django_id)
        redis_instance = all_redis_instances[index_of_redis_instance]
    except:
        redis_instance = None
...
async def get_or_create_redis_instance_from_django_instance(
    ...
    if redis_root.performance['enabled']:
        if 'get_or_create_redis_instance_from_django_instance' not in redis_root.performance.keys():
            redis_root.performance['get_or_create_redis_instance_from_django_instance'] = {
                'requests': 0,
                'time': 0
            }
        redis_root.performance['get_or_create_redis_instance_from_django_instance']['requests'] += 1
        started_in = datetime.datetime.now()

    redis_instance = get_redis_instance_by_django_id_from_all_redis_instances(
        django_instance.id,
        all_redis_instances
    )
    ...
    if redis_root.performance['enabled']:
        ended_in = datetime.datetime.now()
        redis_root.performance['get_or_create_redis_instance_from_django_instance']['time'] += (ended_in - started_in).total_seconds()
