...
class RedisQuerySet(django_models.query.QuerySet):
    ...
    def update(self, **kwargs):
        ...
        print('\nRedisQuerySet update:')
        print('update', concrete_model, self)
        django_ids_to_update = [django_instance.id for django_instance in self]
        data_to_update = redis_root.get(self.model, django_id__in=django_ids_to_update)
        print(f'now data to update {self.model}:', len(data_to_update))
        redis_root.update(self.model, data_to_update, **kwargs)
        print(f'now redis {self.model}:', redis_root.get(self.model))
        return self
    ...
    def delete(self):
        ...
        print('delete', concrete_model, concrete_model.id)
        redis_root = get_redis_root()
    ...
    if django_field.__class__ == django_models.ForeignKey:
        if django_field_value is None:
            ...
            if save_related_models:
                if django_foreign_key_instance:
                    print('creating new instance foreign', django_field, django_foreign_key_instance)
        ...
        if save_related_models:
            if len(django_many_to_many_instances):
                ...
                print('creating new instance m2m', django_field, django_instance_model)
                redis_value = [
