...
class MetricSaverBase(object):
    ...
    def add_record(self, epoch_train_acc, epoch_train_loss, epoch_eva_acc=None, epoch_eva_loss=None, monitor='eva_acc', mode='max'):
...
class TensorBoardSaver(MetricSaverBase):
    ...
    def start_server(self):
        self.subp = subprocess.Popen([f'tensorboard --logdir {self.dir_name} --bind_all --load_fast false'], shell=True)
        time.sleep(5)
    ...
    def add_record(self, epoch_train_acc, epoch_train_loss, epoch_eva_acc=None, epoch_eva_loss=None):
        flag = super().add_record(epoch_train_acc, epoch_train_loss, epoch_eva_acc, epoch_eva_loss)
        tag = self.tag
    ...
    for _ in range(fold):
        for i in range(100):
            ...
            if b(_a, _b, _c, _d):
                print(_, i, _c)
            # time.sleep(0.1)
            if b.EarlyStopping(patience=20, min_delta=0.001):
                print(_, i, 'continue !!!')
                break
            time.sleep(1)
        b.next_fold()
