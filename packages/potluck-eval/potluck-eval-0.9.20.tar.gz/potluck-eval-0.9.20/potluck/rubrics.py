...
class Goal:
    ...
    def unique_id(taskid, identifier):
        """
        A static method: given a task of interest and an identifier,
        keeps track of identifiers provided and returns them as-is,
        except when a duplicate is provided, in which case it appends a
        -number suffix to the duplicate to make it unique and returns
        that. The -number suffixes start at -2 for the second copy; -1 is
        never used because the first copy doesn't get a suffix added.
        """
        task_ids = Goal.USED_IDS.setdefault(taskid, {})
        seen_before = task_ids.get(identifier, 0)
        ...
        if seen_before == 0: # not seen before
            ...
            task_ids[identifier] += 1
            result = identifier + '-' + str(seen_before + 1)
            return result
    ...
    def __init__(
        ...
        self.identifier = Goal.unique_id(taskid, identifier)
    ...
    def table(self, blank=False):
        ...
        - 'id': This goal's identifier, prefixed with "goal:", then its
            category tag, and then a period. May be absent on some rows
            representing groups of goals rather than individual goals.
...
class NoteGoal(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        on the rubric) may also be given. By default, the feedback_only
        tag will be applied.
