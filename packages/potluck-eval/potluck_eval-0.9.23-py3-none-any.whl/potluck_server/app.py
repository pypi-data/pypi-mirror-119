...
def route_feedback(
    ...
    task["feedback_html"] = get_feedback_html(
        course,
        semester,
        task_info,
        target_user,
...
def launch_potluck(
    ...
    with open(logfile, 'wb') as log:
        if app.config["USE_XVFB"]:
            ...
            full_args = [ potluck_exe ] + potluck_args
            log.write(
                ("Full args: " + repr(full_args) + '\n').encode("utf-8")
            )
            subprocess.Popen(
...
def get_inflight(course, semester, username, phase, psid, taskid):
    ...
    if new_status not in ("error", "completed"):
        elapsed = (
          ...
          - potluck.time_utils.time_from_timestring(timestamp)
        )
...
def fetch_time_spent(course, semester, username, phase, psid, taskid):
    ...
    - "taskid": The task ID.
    - "timestamp": Timestamp (see `potluck.time_utils.timestring`)
...
class AsFeedbackHTML(sync.View):
    """
...
def get_feedback_html(
    ...
    try:
        result = sync.read_file(
            report_file[:-5] + ".html", # swap .json for .html
            AsFeedbackHTML,
            missing="missing", # return a string if there is no such file
            cache=False
            # These include student code, instructions, etc., so it would
            # be expensive to cache them.
        )
    except Exception:
        flask.flash("Failed to read feedback file.")
        result = "missing"
