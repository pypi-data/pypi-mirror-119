...
class Netro:
    ...
    def __str__(self):
        return 'Name: ' + self.name + '\n' + 'Status: ' + self.status + '\n' + 'Zones: ' + str(self.zones) + '\n' + 'Device events: ' + str(self.device_events) + '\n' + 'Watering events: ' + str(self.water_events) + '\n'
    ...
    def update_info(self):
        ...
        self.zones = [{}] * ((response.json()['data'])['device']['zone_num'] + 1)
    ...
    def update_schedules(self):
        ...
        for schedule in (response.json()['data'])['schedules']:
            if schedule['status'] == 'WATERING':
                (self.zones[schedule['zone']])['status'] = 'WATERING'
            (self.zones[schedule['zone']])['schedules'].append({
    ...
    def update_moistures(self):
        ...
        for moisture in (response.json()['data'])['moistures']:
            if (self.zones[moisture['zone']])['moisture_date'] < moisture['date']:
                (self.zones[moisture['zone']])['moisture'] = moisture['moisture']
                (self.zones[moisture['zone']])['moisture_date'] = moisture['date']
    ...
    def water(self, delay: int = 0, duration: int = 5, zone: int = 0) -> bool:
        if zone == 0:
            data = '{"key":"' + self._key + '",' + '"duration":' + duration + ',"delay":' + delay + '}'
        else:
            data = '{"key":"' + self._key + '",' + '"duration":' + duration + ',"delay":' + delay + ',"zones":[' + zone + ']}'
