...
def test_weather_xpaths():
    """
    A simple test to see if the methods in weather_xpath_selector work ok.
    """
    expected_value = '/html/body/app-root/app-history/one-column-layout/wu-header/sidenav/mat-sidenav-container/mat-sidenav-content/div/section/div[2]/div[1]/div[3]/div[1]/div/lib-city-history-summary/div/div[2]/table/tbody[6]/tr[1]/td[1]'
    output = weather_xpaths.get_day_length()
...
class Weather_Scrape(Scraping_Space):
    ...
    """
    def __init__(self, Giant_dataset, location_key_match):
        """
    ...
    def complete_weather_scrape(self, month_location_match):
        """
    ...
    def month_year_weather_scrape(self, months):
        ...
        for locations in months['Locations']:  #We are finding the key for each location here.
            self.location_and_month_scrape(locations, month_info.get_amount_of_days(), current_year, month_number, current_month_dict)
    ...
    def location_and_month_scrape(self, locations, amount_of_days, current_year, month_number, current_month_dict):
        """
        ...
        for i in range(1, amount_of_days + 1):  #This range means for every day of the month
            current_month_dict.append(self.day_scrape(i, locations, current_key, current_year, month_number, current_month_dict))
            
        return(current_month_dict)
...
test_location_key_match = [{'Location Name': 'New York','key': 'KLGA'}, {'Location Name': 'London','key': 'EGLC'}, {'Location Name': 'Tokyo' ,'key': 'RJTT'}, {'Location Name': 'Hong Kong','key': 'VHHH'}, {'Location Name': 'Oslo','key': 'ENGM'}, ]
#Test values:
typical_url = ';https://www.wunderground.com/history/daily/KLGA/date/2021-8-9' #This is a good testing example for the type of website we are searching through. 



class Test_Weather_Scrape():
    def __init__(self):
        self.tester = Weather_Scrape(Giant_dataset = [], location_key_match = test_location_key_match)
    
    def test_complete_weather_scrape(self):
        """
        The test that go through the all encompassing functionality. 
        """
        return self.tester.complete_weather_scrape(test_month_location_match)

    
    

test_weather = Test_Weather_Scrape()
print(test_weather.test_complete_weather_scrape())