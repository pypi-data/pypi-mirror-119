...
class pygl:
    ...
    def __init__(self, **kwargs):
        self.Mode = (kwargs["display_mode"]
                         if "display_mode" in kwargs else GLUT_RGBA)
        self.Size= (kwargs["window_size"]
                        if "window_size" in kwargs else (500, 500))
        self.Position = (kwargs["window_position"]
                            if "window_position" in kwargs else (0, 0))
        self.Title = (kwargs["window_title"]
                         if "window_title" in kwargs else "new title")
        self.Color = (kwargs["bgcolor"]
                     if "bgcolor" in kwargs else (0, 0, 0, 1.0))
        self.range = (kwargs["axis_range"] if "axis_range" in kwargs else (
            0, 100, 0, 100))
    ...
    def run(self, cb: Callable):
        ...
        self.windowPosition(*self.Position)
        glutCreateWindow(self.Title)
        glutDisplayFunc(cb)
        self.axis_range(*self.range)
        glutMainLoop()

    def displayMode(self, mode: Constant):
