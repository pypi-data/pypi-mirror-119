...
def __init__(hub):
    ...
    hub.pop.sub.dynamic(
        sub=hub.exec.boto3,
        subname="client",
        resolver=hub.exec.boto3.init.client_resolver,
        context=None,
    )
    hub.pop.sub.dynamic(
        sub=hub.exec.boto3,
        subname="resource",
        resolver=hub.exec.boto3.init.resource_resolver,
        context=None,
    )


def client_resolver(hub, path: str, context):
    """
    ...
    async def _client_caller(ctx, *args, **kwargs) -> Dict[str, Any]:
        result = {"comment": None, "ret": None, "status": True}
        try:
...
def resource_resolver(hub, path: str, context):
    """
    ...
    Examples:
        ...
        .. code-block: python
            ...
            resource = hub.tool.boto3.resource.create(ctx, "Vpc", id="vpc-71d00419")
    ...
    async def _resource_caller(ctx, *args, **kwargs):
        ...
        try:
            if operation == "id":
                ...
                resource_ = hub.tool.boto3.resource.create(
