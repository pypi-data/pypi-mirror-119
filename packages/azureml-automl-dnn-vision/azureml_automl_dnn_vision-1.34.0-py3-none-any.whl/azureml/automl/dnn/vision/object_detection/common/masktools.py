...
from .constants import MaskToolsParameters, MaskToolsLiterals
...
def _extract_settings_and_convert_mask_to_polygon(mask, masktool_settings):
    """A wrapper for convert_mask_to_polygon function with relevant settings extraction

    :param mask: Pixel mask, where each pixel has an object (float) score in [0, 1], in torch.Size([1, height, width])
    :type: mask: <class 'torch.Tensor'>
    :param masktool_settings: dictionary containing settings for masktool
    :type: masktool_settings: dict
    :return: Refined bitmap mask with pixel values 1 or 0
    :rtype: numpy.ndarray of shape (height, width)
    """

    if masktool_settings is not None:
        score_threshold = masktool_settings[MaskToolsLiterals.MASK_PIXEL_SCORE_THRESHOLD]
        max_polygon_points = masktool_settings[MaskToolsLiterals.MAX_NUMBER_OF_POLYGON_POINTS]
    else:
        # need to consider when masktool_settings is None coming from score_validation_data()
        score_threshold = MaskToolsParameters.DEFAULT_MASK_PIXEL_SCORE_THRESHOLD
        max_polygon_points = MaskToolsParameters.DEFAULT_MAX_NUMBER_OF_POLYGON_POINTS

    mask = convert_mask_to_polygon(mask, max_polygon_points=max_polygon_points,
