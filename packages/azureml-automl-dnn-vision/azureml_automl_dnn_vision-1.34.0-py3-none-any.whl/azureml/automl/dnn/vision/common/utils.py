...
from azureml.automl.dnn.vision.object_detection.common.constants import ModelNames as od_ModelNames
...
def _read_image(ignore_data_errors: bool, image_url: str, use_cv2: bool = False) -> Optional[Any]:
    try:
        if use_cv2:
...
def _extract_od_label(dataset: Optional[Any] = None,
    ...
    :type output_file: str
     """
    from azureml.automl.dnn.vision.object_detection.data.datasets import ObjectDetectionDatasetBaseWrapper
    dataset = cast(ObjectDetectionDatasetBaseWrapper, dataset)
...
def _save_image_lf(train_ds: Union[Any, str] = None,
    ...
    train_ds = cast(ObjectDetectionDatasetBaseWrapper, train_ds)
    val_ds = cast(ObjectDetectionDatasetBaseWrapper, val_ds)
