...
def run(params):
    ...
    def setting_handler():
        ...
        response_content = open(os.path.join(mydir, "./html/setting.html"), 'r', encoding="UTF-8").read().replace(
            "{theme}", support_theme).replace("{language}", support_language)
        if config_content.get("open_and_edit", "no") == "no":
    ...
    def handle_403(ex):
        return open(os.path.join(mydir, "./html/error.html"), "r", encoding="utf-8").read().replace("{errorcode}",
                                                                                                    "403").replace(
            "{message}",
            '您没有权限访问此资源。有可能是您没有在begin页面完成身份验证，<a href="/begin">点此前往</a>。也有可能是您用see页面访问了一个目录，或是Git无法使用却访问了git页面。<br>' + str(
                ex)), 404
