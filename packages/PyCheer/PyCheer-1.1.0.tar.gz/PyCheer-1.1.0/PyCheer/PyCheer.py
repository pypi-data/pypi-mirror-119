...
if os.environ.get("HOME"):
    configpath = os.path.join(os.environ.get("HOME"), ".PyCheer/config.json").replace("\\", "/")
else:
    configpath = os.path.join(os.environ.get("APPDATA"), "PyCheer/config.json").replace("\\", "/")
config_content = {}
support_type = {
    "c": "c_cpp",
    "cpp": "c_cpp",
...
def run(params):
    ...
    def see_handler():
        ...
        if get_suffix(file_path) in support_type.keys():
            response_data = open(os.path.join(mydir, "./html/editcode.html"), encoding="UTF-8").read().replace(
                "<init_code>", quote(open(file_path, encoding="UTF-8").read())).replace("<theme>", config_content[
                "theme"]).replace("<mode>", support_type[get_suffix(file_path)])
            ...
            if config_content.get("open_and_edit", "no") == "no":
                ...
                response_data = open(os.path.join(mydir, "./html/editcode.html"), encoding="UTF-8").read().replace(
                    "<init_code>", quote(open(file_path, encoding="UTF-8").read())).replace("<theme>", config_content[
                    "theme"]).replace('editor.getSession().setMode("ace/mode/<mode>");', "")
                response = make_response(response_data)
                response.status = 200
                if request.args.get("token") == token or check_password(request.args.get("token")):
                    response.set_cookie("token", token)
    ...
    def setting_handler():
        if request.args.get("token") != token and request.cookies.get("token") != token and (
                not check_password(request.args.get("token"))) and (not check_password(request.cookies.get("token"))):
            return '', 302, [("Location", '/begin?go_url=' + quote("/setting.html"))]
        ...
        if config_content.get("open_and_edit", "no") == "no":
            response_content = response_content.replace("{open_and_edit}",
                                                        '''<option value="no">仅以只读模式打开</option><option value="yes">尝试以文本文件模式编辑</option>''')
        else:
            response_content = response_content.replace("{open_and_edit}",
                                                        '''<option value="yes">尝试以文本文件模式编辑</option><option value="no">仅以只读模式打开</option>''')
    ...
    def tree_handler():
        ...
        if len(path_split) > 0:
            ...
            for i in range(len(path_split) - 1):
                ...
                breadcrumb_code += '<li><a href="/tree?path=%s">&nbsp;/&nbsp;%s</a></li>' % (
        ...
        return open(os.path.join(mydir, "./html/tree_tpl.html"), "r", encoding="UTF-8").read().replace("{breadcrumb}",
            ...
            "{view_files}", view_files_code).replace("{git_button}", git_button)
    ...
    def git_handler():
        ...
        response = make_response(git_content)
        response.status = 200
        response.headers["Content-Type"] = "text/html;charset=utf-8"
        return response
    ...
    def seeicon_handler():
        ...
        for i in svg_by_type.keys():
            icon_by_type += '<li class="list-group-item">%s%s</li>' % (svg_by_type[i], i)
        for i in svg_by_suffix.keys():
            icon_by_suffix += '<li class="list-group-item">%s%s</li>' % (svg_by_suffix[i], i)
        response = make_response(
            open(os.path.join(mydir, "./html/seeicon.html"), encoding="UTF-8").read().replace("{icon_by_type}",
                                                                                              icon_by_type).replace(
                "{icon_by_suffix}", icon_by_suffix))
        response.status = 200
        response.headers["Content-Type"] = "text/html;charset=utf-8"
        return response
    ...
    def hello_handler():  # 访问这个路径，获得关于PyCheer的信息。
        response = make_response(
            open(os.path.join(mydir, "./html/hello.html"), encoding="UTF-8").read().replace("{VERSION}",
                                                                                            __version__).replace(
                "{WORKDIR}", workdir))
        response.status = 200
        response.headers["Content-Type"] = "text/html;charset=utf-8"
    ...
    def handle_403(ex):
        return open(os.path.join(mydir, "./html/error.html"), "r", encoding="utf-8").read().replace("{errorcode}",
                                                                                                    "403").replace(
            "{message}", '您没有权限访问此资源。有可能是您没有在begin页面完成身份验证，<a href="/begin">点此前往</a>。也有可能是您用see页面访问了一个目录，或是Git无法使用却访问了git页面。<br>'+str(ex)), 404
    ...
    def handle_500(ex):
        return open(os.path.join(mydir, "./html/error.html"), "r", encoding="utf-8").read().replace("{errorcode}",
            ...
            "{message}", "服务端发生了错误。您可以查看服务控制台信息。<br>" + str(
                ex) + '<br>如果您实在排查不到问题，您可以<a href="https://gitee.com/cyrxdzj/PyCheer/issues/new">提交ISSUE</a>。'), 404
...
def main_function():
    ...
    if not os.path.exists(configpath):
        try:
            ...
            pass
        with open(configpath, "w", encoding="UTF-8") as fb:
            fb.write(json.dumps({"theme": "monokai", "password": "", "open_and_edit": "no"}, indent=4))
    with open(configpath, "r", encoding="UTF-8") as fb:
        try:
            config_content = json.loads(fb.read())
        except:
            config_content = {"theme": "monokai", "password": "", "open_and_edit": "no"}
