...
class EarlyStopping(Callback):
	...
	def onEpochEnd(self, model, epochResults:Dict, workingDirectory:Path, isTraining:bool):
		...
		if fIsBetter(score, self.bestMetricScore):
			...
			self.numBadEpochs += 1
			Debug.log(2, "[EarlyStopping] Early Stopping is being applied. Num bad in a row: %d. Patience: %d" % \
				(self.numBadEpochs, self.patience))

		if self.numBadEpochs >= self.patience:
			Debug.log(2, "[EarlyStopping] Num bad epochs in a row: %d. Stopping the training!" % (self.numBadEpochs))
			model.finished = True
