...
from .utilities import Debug, trGetData, npGetData
from .utilities.tr_utils import _getOptimizerStr
from .metrics import Metric

# Generic module that takes a NWModule and trains it on some train/validation generators
...
class NWTrainer:
	...
	def epochPrologue(self, epochResults, isTraining:bool):
		...
		for Key in epochResults.keys():
			res[Key] = {}
			for metric in self.model.getMetrics().values():
	...
	def runOneEpoch(self, generator:Generator, stepsPerEpoch:int, isTraining:bool, \
		isOptimizing:bool, prefix:str="") -> Dict[str, float]:
		assert stepsPerEpoch > 0
		if isOptimizing == False and tr.is_grad_enabled():
			Debug.log(2, "[NWTrainer::runOneEpoch] Warning! Not optimizing, but grad is enabled.")
		...
		Range = Debug.range(stepsPerEpoch, desc="[%s] Iteration" % prefix, postfix={str(k):0.000 for k in res})
		for i in Range:
			npItems = next(generator)
			...
			if Debug.use("tqdm") and ((i % 30 == 0 or i == stepsPerEpoch - 1) or i == stepsPerEpoch - 1):
				# TODO: some architectures (i.e. graph) have deep results (so dict[dict[running_mean]])
				currentResults = {}
				for k in res:
					if not isinstance(res[k], RunningMean):
						continue
					currentResults[str(k)] = "%2.3f" % res[k].get()
				Range.set_postfix(currentResults)

		res["duration"] = datetime.now() - startTime
		return res

	def train(self, generator, numSteps, numEpochs, validationGenerator, validationNumSteps):
		currentEpoch = len(self.model.getTrainHistory()) + 1
		assert currentEpoch <= numEpochs + 1, "%d vs %d" % (currentEpoch, numEpochs)
		N = numEpochs - currentEpoch + 1
		if N == 0:
			Debug.log(1, "[NWTrainer::train] Model is already trained for %d epochs. Skipping." % currentEpoch)
			return

		self.model.numEpochs = numEpochs
		Debug.log(2, "[NWTrainer::train] Training for %d epochs (%d iterations per epoch) starting from epoch %d" \
			% (N, numSteps, currentEpoch))

		Range = Debug.range(N, initial=currentEpoch, total=numEpochs, position=0, desc="Epoch")
		self.model.finished = False
		for i in Range:
			if self.model.finished:
				Debug.log(1, "[NWTrainer::train] Finished flag was set from somewhere else. Stopping!")
				break
			...
			res = self.runOneEpoch(generator, numSteps, isTraining=True, isOptimizing=True, prefix="Train")
			...
			if not validationGenerator is None:
				...
				self.callbacksOnEpochStart(isTraining=True)
				with tr.no_grad():
					res = self.runOneEpoch(validationGenerator, validationNumSteps, \
	...
	def test(self, generator, numSteps):
		with tr.no_grad():
			self.callbacksOnEpochStart(isTraining=False)
			metricResults, duration = \
				self.runOneEpoch(generator, numSteps, isTraining=False, isOptimizing=False, prefix="Test")
