...
def process_cache(cache):
    ...
    enable_default_ttl = cache.get('enable_default_ttl', False)
    default_ttl = cache.get('default_ttl', 1)
    default_ttl_unit = cache.get('default_ttl_unit', 'min')
    ...
    if not isinstance(cache_key, list):
        raise Exception(
            'Bad cache_key field value type: ' + str(type(cache_key)))
    ...
    if enable_default_ttl:
        data['enable_default_ttl'] = True
        data['default_ttl'] = default_ttl
        data['default_ttl_unit'] = default_ttl_unit
    if enforce_cache:
