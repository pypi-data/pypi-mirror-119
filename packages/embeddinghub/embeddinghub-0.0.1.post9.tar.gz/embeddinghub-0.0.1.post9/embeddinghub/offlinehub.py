...
class Index:
    ...
    def __init__(self, key_emb_iter, dims=None):
        self._data = {}
        if dims is None:
            key_emb_iter = list(key_emb_iter)
            if len(key_emb_iter) == 0:
                raise Exception("Unable to determine dims")
            dims = len(key_emb_iter[0])
    ...
    def nearest_neighbor(self, num, key=None, embedding=None):
        ...
        if has_key:
            ...
            num_retrieve = num
        list_results, list_distances = self._idx.knn_query([embedding],
                                                           num_retrieve)
