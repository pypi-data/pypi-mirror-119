...
from utils.bounding_box_utils import rescale_bbox
...
def get_and_preprocess_template(image, template_shape, bbox):
    ...
    template = generator_utils.get_transformation(image, output_hw[:2], bbox['bounding_box'].tolist())
...
def get_annotation_vals(query):
    def _f(annotations):
        ...
        for x in annotations:
            if type(query) == list:
                anns.append([
                    utils.get_value_in_nested_dict(ann, query) for ann in x
                ])
            else:
...
def get_reshaped_masks(target_shape, rescaled_bounding_boxes):
    ...
    """
    masks = [generator_utils.get_mask(target_shape[:2], rescaled_img_bounding_boxes)
...
def convert_bb_to_yolov4bb(bounding_boxes, input_shapes, output_shapes, anchors=None):
    ...
    yolo_bounding_boxes = utils.utils.to_num(yolo_bounding_boxes)
