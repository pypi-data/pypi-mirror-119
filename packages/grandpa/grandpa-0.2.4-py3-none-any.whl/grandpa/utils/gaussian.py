...
from . import bounding_box_utils
...
class GaussianGenerator:
    ...
    def gen(self, score_shape, points_list):
        ...
        # TODO: Use polygons
        boxes = [bounding_box_utils.four_point_bb_to_2_point(points, round_values=True) for points in points_list]
        for box in boxes:
...
def reorder_points(point_list):
    ...
    if len(point_list) == 3:
        point_list = bounding_box_utils.convert_triangle_to_quadrangle(point_list)
    elif len(point_list) == 2:
        point_list = bounding_box_utils.two_point_bb_to_4_point(point_list)
    elif len(point_list) > 4:
        point_list = bounding_box_utils.convert_polygon_box_to_4_point_bb(point_list)
    elif len(point_list) < 2:
...
def get_boxes_by_id(boxes, ids):
    ...
    for b_id in ids:
        for box in boxes:
            if box['id'] == b_id:
                boxes_new.append(bounding_box_utils.convert_polygon_box_to_4_point_bb(box['box']))
                break
    return boxes_new
