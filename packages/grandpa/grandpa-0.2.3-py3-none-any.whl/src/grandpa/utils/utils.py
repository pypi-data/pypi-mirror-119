...
IR_FRAMEWORK_MODE_HEADLESS = os.getenv("IR_FRAMEWORK_MODE_HEADLESS") is not None
...
def get_training_output_path(opt: dict):
    """
    Checks if the output path in the settings file is valid.

    Args:
        opt: Parsed settings.json dict.

    Returns:
        Valid output path.
    """
    try:
        training_output_path = opt["training_output_path"]
    except KeyError:
        training_output_path = os.getcwd() + '\\training_output\\'
        if not os.path.exists(training_output_path):
            os.mkdir(training_output_path)
        print('\033[93m' + f'Warning: No training output path set in config. '
                           f'Setting output dir to default directory ({training_output_path}).' + '\033[0m')

    return check_output_path(training_output_path, 'training_output_', default_dir=os.getcwd() + '\\training_output\\')


def check_output_path(path: str, alt_folder_name: str, default_dir: str = None):
    """
    Will check if the given path is valid (empty folder), creates a valid path if the given path is not valid.

    Args:
        path: Path to check.
        alt_folder_name: Name of the folder that will be created if the given dir is not empty.
        default_dir: Default directory in case the given path does not exist. Will default to os.getcwd() if None.

    Returns:
        Valid path of an empty folder.
    """
    if not os.path.exists(path):
        if default_dir is not None:
            print('\033[93m' + f'Warning: Path {path} does not exist. '
                               f'Setting path to default directory ({default_dir}).' + '\033[0m')
            path = default_dir
        else:
            print('\033[93m' + f'Warning: Path {path} does not exist. '
                               f'Setting path to current work directory ({os.getcwd()}).' + '\033[0m')
            path = os.getcwd() + '\\'
    elif not os.path.isdir(path):
        if default_dir is not None:
            print('\033[93m' + f'Warning: Path {path} is not a directory. '
                               f'Setting path to default directory ({default_dir}).' + '\033[0m')
            path = default_dir
        else:
            print('\033[93m' + f'Warning: Path {path} is not a directory. '
    ...
    if len(os.listdir(path=path)) != 0:
        sub_dir = get_next_numeric_folder_in_dir(path, alt_folder_name)
        print('\033[93m' + f'Warning: Path {path} is not empty. '
                           f'Creating new subfolder ({sub_dir}).' + '\033[0m')
        os.mkdir(sub_dir)
        path = sub_dir
...
class LabelPrintedLenCalc:
    def __init__(self, family="Arial", font_size=20, char_num=63488):
        self.known_chars = {}
        self.family = family
    ...
    def __get_char_lens_tkinter(self):
        if os.path.exists(unify_path(os.path.join(os.getcwd(), 'cache/utf8_char_map.json'))):
            with open(unify_path(os.path.join(os.getcwd(), 'cache/utf8_char_map.json')), 'r') as file:
                self.known_chars = json.load(file)
        else:
            raise ValueError('No char map found')
