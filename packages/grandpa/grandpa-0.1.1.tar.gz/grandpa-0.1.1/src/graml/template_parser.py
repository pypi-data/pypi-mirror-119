...
from graml.built_in_nodes import ValueWrapperNode, ResultWrapperNode, SettingReaderNode, \
    ListWrapperNode, DictWrapperNode, FuncExecNode
from graml.utils import do_import, print_warning
from graml.routing import Router
import json


class TemplateParser:
    def __init__(self, template: dict, settings, router: Router):
    ...
    def init_node(self, node_address, node_definition, node_call_stack=None):
        ...
        try:
            ...
            raise GraphDefinitionError(
                f"Node {node_address} could not be found at import path {node_definition['node']}. "
                f"Please check the import path.")
    ...
    @staticmethod
    def __import_node(node: str):
        if '.' in node:
            return do_import(node)
        else:
            return do_import('graml.built_in_nodes.' + node)
