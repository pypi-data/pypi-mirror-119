...
def test_setup_cloud_aioa(mock_api_client: Mock, region: Optional[str]) -> None:
    ...
    mock_api_client.create_cloud_api_v2_clouds_post.assert_called_once_with(
...
def test_setup_cloud_aioa_invalid(mock_api_client: Mock) -> None:
    ...
    mock_api_client.create_cloud_api_v2_clouds_post.assert_not_called()
...
def test_setup_cloud_gcp_bad_region(mock_api_client: Mock) -> None:
...
def test_setup_cloud_invalid_provider(mock_api_client: Mock, is_aioa: bool) -> None:
    cloud_controller = CloudController(api_client=mock_api_client)
    with pytest.raises(ClickException):
        cloud_controller.setup_cloud(
            is_aioa=is_aioa,
...
def test_set_default_cloud(
    mock_api_client: Mock, cloud_id: Optional[str], cloud_name: Optional[str]
) -> None:
    cloud_controller = CloudController(api_client=mock_api_client)
    if not (cloud_id or cloud_name) or (cloud_id and cloud_name):
        ...
        mock_api_client.update_default_cloud_api_v2_organizations_update_default_cloud_post.assert_called_once_with(
            cloud_id="cloud_id_1"
        )
