...
class Lambda(node.StatefulNodeMixin, flow.Flow):
    ...
    def __init__(self, **kwargs):
        ...
        self.parameters_changed.connect(
    ...
    def create_output(self, port_definition=None, **kwargs):
        if (port_definition and port_definition.name == 'Function'
            ...
            return super().create_output(
                port_definition=port_definition, **kwargs)
    ...
    def _top_validate(self):
        self._state_machine.validate(False)
    ...
    def _internal_validate(self):
        ...
        if output_connected:
            ...
            valid = all(node_.is_configuration_valid()
                        for node_ in all_nodes)
            armable = self._input_event() != node.Events.inputs_are_unavailable
    ...
    def flow_input_available(self, flow_input):
        ...
        means that the parent_port is missing or not connected. In either
        case, there will be no data for the node.
        """
        # TODO: consider if this behavior could simply be changed. Removed
        # lambda inputs could behave like optional ports and allow execution.
        assert flow_input in self._flow_inputs
        return False

    def update_input_state(self):
        valid = self._internal_validate()
        state = self.state()
        event = node.Events.parameters_are_invalid
        if valid:
            event = node.Events.parameters_are_valid
        self._state_machine.change_node_state(event)
        super().update_input_state()
    ...
    def on_state_validating(self):
        self.validate_done(self._internal_validate())
    ...
    def _handle_parameters_changed(self):
        if self.is_done() or self.in_error_state() or self.is_queued():
            super().arm()
        ...
        if execute:
            result['parameters']['data']['json-data'] = {
                ...
                'value': base64.b64encode(json.dumps({
                    ...
                    'env': environment_variables.instance().to_dict(),
                    'folders': {
