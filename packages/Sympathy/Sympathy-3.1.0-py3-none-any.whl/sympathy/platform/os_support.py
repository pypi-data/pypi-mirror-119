...
class TimeoutError(Exception):
    # TODO(erik): TimeoutError is now standard, remove?
    pass
...
class WindowsCryptProtect:
    """
    https://docs.microsoft.com/en-us/windows/win32/api/dpapi/nf-dpapi-cryptprotectdata
    https://github.com/mhammond/pywin32/blob/master/win32/test/test_win32crypt.py
    """
    _CRYPTPROTECT_LOCAL_MACHINE = 0x00
    _CRYPTPROTECT_UI_FORBIDDEN = 0x01
    _CRYPTPROTECT_AUDIT = 0x03

    def __init__(self,
                 desc: str = '',
                 entropy: bytes = b'',
                 flags: int = _CRYPTPROTECT_UI_FORBIDDEN,
                 ):
        self._desc = desc
    ...
    def encrypt(self, data: bytes) -> bytes:
        return win32crypt.CryptProtectData(
            data, self._desc, self._entropy, None, None, self._flags)

    def decrypt(self, data: bytes) -> bytes:
        desc, data = win32crypt.CryptUnprotectData(
            data, self._entropy, None, None, self._flags)
        assert desc == self._desc
        return data
...
def encoded_file(filename, mode, encoding='utf8'):
    return open(filename, mode, encoding=encoding, errors='replace')
