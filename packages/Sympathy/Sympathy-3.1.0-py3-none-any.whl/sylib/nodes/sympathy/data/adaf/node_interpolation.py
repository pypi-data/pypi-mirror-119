...
def resample_file_with_spec(parameter_root, spec, in_adaffile, out_adaffile,
    ...
    raster_col = parameter_root['raster_colname'].selected
    ...
    if raster_col:
        raster_names = spec.get_column_to_array(raster_col)
    else:
        raster_names = np.zeros(signals.shape, dtype=str)

    dt_to_signals = collections.defaultdict(list)
    tbname_to_signals = collections.defaultdict(list)
    for i, (dt, to_tb, signal, raster_name) in enumerate(
            zip(dts, to_tbs, signals, raster_names)):
        if dt and not np.isnan(dt):
            dt_to_signals[(dt, raster_name)].append(signal)
        elif to_tb:
            tbname_to_signals[(to_tb, raster_name)].append(signal)
        else:
    ...
    for raster_name, dts in raster_names_dt.items():
        if len(dts) > 1:
            ...
            if raster_names and raster_name in raster_names:
                raise SyConfigurationError('Repeated raster names.')
            else:
                raise SyConfigurationError(round_error)
    for (tbname, raster_name), tb_signals in tbname_to_signals.items():
        ...
        system_name = 'Resampled system'
        raster_name = raster_name or old_raster_name
...
class InterpolateADAFsFromTable(synode.Node):
    ...
    parameters.set_list(
        'raster_colname',
        label='Column with output raster names',
        description=(
            'The selected column should contain raster names '
            'for the resampled rasters, OPTIONAL.\n'
            'When not specified, names are determined from sample rates or '
            'time bases as usual.'
        ),
        editor=synode.editors.combo_editor(
            include_empty=True, edit=True, filter=True))
...
class ChooseSpecColsTab(QtWidgets.QWidget):
    def __init__(self, parameter_root):
        ...
        layout.addWidget(parameter_root['raster_colname'].gui())
