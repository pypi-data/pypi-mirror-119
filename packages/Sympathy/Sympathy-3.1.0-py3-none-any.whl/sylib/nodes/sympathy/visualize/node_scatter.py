...
def _create_and_add_axes_3d(fig):
    ...
    if mpl_version < LooseVersion('3.4.0'):
        # In older versions Axes3D automatically added itself to the
        # figure.
        axes = Axes3D(fig)
    elif mpl_version < LooseVersion('3.5.0'):
        # In 3.4 Axes3D added itself to the figure and issued a warning
        # unless auto_add_to_figure is set to False.
        axes = Axes3D(fig, auto_add_to_figure=False)
        fig.add_axes(axes)
    else:
        # In 3.5 and above Axes3D never adds itself to the figure.
        axes = Axes3D(fig)
        fig.add_axes(axes)
    return axes
...
class Super3dNode(synode.Node):
    ...
    def execute(self, node_context):
        ...
        axes = _create_and_add_axes_3d(fig)
        with node_context.input['port1'] as tablefile:
            tabledata = tablefile.to_recarray()
...
class Plot3d(object):
    ...
    def _axes_2d_to_3d(self):
        ...
        if self._axes is not None:
            self._fig.delaxes(self._axes)
        self._axes = _create_and_add_axes_3d(self._fig)
        self._2d_axes = False
