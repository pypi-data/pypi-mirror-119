...
class MiniRealSpacePlot(GLViewWidget):
    ...
    def _refresh_photon(self):
        ...
        direction = [np.sin(alpha) * np.cos(beta),
                     np.sin(alpha) * np.sin(beta), np.cos(alpha)]
        ...
        photon_line = GLLinePlotItem(
            pos=pos, color=color, width=5, antialias=True)
    ...
    def _get_iso_mesh(self, data, color, sign):
        ...
        mesh = GLMeshItem(meshdata=mesh_data, smooth=True,
                          shader='edgeHilight')
...
class Mini3DKSpacePlot(GLViewWidget):
    ...
    def rotate_orbital(self, phi=0, theta=0, psi=0):
        ...
        # Undo current orientation
        self._rotate_item(self.mesh, old_axes, *self.orientation,
                          backward=True)
        # Actuallly rotate to new orientation
