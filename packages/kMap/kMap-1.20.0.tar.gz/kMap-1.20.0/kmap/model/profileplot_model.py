    ...
    def get_plot_data(self, data, crosshair, region,
        ...
        if region == 'x':
            ...
            y_points = np.linspace(
                data.y_axis[0], data.y_axis[-1], num=line_sample,
                endpoint=True)

        elif region == 'y':
            y_points = line_sample * [crosshair.y]
            x_points = np.linspace(
                data.x_axis[0], data.x_axis[-1], num=line_sample,
                endpoint=True)

        elif region == 'roi' or region == 'border' or region == 'ring':
            # Circular parametrization in mathematical direction
            angles = np.linspace(0, 2 * np.pi,
                                 num=phi_sample, endpoint=False)

            # Find radii smaple points
            if region == 'roi':
                radii = np.linspace(0, crosshair.radius,
                                    num=line_sample, endpoint=True)
                ...
                # Ring
                radii = np.linspace(crosshair.radius,
        ...
        intensities = np.array(data.interpolate_points(
            x_points, y_points, fill_value=np.nan))
