...
MAX_TIME_STEPS = 1e8
MAX_GRID_CELLS = 20e9
MAX_CELLS_STEPS = 5e15 # max product of grid cells and time steps
...
def _check_monitor_size(self, monitor):
    ...
    span_in = intersect_box(self.grid_sym.span, monitor.span)
    size_in = span_in[:, 1] - span_in[:, 0]
    size_in[size_in == 0] = fp_eps
    if np.any(size_in < 0):
        Np = 0
    else:
        Np = np.prod([np.ceil(s/self.grid.mesh_step[d]) for (d, s) in enumerate(size_in)])
