...
class SourceData(object):
    ...
    def __init__(self, source):
        ...
        if isinstance(source, Field2DSource):
            ...
            # Phases accounting for the temporal and spatial offset between J and M
            self.phaseMJ_s = 0
            self.phaseMJ_t = 0
    ...
    def _currents(self, src_inds):
        ...
        if isinstance(self.source, VolumeSource):
            ...
            # Turn complex source on if needed
            src_re = np.amax(np.abs(np.real(src_amps)))
            src_im = np.amax(np.abs(np.imag(src_amps)))
            if src_im/(src_re + fp_eps) > fp_eps:
                self.complex = True
    ...
    def _set_mode_phases(self):
        ...
        self.phaseMJ_t = np.pi * source.source_time.frequency * mplane.time_step
        # Spatial offset phase
        zH = (mplane.grid.coords[2][1] - mplane.grid.coords[2][0]) / 2
        self.phaseMJ_s = -self.mode.kvector[2] * zH
        self.phaseM += self.phaseMJ_t + self.phaseMJ_s
    ...
    def _set_mode_gaussian_beam(self):
        ...
        f0 = source.source_time.frequency
        zE = source.waist_distance
        ...
        GE = paraxial_gaussian_beam(xr, yr, zr, w0, f0, k0*source.dir_ind)
        xr, yr, zr = self._propagation_axes(xg, yg, zH*np.ones_like(xg))
        GH = paraxial_gaussian_beam(xr, yr, zr, w0, f0, k0*source.dir_ind)
        E[1, :, :] = E_vec[1] * GE
        H[0, :, :] = H_vec[0] * GH
        ...
        fields, as set above. We still set the temporal phase offset. """

        # Set the temporal phase offset as usual
        self.phaseMJ_t = np.pi * f0 * mplane.time_step
        self.phaseM += self.phaseMJ_t
