...
def _monitor_slice(sim, mnt_data, sind=None, normal=None, normal_ind=0,
    ...
    return (fvals, eps_r, coords, x_lab, y_lab, ax_title)

def _plot_field_2D(fvals, eps_r, coords, x_lab, y_lab, ax_title,
    ...
    im = ax.pcolormesh(coords[0], coords[1], fvals.T, cmap=cmap, vmin=clim[0], vmax=clim[1],
        edgecolor=None, lw=0)
    if eps_r is not None:
        ealph = min(eps_alpha, 1)
        if (np.amax(eps_r) - np.amin(eps_r) > 1e-10):
            _plot_eps(eps_r.T, coords, cmap=cmap_eps, clim=None, ax=ax, cbar=False, alpha=eps_alpha)
...
def viz_field_2D(self, monitor, sample_ind=0, normal=None, normal_ind=0,
                    ...
                    clim=None, cmap=None, eps_alpha=0.3, eps_freq=None):
    ...
    (fvals, eps_r, coords, x_lab, y_lab, ax_title) = _monitor_slice(
        self, mnt_data, sample_ind, normal, normal_ind, field, val, comp, eps_alpha, eps_freq)

    im = _plot_field_2D(fvals, eps_r, coords, x_lab, y_lab, ax_title, val, ax, cbar, clim,
        eps_alpha)
