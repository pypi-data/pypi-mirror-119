...
class EchoTestTask:
    ...
    def clean(self, block_id):
        ...
        self.__execute_flag = None
        self.__checked_flag = None
    ...
    def stop(self, block_id):
        if self.__current_task_info:
            ...
            while True:
                # 确保两个线程都退出了，返回stop结束
                if (self.__checked_flag is None or self.__checked_flag) \
                        and (self.__execute_flag is None or self.__execute_flag):
                    break
                time.sleep(0.1)
        else:
    ...
    def check_request_valid(self, block_id):
        if not self.__current_task_info:
            server_logger.error("Error block_id, no task is being executed.")
            return False
        if not block_id == self.__current_task_info[TEST_SUITE.BLOCK_ID]:
            server_logger.error("Error block_id, the block_id is not current block_id.")
            return False
        return True
