...
def reversible_mass_action_1_1(S1: float, P1: float, k_fwd: float, k_bwd: float) -> float:
    return k_fwd * S1 - k_bwd * P1


def reversible_mass_action_2_1(S1: float, S2: float, P1: float, k_fwd: float, k_bwd: float) -> float:
    return k_fwd * S1 * S2 - k_bwd * P1


def reversible_mass_action_3_1(
    S1: float, S2: float, S3: float, P1: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 * S3 - k_bwd * P1


def reversible_mass_action_4_1(
    S1: float, S2: float, S3: float, S4: float, P1: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 * S3 * S4 - k_bwd * P1


def reversible_mass_action_1_2(S1: float, P1: float, P2: float, k_fwd: float, k_bwd: float) -> float:
    return k_fwd * S1 - k_bwd * P1 * P2


def reversible_mass_action_2_2(
    S1: float, S2: float, P1: float, P2: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 - k_bwd * P1 * P2


def reversible_mass_action_3_2(
    S1: float, S2: float, S3: float, P1: float, P2: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 * S3 - k_bwd * P1 * P2


def reversible_mass_action_4_2(
    S1: float, S2: float, S3: float, S4: float, P1: float, P2: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 * S3 * S4 - k_bwd * P1 * P2


def reversible_mass_action_1_3(
    S1: float, P1: float, P2: float, P3: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 - k_bwd * P1 * P2 * P3


def reversible_mass_action_2_3(
    S1: float, S2: float, P1: float, P2: float, P3: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 - k_bwd * P1 * P2 * P3


def reversible_mass_action_3_3(
    S1: float, S2: float, S3: float, P1: float, P2: float, P3: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 * S3 - k_bwd * P1 * P2 * P3


def reversible_mass_action_4_3(
    S1: float, S2: float, S3: float, S4: float, P1: float, P2: float, P3: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 * S3 * S4 - k_bwd * P1 * P2 * P3


def reversible_mass_action_1_4(
    S1: float, P1: float, P2: float, P3: float, P4: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 - k_bwd * P1 * P2 * P3 * P4


def reversible_mass_action_2_4(
    S1: float, S2: float, P1: float, P2: float, P3: float, P4: float, k_fwd: float, k_bwd: float
) -> float:
    return k_fwd * S1 * S2 - k_bwd * P1 * P2 * P3 * P4


def reversible_mass_action_3_4(
    S1: float, S2: float, S3: float, P1: float, P2: float, P3: float, P4: float, k_fwd: float, k_bwd: float
) -> float:
...
def reversible_michaelis_menten(
    S: float, P: float, vmax_fwd: float, vmax_bwd: float, kms: float, kmp: float
) -> float:
    return (vmax_fwd * S / kms - vmax_bwd * P / kmp) / (1 + S / kms + P / kmp)


def reversible_uncompetitive_inhibition(
    S: float, P: float, I: float, vmax_fwd: float, vmax_bwd: float, kms: float, kmp: float, ki: float
) -> float:
...
def reversible_michaelis_menten_keq(
    S: float, P: float, vmax_fwd: float, kms: float, kmp: float, keq: float
) -> float:
...
def ordered_2(A: float, B: float, vmax: float, kmA: float, kmB: float, kiA: float) -> float:
...
def random_order_2(A: float, B: float, vmax: float, kmA: float, kmB: float, kiA: float) -> float:
    return vmax * A * B / (A * B + kmB * A + kmA * B + kiA * kmB)
...
def random_order_2_2(
    ...
    nominator = vmaxf * A * B / (kiA * kmB) - vmaxr * P * Q / (kmP * kiQ)
    denominator = (
        1 + (A / kiA) + (B / kiB) + (P / kiP) + (Q / kiQ) + (A * B / (kiA * kmB)) + (P * Q / (kmP * kiQ))
    )
    return nominator / denominator


def ping_pong_2(A: float, B: float, vmax: float, kmA: float, kmB: float) -> float:
    return vmax * A * B / (A * B + kmA * B + kmB * A)


def ping_pong_3(A: float, B: float, C: float, vmax: float, kmA: float, kmB: float, kmC: float) -> float:
    return (vmax * A * B * C) / (A * B * C + (kmA * B * C) + (kmB * A * C) + (kmC * A * B))


def ping_pong_4(
    A: float, B: float, C: float, D: float, vmax: float, kmA: float, kmB: float, kmC: float, kmD: float
) -> float:
    return (vmax * A * B * C * D) / (
