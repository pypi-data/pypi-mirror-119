...
class LinearLabelModel(_AbstractStoichiometricModel):
    ...
    def generate_y0(
        self, initial_labels: Union[Dict[str, int], Dict[str, List[int]]] = None
    ) -> Dict[str, float]:
    ...
    def get_fluxes_dict(
        ...
        return self._get_fluxes(fcd=y, v_ss=v_ss, external_label=external_label)
    ...
    def _get_rhs(self, t: float, y_labels: List[float]) -> List[float]:  # type: ignore[override]
    ...
    def get_right_hand_side(
        self,
        y_labels: Union[List[float], Dict[str, float]],
        y_ss: Dict[str, float],
        v_ss: Dict[str, float],
        external_label: float = 1.0,
        t: float = 0.0,
        ...
        if isinstance(y_labels, dict):
            y_labels = cast(List[float], [y_labels[i] for i in self.compounds])
        return dict(zip(self.compounds, self._get_rhs(t=t, y_labels=y_labels)))
