...
class _BaseSimulator(Generic[BASE_MODEL_TYPE], ABC):
    ...
    def _initialise_integrator(self, *, y0: Union[np.ndarray, List[float]]) -> None:
    ...
    def _normalise_split_array(
        self,
        *,
        split_array: List[np.ndarray],
        normalise: Union[float, List[float]],
    ) -> List[np.ndarray]:
    ...
    def _save_simulation_results(self, *, time: np.ndarray, results: np.ndarray, skipfirst: bool) -> None:
    ...
    def get_time(self, concatenated: bool = True) -> Union[None, np.ndarray, List[np.ndarray]]:
        """Get simulation time.
    ...
    def simulate(
        ...
        **integrator_kwargs: Dict[str, Any],
        ...
        self._save_simulation_results(time=time_array, results=results_array, skipfirst=True)
    ...
    def simulate_to_steady_state(
        ...
        **integrator_kwargs: Dict[str, Any],
        ...
        time_array = np.array([time])
        results_array = np.array([results])
        self._save_simulation_results(time=time_array, results=results_array, skipfirst=False)
        return time_array, results_array
    ...
    def get_results_dict(
        ...
        normalise: Union[float, List[float]] = None,
        concatenated: bool = True,
    ) -> Union[None, Dict[str, np.ndarray], List[Dict[str, np.ndarray]]]:
        """Get simulation results.
        ...
        if results is None:
            return None
        if concatenated:
            results = cast(np.ndarray, results)  # this is just a typing annotation
            return dict(zip(self.model.get_compounds(), results.T))
        else:
            results = cast(List[np.ndarray], results)  # this is just a typing annotation
    ...
    def store_results_to_file(self, filename: str, filetype: str = "json") -> None:
        ...
        res = cast(
            Dict[str, np.ndarray], self.get_results_dict(concatenated=True)
        )  # cast is just typing annotation
        time = cast(np.ndarray, self.get_time(concatenated=True))  # cast is just typing annotation
        res["time"] = time
    ...
    def load_results_from_file(self, filename: str, filetype: str = "json") -> None:
        ...
        if filetype == "json":
            with open(filename, "r") as f:
                res: Dict[str, np.ndarray] = json.load(fp=f)
        elif filetype == "pickle":
...
class _BaseRateSimulator(Generic[RATE_MODEL_TYPE], _BaseSimulator[RATE_MODEL_TYPE]):
    def __init__(
        self,
        ...
        _BaseSimulator.__init__(self, model=model, integrator=integrator, y0=y0, time=time, results=results)
        self.full_results: Optional[List[np.ndarray]] = None
        self.fluxes: Optional[List[np.ndarray]] = None
    ...
    def update_parameter(self, parameter_name: str, parameter_value: float) -> None:
    ...
    def _calculate_fluxes(self) -> None:
        fluxes = []
        if any((i is None for i in [self.time, self.results, self.simulation_parameters])):
            return None
        for t, y, pars in zip(
            cast(List[np.ndarray], self.time),
            cast(List[np.ndarray], self.results),
            cast(List[Dict[str, float]], self.simulation_parameters),
        ):
    ...
    def get_fluxes_array(
        ...
        fluxes = cast(List[np.ndarray], self.fluxes)
        if normalise is not None:
    ...
    def get_fluxes_dict(
        ...
        normalise: Union[float, List[float]] = None,
        concatenated: bool = True,
    ) -> Optional[Union[Dict[str, np.ndarray], List[Dict[str, np.ndarray]]]]:
        """Get the model fluxes for the simulation."""
        fluxes = self.get_fluxes_array(normalise=normalise, concatenated=concatenated)
        if fluxes is None:
            return None
        if concatenated:
            return dict(zip(self.model.rates, cast(np.ndarray, fluxes).T))
        else:
    ...
    def get_full_results_array(
        ...
        full_results = cast(List[np.ndarray], self.full_results).copy()
        if normalise is not None:
    ...
    def get_full_results_dict(
        ...
        if concatenated:
            return dict(zip(all_compounds, cast(np.ndarray, full_results).T))
        else:
    ...
    def get_full_results_df(
        ...
        if concatenated:
            ...
            return [
                pd.DataFrame(data=res, index=t, columns=all_compounds) for t, res in zip(time, full_results)
            ]
    ...
    def get_variable(
        ...
        if full_results_dict is None:
            return None
        if concatenated:
            return cast(Dict[str, np.ndarray], full_results_dict)[variable]
        else:
            return [i[variable] for i in cast(List[Dict[str, np.ndarray]], full_results_dict)]
    ...
    def _parameter_scan_worker(
        ...
        *,
        ...
        if t is None or y is None:
            concentrations = dict(zip(m.get_all_compounds(), np.full(len(m.get_all_compounds()), np.NaN)))
    ...
    def _scan_threading(self, parameter_name: str, parameter_values: list[float], worker: Callable) -> list:
    ...
    def _scan_multiprocessing(
        self, parameter_name: str, parameter_values: list[float], worker: Callable
    ) -> list:
        with tqdm(total=len(parameter_values), desc=parameter_name) as pbar:
    ...
    def parameter_scan_with_fluxes(
        ...
        **integrator_kwargs: Dict[str, Any],
        ...
        for (i, conc, flux) in results:
            concentrations[i] = conc
            fluxes[i] = flux
        return pd.DataFrame(concentrations).T.sort_index(), pd.DataFrame(fluxes).T.sort_index()

    def parameter_scan_2d(
        self,
        p1: Tuple[str, List[float]],
        p2: Tuple[str, List[float]],
        tolerance: float = 1e-8,
        multiprocessing: bool = True,
        **integrator_kwargs: Dict[str, Any],
        ...
        for value in tqdm(parameter_values2, total=len(parameter_values2), desc=parameter_name2):
    ...
    def plot_grid(
        ...
        if x is None or y is None:
            return None, None
        plot_groups = [(x, y.loc[:, compounds].values) for compounds in compound_groups]
        return plot_grid(
    ...
    def plot_against_variable(
        ...
        x = results.loc[:, variable]
        y = results.loc[:, compounds]
        return plot(
            plot_args=(x, y),
            legend=compounds,
            xlabel=xlabel,
    ...
    def plot_fluxes_grid(
        ...
        if time is None or fluxes is None:
            return None, None
        fluxes = cast(pd.DataFrame, fluxes)
        plot_groups = [
            (
                time,
                fluxes.loc[:, group].values,
            )
    ...
    def plot_fluxes_against_variable(
        ...
        if x is None or y is None:
            return None, None
        y = cast(pd.DataFrame, y).loc[:, rate_names]
        return plot(
            plot_args=(x, y),
            legend=rate_names,
            xlabel=xlabel,
