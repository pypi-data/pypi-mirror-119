...
class _LabelSimulate(_BaseRateSimulator[LabelModel]):
    ...
    def generate_y0(
        self,
        base_y0: Union[List[float], Dict[str, float]],
    ...
    def get_total_concentration(self, compound: str) -> Optional[np.ndarray]:
        """Get the total concentration of all isotopomers of a compound."""
        res = self.get_full_results_dict(concatenated=True)
        if res is None:
            return None
        return cast(Dict[str, np.ndarray], res)[compound + "__total"]

    def get_unlabeled_concentration(self, compound: str) -> Optional[np.ndarray]:
        """Get the concentration of an isotopomer that is unlabeled."""
        carbons = "0" * self.model.label_compounds[compound]["num_labels"]
        res = self.get_full_results_dict()
        if res is None:
            return None
        return cast(Dict[str, np.ndarray], res)[compound + f"__{carbons}"]

    def get_total_label_concentration(self, compound: str) -> Optional[np.ndarray]:
        ...
        if total is None or unlabeled is None:
            return None
        return cast(np.ndarray, total - unlabeled)

    def get_all_isotopomer_concentrations_array(self, compound: str) -> Optional[np.ndarray]:
        """Get concentrations of all isotopomers of a compound."""
        res = self.get_all_isotopomer_concentrations_df(compound=compound)
        if res is None:
            return None
        return cast(np.ndarray, res.values)

    def get_all_isotopomer_concentrations_dict(self, compound: str) -> Optional[Dict[str, np.ndarray]]:
        """Get concentrations of all isotopomers of a compound."""
        res = self.get_all_isotopomer_concentrations_df(compound=compound)
        if res is None:
            return None
        return dict(zip(res.columns, res.values.T))
    ...
    def get_concentrations_by_reg_exp_array(self, reg_exp: str) -> Optional[np.ndarray]:
        ...
        if isotopomers is None or df is None:
            return None
        df = cast(pd.DataFrame, df)[isotopomers]
        return cast(np.ndarray, df[isotopomers].values)

    def get_concentrations_by_reg_exp_dict(self, reg_exp: str) -> Optional[Dict[str, np.ndarray]]:
        """Get concentrations of all isotopomers of a compound."""
        isotopomers = [i for i in self.model.get_compounds() if re.match(reg_exp, i)]
    ...
    def get_concentration_at_positions(
        ...
        if res is None:
            return None
        return cast(np.ndarray, np.sum(res, axis=1))

    def get_concentrations_of_n_labeled_array(self, compound: str, n_labels: int) -> Optional[np.ndarray]:
        """Get concentrations of all isotopomers that carry n labels."""
        res = self.get_concentrations_of_n_labeled_df(compound=compound, n_labels=n_labels)
        if res is None:
            return None
        return cast(np.ndarray, res.values)
    ...
    def get_concentrations_of_n_labeled_df(self, compound: str, n_labels: int) -> Optional[pd.DataFrame]:
        """Get concentrations of all isotopomers that carry n labels."""
        ...
        if res is None:
            return None
        return cast(pd.DataFrame, res)[isotopomers]
    ...
    def plot(
        ...
        x = self.get_time()
        y = self.get_full_results_df(concatenated=True)
        if x is None or y is None:
            return None, None
        y = cast(pd.DataFrame, y).loc[:, compounds]
        return plot(
            plot_args=(x, y),
            legend=None,
            xlabel=xlabel,
    ...
    def _calculate_label_distribution(self, *, compound: str, relative: bool) -> Optional[np.ndarray]:
