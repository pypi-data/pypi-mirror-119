...
# from material_engineering_QBD.chemical_QBD import const
...
def interpolation__exception__AB(material: str, parameters: dict, bowings: dict):
    ...
    return fraction__A * parameters[material__A] + fraction__B * parameters[material__B] - fraction__A * fraction__B * bowings[material__A + material__B]
...
def interpolation__exception__ABC(material: str, parameters: dict, bowings: dict):
    ...
    if fraction__A == .0:   return fraction__B * parameters[element__A + element__B] + fraction__C * parameters[element__A + element__C] - fraction__B * fraction__C * bowings[composition]
    elif fraction__C == .0: return fraction__A * parameters[element__A + element__C] + fraction__B * parameters[element__B + element__C] - fraction__A * fraction__B * bowings[composition]
    else:
...
def profile__plain(thicknesses, values):
    ...
    for i in range(0,len(thicknesses) - 1):
        ground.append(thicknesses[i])
        fence.append(values[i])

        ground.append(thicknesses[i])
        fence.append(values[i + 1])

    ground.append(thicknesses[-1])
