...
from pyrasgo.primitives import Feature, FeatureList, DataSource
...
class Save():
    ...
    def feature(self,
        ...
        if ft:
            if if_exists == 'return':
                ...
                raise APIError(f"Column {column_name} is already registered in DataSource {data_source_id} as Feature {ft.id}")
    ...
    def features_dict(self, features_dict: dict,
        ...
        tags = []
        if features_dict.get("tags"):
            for t in features_dict.get("tags"):
                tags.append(t)
        attributes = []
        if features_dict.get("attributes"):
            for a in features_dict.get("attributes"):
                for k, v in a.items():
                    attributes.append({k: v})
        source_code = features_dict.get("sourceCode")
        source_type = features_dict.get("sourceType", "Table")
        ...
        for dim in features_dict["dimensions"]:
            ...
            ds_col_id = ds_column[0] if len(ds_column)>0 else None
        ...
        for feature in features_dict["features"]:
            ...
            for kv in feature_attributes:
                for i in kv:
                    attributes_put.append(api.Attribute(key=i, value=kv[i]))
    ...
    def source_features(self, id: int,
                        ...
                        columns: List[api.DataSourceColumnPut],
                        features: Optional[List[api.FeatureColumnPut]],
                        dimensions: Optional[List[api.DimensionColumnPut]],
        ...
        data_source = api.DataSourcePut(name=name,
                                        table=table,
