...
class HubController:
	...
	def get_sku_by_id(self, sku: str):
		return requests.get(f'{self.__get_endpoint_base()}/skus/{sku}', headers=self.headers)
	...
	def put_products(self, products: List[HubProduct], seller_id: int):
		return requests.put(f'{self.__get_endpoint_base()}/sellers/{seller_id}/skus/bulk', headers=self.headers, data=unpack_payload_dict(products))

	@AttemptRequests(success_codes=[200])
	def put_order(self, external_seller_id: str, external_channel_id: str, order: HubOrder):
		return requests.put(f'{self.__get_endpoint_base()}/sellers/{external_seller_id}/channels/{external_channel_id}/orders', data=unpack_payload_dict(order))

	@AttemptRequests(success_codes=[200])
	def get_logistics_shipping_fee(self, seller_id: str, zip_code: str):
		return requests.get(f'{self.__get_endpoint_base()}/sellers/{seller_id}/logistics/zipcode/{zip_code}/shipping/fee')

	@AttemptRequests(success_codes=[200])
	def get_seller_channel_type(self, external_seller_id: str, external_channel_id: str, channel_type: ChannelType):
		return requests.get(f'{self.__get_endpoint_base()}/sellers/{external_seller_id}/channels/{external_channel_id}/type/{channel_type.value}')
