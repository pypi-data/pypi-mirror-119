...
class HubController:
	...
	def get_price_and_stock_by_external_ids(self, external_seller_id: str, external_channel_id: str, external_sku: str):
		return requests.get(f'{self.environment.value}/sellers/{external_seller_id}/channels/{external_channel_id}/skus/external/{external_sku}', headers=self.headers)
	...
	def put_order(self, external_seller_id: str, external_channel_id: str, order: HubOrder):
		return requests.put(f'{self.environment.value}/sellers/{external_seller_id}/channels/{external_channel_id}/orders', data=unpack_payload_dict(order))

	@AttemptRequests(success_codes=[200])
	def get_logistics_shipping_fee(self, seller_id: str, zip_code: str):
		return requests.get(f'{self.environment.value}/sellers/{seller_id}/logistics/zipcode/{zip_code}/shipping/fee')

	@AttemptRequests(success_codes=[200])
	def get_seller_channel_type(self, external_seller_id: str, external_channel_id: str, channel_type: str):
		return requests.get(f'{self.environment.value}/sellers/{external_seller_id}/channels/{external_channel_id}/type/{channel_type}')
