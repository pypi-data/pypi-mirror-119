...
class HubController:
	...
	@AttemptRequests(success_codes=[201])
	def put_order(self, seller_name: str, channel_name: str, order: HubOrder):
		return requests.put(f'{self.environment.value}/sellers/{seller_name}/channels/{channel_name}/orders', data=unpack_payload_dict(order))

	@AttemptRequests(success_codes=[200])
	def get_seller_channel_by_name(self, seller_name: str, channel_name: str):
		return requests.post(f'{self.environment.value}/sellers/{seller_name}/orders')
