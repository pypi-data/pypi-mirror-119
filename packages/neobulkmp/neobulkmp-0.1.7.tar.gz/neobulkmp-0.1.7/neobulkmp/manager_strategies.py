...
class StrategyBase:
    ...
    cache_storage_available = None

    def __init__(self, manager: "neobulkmp.Manager"):
        self.manager = manager
        if self.cache_storage_available is None:
            self.cache_storage_available = (
                getattr(psutil.virtual_memory(), "total") * 0.80
            )
        else:
            self.cache_storage_available = humanfriendly.parse_size(
                self.cache_storage_available
            )
    ...
    def _get_cache_size_total(self) -> int:
        return self.cache_storage_available
