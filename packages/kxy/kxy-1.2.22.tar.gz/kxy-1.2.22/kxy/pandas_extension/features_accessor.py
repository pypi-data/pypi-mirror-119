...
class FeaturesAccessor(BaseAccessor):
	...
	def entity_features(self, entity, exclude=[], entity_name='*'):
		...
		if ord_columns:
			...
			agg.update({'SUM(|%s|)' % col: (col, nansumabs) for col in ord_columns})
			agg.update({'MEAN(|%s|)' % col: (col, nanmeanabs) for col in ord_columns})
			agg.update({'STD(|%s|)' % col: (col, nanstdabs) for col in ord_columns})
			agg.update({'MEDIAN(|%s|)' % col: (col, nanmedianabs) for col in ord_columns})
			agg.update({'SKEW(|%s|)' % col: (col, nanskewabs) for col in ord_columns})
			agg.update({'KURT(|%s|)' % col: (col, nankurtosisabs) for col in ord_columns})
			agg.update({'Q25(|%s|)' % col: (col, q25abs) for col in ord_columns})
			agg.update({'Q75(|%s|)' % col: (col, q75abs) for col in ord_columns})
			agg.update({'MIN(|%s|)' % col: (col, nanminabs) for col in ord_columns})
			agg.update({'MAX(|%s|)' % col: (col, nanmaxabs) for col in ord_columns})
			agg.update({'MAX(|%s|)-MIN(|%s|)' % (col, col): (col, nanmaxmminabs) for col in ord_columns})
