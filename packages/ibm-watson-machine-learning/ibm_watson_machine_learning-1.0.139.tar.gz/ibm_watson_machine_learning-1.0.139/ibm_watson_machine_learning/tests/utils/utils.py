...
__all__ = [
    ...
    "setup_nfs_env",
    "save_multiple_data_to_containers",
    "save_data_to_container",
    "save_multiple_data_to_cos_bucket",
...
def create_bucket(cos_resource, bucket_name):
    """
    Creates bucket of with a given name and timestamp suffix.
    """
    try:
...
def save_multiple_data_to_containers(data_path, data_filenames, wml_client, data_cos_path=None):
    for file in data_filenames:
        cos_file_path = os.path.join(data_cos_path, file) if data_cos_path else file
        save_data_to_container(os.path.join(data_path, file), cos_file_path, wml_client)


def save_data_to_container(data_path, data_cos_path, wml_client):
    import requests
...
def save_multiple_data_to_cos_bucket(data_path, data_filenames, access_key_id, secret_access_key, cos_endpoint,
                                     bucket_name, data_cos_path=None):
    for file in data_filenames:
        data_file_path = os.path.join(data_path, file) if data_path else file
        cos_file_path = os.path.join(data_cos_path, file) if data_cos_path else file

        bucket_name = save_data_to_cos_bucket(data_file_path,
                                              cos_file_path,
