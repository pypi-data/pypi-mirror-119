...
os.chdir(r"/home/liangyi/anaconda3/data/MDLE")
...
def display_current_JSONDF():
    for i in range(len(Working_on_sch)):
        ...
        display(Working_on_sch[i])
        display(Working_on_add[i])
        display('----------------------------------------------------------------')
...
def editDFs(inname:str,inValue:str):
    ...
    for i in range(len(chunks_list)):
        for j in Working_on_add[i].index:
            if Working_on_add[i].loc[j,'name']==inname:
                Working_on_add[i].loc[j,'value']=inValue

def dealWithInput(inStr:str):
    if "?" == inStr[0]:
        if inStr[1:] in all_name.keys():
            return all_name[inStr[1:]]
        else:
...
temName='SunsVoc_json_template.json'
...
for i in range(len(chunks_list)):
    ...
    for j in s_app.index:
        if "$" in s_app.loc[j,'value']:
            if s_app.loc[j,'name'] not in all_name:
                all_name[s_app.loc[j,'name']]=s_app.loc[j,'description']
            else:
                all_name[s_app.loc[j,'name']]=all_name[s_app.loc[j,'name']]+', '+s_app.loc[j,'description']
...
while exitFlag!=True:
    ...
    inputStr=input('Input the command, use ?name to search detial description, xxx>xxxx to edit base on name, !save to save json file, !exit to exit')  
