...
class ActivityLink:
    """
    Holds three variations of the invite link. Without any attributes, returns complete http link
...
class DisnakeTogether:
    ...
    Parameters
    ----------
    client/bot: Union[:class:`disnake.Client`, :class:`disnake.AutoShardedClient`, :class:`commands.Bot`,
    ...
    def __init__(self, client: Union[Client, Bot, AutoShardedClient, AutoShardedBot], *, debug: Optional[bool] = False):
        ...
        Parameters
        ----------
        client: Union[:class:`disnake.Client`, :class:`disnake.ext.Bot`]
            The client/bot variable used for your project
        ...
        if isinstance(debug, bool):
            ...
            self.debug = False
            print('\033[93m' + "[WARN] (disnake_together) Debug parameter did not receive a bool object. "
                               "Reverting to Debug = False." + '\033[0m')

    async def create_link(self, voice_channel_id: Union[int, str], option: Union[int, str], *,
                          max_age: Optional[int] = 0,
                          max_uses: Optional[int] = 0) -> ActivityLink:
        """
        Generates an invite link to a VC with the Discord Party VC Feature.

        Parameters
        ----------
        voice_channel_id: Union[:class:`str`, :class:`int`]
        ...
        if option and (str(option).lower().replace(" ", "") in defaultApplications.keys()):
            ...
            # Error Handling
                ...
                if self.debug:
                    async with ClientSession() as session:
                        async with session.post(f"https://discord.com/api/v8/channels/{voice_channel_id}/invites",
                                                json=data,
                                                headers={
                    ...
                    print('\033[95m' + '\033[1m' + '[DEBUG] (disnake_together) Response Output:\n' + '\033[0m' + str(
                        result))
            ...
            if self.debug:
                print(
                    '\033[95m' + '\033[1m' + '[DEBUG] (disnake_together) Response Output:\n' + '\033[0m' + str(result))

            return ActivityLink(result['code'])

        # User Defined Application ID
        elif option and (str(option).replace(" ", "") not in defaultApplications.keys()) \
            ...
            raise InvalidActivityChoice(
                ...
                "input a custom application ID.".format('", "'.join(defaultApplications.keys()))
            )
