...
from . import subjects, dart, data
...
class AnalyseOneSubject:
    def __init__(self, edate: str, subject: str, pkl: Pickle):
        ...
        self.dartinfo_list = dart.get_dartinfo_list(edate=edate, title=subject)
        self.pickle = pkl

    def pre_ansys_test(self, dartinfo: data.DartInfo) -> bool:
        """
        1. 코드가 등록되지 않은 종목
        2. 이미 이전에 분석된 공시
        3. 이미 이전에 노티한 공시
    ...
    @staticmethod
    def notify_and_write_record(dartinfo: data.DartInfo, cls: subjects.DartSubject):
        """
        ...
        try:
            ...
            noti.telegram_to(botname='eval', text=report.for_telegram(code=dartinfo.code))
        except TimedOut:
            time.sleep(3)
        ...
        mongo.Noti().save(noti_dict=noti_data)

    @staticmethod
    def distribute_to_corps_db(dartinfo: data.DartInfo, cls: subjects.DartSubject, is_noti: bool):
        """
        ...
        dart_data = {'rcept_no': dartinfo.rno,
                     ...
                     'text': cls.score.opinion,
                     'is_noti': is_noti}
        mongo.DartByCode(code=dartinfo.code).save(dart_dict=dart_data)
    ...
    def run(self) -> bool:
        ...
        for i, dartinfo in enumerate(self.dartinfo_list):
            ...
            subject_cls = getattr(subjects, self.subject)(dartinfo, driver)
            subject_cls.run()
            self.pickle.contents.analysed.append(dartinfo.rno)

            # 분석후 포인트가 노티포이트를 넘기고 거래정지 종목이 아니면 노티한다.
            logger.info(f'subject_cls.score : {subject_cls.score}')
            if (subject_cls.score.point >= subjects.DartSubject.NOTI_POINT and
                    not self.is_trading_halt(code=dartinfo.code, driver=driver)):
                is_noti = True
