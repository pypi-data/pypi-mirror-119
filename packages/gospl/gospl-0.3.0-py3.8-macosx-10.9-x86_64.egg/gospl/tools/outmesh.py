...
class WriteMesh(object):
    ...
    def _outputMesh(self):
        ...
        with h5py.File(h5file, "w") as f:
            f.create_dataset(
                "elev", shape=(self.lpoints, 1), dtype="float32", compression="gzip",
            )
            f["elev"][:, 0] = self.hLocal.getArray()
            f.create_dataset(
                "erodep", shape=(self.lpoints, 1), dtype="float32", compression="gzip",
            )
            f["erodep"][:, 0] = self.cumEDLocal.getArray()
            f.create_dataset(
                "flowAcc", shape=(self.lpoints, 1), dtype="float32", compression="gzip",
            )
            ...
            f["flowAcc"][:, 0] = data
            f.create_dataset(
                "fillFA", shape=(self.lpoints, 1), dtype="float32", compression="gzip",
            )
            ...
            f["fillFA"][:, 0] = data
            f.create_dataset(
                "sedLoad", shape=(self.lpoints, 1), dtype="float32", compression="gzip",
            )
