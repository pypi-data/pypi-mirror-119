...
class ConfigService:
    ...
    def update_merged_config(self):
        ...
        for provider in providers:
            ...
            if providerId:
                ...
                if saved_provider_ids[provider] == providerId:
    ...
    def get_template_render_paths(self, key=None):
        result = get_dict_item(self.local_config_raw, ['template', 'render']) or {}
        if not key:
            return result
        else:
    ...
    def register_template_custom_render_path(self, key, render_path):
        ...
        self.local_config_raw['template']['render'] = result
        self.save_local_config()

    def unregister_template_custom_render_path(self, key):
        result = get_dict_item(self.local_config_raw, ['template', 'render'])
        if key in result:
            self.local_config_raw['template']['render'].pop(key)
            self.save_local_config()
    ...
    def delete(self, key, config_scope=ConfigScope.Local):
        if config_scope == ConfigScope.Global:
            Logger.info(f"Deleting '{key}' from the global DSO configurations...")
            parent = get_dict_item(self.global_config_raw, key.split('.')[:-1])
    ...
    def init(self, working_dir, init_config, config_overrides, override_locally):
