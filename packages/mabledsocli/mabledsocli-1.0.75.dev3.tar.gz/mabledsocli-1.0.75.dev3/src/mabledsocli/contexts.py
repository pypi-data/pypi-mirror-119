...
class Context():
    ...
    @property
    def effective(self):
        return self.get_namespace(), self.get_project(), self.get_application(), self.get_stage(), self.scope

    @property
    def requested(self):
        return self.get_namespace(ignore_scope=True), self.get_project(ignore_scope=True), self.get_application(ignore_scope=True), self.get_stage(ignore_scope=True)
...
class ContextService():
    ...
    # def load_local_config(self, silent=True):
    ...
    #     if not int(self.merged_config['version']) == int(__default_config['version']):
    #         if int(self.merged_config['version']) > int(__default_config['version']):
    #             Logger.warn(MESSAGES['DSOConfigNewer'].format(__default_config['version'], self.merged_config['version']))
    #         else:
    #             Logger.warn(MESSAGES['DSOConfigOlder'].format(__default_config['version'], self.merged_config['version']))
