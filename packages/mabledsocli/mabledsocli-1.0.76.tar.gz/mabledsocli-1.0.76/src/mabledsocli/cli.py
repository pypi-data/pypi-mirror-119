...
@click.option('--config-override', 'config_override', metavar='<key>=<value>,...', required=False, default='', show_default=False, help=f"{CLI_PARAMETERS_HELP['common']['config']}")
...
def edit_parameter(namespace, project, application, stage, scope, global_scope, project_scope, verbosity, config_override, working_dir, key, key_option):
    ...
    try:
        ...
        if result['Parameters']:
            ...
            raise DSOException(CLI_MESSAGES['ParameterNotFound'].format(key, Configs.get_namespace(ContextSource.Target), Configs.get_project(ContextSource.Target), Configs.get_application(ContextSource.Target), Configs.get_short_stage(ContextSource.Target), Configs.scope))
...
def edit_secret(namespace, project, application, stage, scope, global_scope, project_scope, verbosity, config_override, working_dir, key, key_option):
    ...
    try:
        ...
        if result['Secrets']:
            ...
            raise DSOException(CLI_MESSAGES['SecretNotFound'].format(key, Configs.get_namespace(ContextSource.Target), Configs.get_project(ContextSource.Target), Configs.get_application(ContextSource.Target), Configs.get_short_stage(ContextSource.Target), Configs.scope))
...
def edit_template(namespace, project, application, stage, scope, global_scope, project_scope, verbosity, config_override, working_dir, key, key_option):
    ...
    try:
        ...
        if result['Templates']:
            ...
            raise DSOException(CLI_MESSAGES['TemplateNotFound'].format(key, Configs.get_namespace(ContextSource.Target), Configs.get_project(ContextSource.Target), Configs.get_application(ContextSource.Target), Configs.get_short_stage(ContextSource.Target), Configs.scope))
...
@click.option('-m', '--source', required=False, type=click.Choice(['app', 'full', 'summary']), default='app', show_default=True, help=f"{CLI_PARAMETERS_HELP['network']['subnet_layout_mode']}")
...
@click.option('-b', '--verbosity', metavar='<number>', required=False, type=RangeParamType(click.INT, minimum=0, maximum=6), default='2', show_default=True, help=f"{CLI_PARAMETERS_HELP['common']['verbosity']}")
@click.option('-w','--working-dir', metavar='<path>', type=click.Path(exists=True, file_okay=False), required=False, help=f"{CLI_PARAMETERS_HELP['common']['working_dir']}")
def network_layout_subnet_plan(namespace, project, application, stage, scope, global_scope, project_scope, verbosity, config_override, working_dir, source, format):
    ...
    try:
        ...
        if source == 'app':
            ...
            result = Networks.layout_subnet_plan(subnet_plan, summary=(source == 'summary'))
