...
class NUEVA_CAT(object):
    ...
    def __init__(self, con, id_prop, padre, editar=False):
        ...
        vbox_main = Gtk.VBox()
        L_nombre_cod = Gtk.Label("Nombre Categoría")
        self.E_nombre_cod = Gtk.Entry()
        L_nombre_col = Gtk.Label("color de la categoría")
    ...
    def __edicion(self, ed):
        ...
        """
        cod_tam = 'SELECT * FROM categoria WHERE id_cat = ' + ed
    ...
    def aceptar_ed(self, widget):
        ...
        cursorObj = self.con.cursor()
        c1 = 'UPDATE categoria SET nombre_cat = "' + nombre_cod + '"  where id_cat = ' +str(self._id_cat)
        cursorObj.execute(c1)
        self.con.commit()
        c2 = 'UPDATE categoria SET color = "' + color + '"  where id_cat = ' + str(self._id_cat)
        cursorObj.execute(c2)
        self.con.commit()
        c3 = 'UPDATE categoria SET memo = "' + memo + '"  where id_cat = ' + str(self._id_cat)
