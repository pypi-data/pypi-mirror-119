...
def deprecated_node(updated_node_name: str):
    """
    ...
    Parameters
    ----------
    updated_node_name : str
        The name of the node replacing the deprecated node.
    ...
    def decorator(node):
        ...
        def new_create_node_data(*args, **kwargs):
            warnings.warn(
                f"The '{node.name}' node will be removed in the future. "
                f"Please use '{updated_node_name}' instead."
            )
