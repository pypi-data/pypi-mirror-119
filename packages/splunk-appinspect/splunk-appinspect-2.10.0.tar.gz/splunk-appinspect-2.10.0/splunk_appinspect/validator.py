...
from splunk_appinspect.python_analyzer.trustedlibs.trusted_libs_manager import (
    TrustedLibsManager,
)
from . import infra
...
class Validator(object):
    ...
    def __init__(
        ...
        if app_class is None:
            ...
            logger_output = (
                "The custom app_class '{}' was provided to the"
        ...
        logger.info(
            "Executing checks using Splunk AppInspect version %s",
            self.appinspect_version,
        )
        self.args["appinspect_version"] = self.appinspect_version
        self.app_names = self.app_package_handler.apps.keys()
        self.validation_report = ValidationReport()

    def __emit_event(self, eventname, _, *args):
    ...
    def packaging_groups(self):
        ...
        consolidated_groups = {}
        packaging_grps = splunk_appinspect.checks.groups(
            included_tags=[PACKAGING_STANDARDS_TAG]
        )
    ...
    def validation_groups(self):
        ...
        for grp in self.groups_to_validate:
            selected_chks = list(grp.checks(excluded_tags=[PACKAGING_STANDARDS_TAG]))
            custom_group = Group(grp.module, selected_chks, grp.report_display_order)
    ...
    def validate(self):
        ...
        try:
            self.__emit_event(
                "start_package_preparation", self.listeners, self.app_package_handler
            )

            python_analyzer_enable = False
            for group in self.validation_groups:
                if group.has_checks(included_tags=["ast"]):
            ...
            ) = infra.refine_tag_set(
                self.args["included_tags"], self.args["excluded_tags"]
            )
            splunk_args[
                "target_splunk_version"
            ] = splunk_appinspect.splunk.TargetSplunkVersion(
                splunk_args["included_tags"]
            )

            self.__emit_event(
                "finish_package_preparation", self.listeners, self.app_package_handler
            )
            ...
            with self.resource_manager.context(splunk_args) as context:
                for app in apps:
                    ...
                    if PACKAGING_STANDARDS_TAG not in self.args["excluded_tags"]:
                        self.__emit_event(
                            "start_package_validation", self.listeners, app
                        )
                        packaging_results = self.__run_checks(
                            app, context, self.packaging_groups
                        )
                        application_validation_report.results = packaging_results
                        self.__emit_event(
                            "finish_package_validation", self.listeners, app
                        )
                    ...
                    if application_validation_report.has_invalid_package:
                        ...
                        for grp, check, rpt in skipped_results:
                            application_validation_report.results.append(
                                (grp, check, rpt)
                            )
                    else:
                        if self.validation_groups:
                            self.__emit_event(
                                "start_app_validation", self.listeners, app
                            )
                            validation_results = self.__run_checks(
                                app, context, self.validation_groups
                            )
                            ...
                            self.__emit_event(
                                "finish_app_validation", self.listeners, app
                            )

                    application_validation_report.validation_completed()
                    self.__emit_event(
                        "finish_app", self.listeners, app, application_validation_report
                    )
                    self.validation_report.add_application_validation_report(
    ...
    def __dispatch_check(self, ready_for_deferred, threadpool, context, app, check):
        if check.deferred:
            return ready_for_deferred.then(
                lambda _: threadpool.submit(self.__execute_check, context, app, check)
            )
    ...
    def __run_checks(self, app, context, groups):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as threadpool:
            ...
            for group in groups:
                logger.debug(
                    (
                        "Executing start_group event for"
                        " Group: %s"
                        " Group_Checks: %s"
                        " Listeners: %s"
                    ),
                ...
                self.__emit_event(
                    "start_dispatching_group", self.listeners, group, group.checks()
                )
                ...
                future_checks = list(
                    map(
                        lambda check: (
                            check,
                            self.__dispatch_check(
                                ready_for_deferred, threadpool, context, app, check
                            ),
                        ),
                        group.checks(),
                    )
                ...
                self.__emit_event(
                    "finish_dispatching_group", self.listeners, group, group.checks()
                )

                logger.debug(
                    (
                        "Executing finish_dispatching_group event for"
                        " Group: %s"
                        " Group_Checks: %s"
                        " Listeners: %s"
                    ),
