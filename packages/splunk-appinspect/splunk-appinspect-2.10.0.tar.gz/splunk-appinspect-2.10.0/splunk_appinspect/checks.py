    ...
    for check_dir in directory_paths:
        ...
        for directory_path, _, file_names in os.walk(check_dir):
            ...
            for file in [
                ...
                if re.match(file_pattern_regex, file_name)
            ]:
...
def generate_group(
    ...
    filtered_checks = [
        ...
        if (
            ...
            and check.matches_version(version)
        )
    ]
    ...
    for check in check_list:
        logger_output = ("check_name:{},matches_tags:{},matches_version:{}").format(
            check.name,
            check.matches_tags(included_tags, excluded_tags),
...
def groups(
    ...
    groups_ordered_by_report_display_order = sorted(
        groups_to_return, key=operator.attrgetter("report_display_order")
    )
...
class Group(object):
    ...
    def __init__(
        self, module, checks=None, report_display_order=None, custom_group=False
    ):
    ...
    def doc_name_human_readable(self):
        ...
        html = markdown.markdown(
            self.doc_raw(), extensions=["markdown.extensions.fenced_code"]
        )
    ...
    def count_total_dynamic_checks(
        self,
        included_tags=None,
        excluded_tags=None,
        version=None,
    ...
    def count_total_standalone_checks(
        self,
        included_tags=None,
        excluded_tags=None,
        version=None,
    ...
    def count_total_cluster_checks(
        self,
        included_tags=None,
        excluded_tags=None,
        version=None,
    ...
    def checks(
        ...
        ordered_checks = sorted(
            check_list, key=operator.attrgetter("report_display_order")
        )

        for check in ordered_checks:
            should_check_be_returned = check.matches_tags(
                included_tags, excluded_tags
            ) and check.matches_version(version)
            logger_output = (
                "check_name:{},matches_tags:{},matches_version:{},should_check_be_returned:{}"
            ).format(
                check.name,
                check.matches_tags(included_tags, excluded_tags),
                check.matches_version(version),
...
class Check(object):
    ...
    def is_cluster_check(self):
        ...
        # TODO: tests needed
        if six.PY2:
            spec = inspect.getargspec(self.fun)  # pylint: disable=W1505
        else:
            spec = inspect.getfullargspec(self.fun)
    ...
    def matches_version(self, version_to_match):
        ...
        if hasattr(self.fun, "min_version"):
            return version_to_match >= self.fun.min_version and (
                self.fun.max_version is None or version_to_match <= self.fun.max_version
            )
    ...
    def matches_tags(self, included_tags, excluded_tags):
        ...
        included_tags_set, excluded_tags_set = splunk_appinspect.infra.refine_tag_set(
            included_tags, excluded_tags
        )
        ...
        if included_tags_set and excluded_tags_set:
            ...
            return not check_tags_set.isdisjoint(
                included_tags_set
    ...
    def run(self, app, resource_manager_context=None):
        ...
        try:
            ...
            args = []
            if six.PY2:
                function_arguments = inspect.getargspec(
                    self.fun
                ).args  # pylint: disable=W1505
            else:
            ...
            for arg in function_arguments:
                if arg in available_args:
                    ...
                    # TODO: tests needed
                    logging.debug(
                        "Getting resource: '%s' for %s", arg, self.fun.__name__
                    )

                    rm_ctx = resource_manager_context[arg]
                    if hasattr(rm_ctx, "state") and rm_ctx.state != 0:
                        error_string = (
                            "{} has been skipped because the specified"
                            " resource {} provided could not be setup correctly."
                        ).format(self.fun.__name__, rm_ctx.__class__.__name__)
                    ...
                    hasattr(resource_manager_context, "context")
                    and arg in resource_manager_context.context
                ):
                    logging.debug(
                        "Getting argument: '%s' for %s", arg, self.fun.__name__
                    )
                    ...
                    error_string = (
                        ...
                        " Instances provided: {}."
                        ""
                    ).format(self.fun.__name__, resource_manager_context.keys())
                    raise ResourceUnavailableException(error_string)
            ...
            reporter.fail(
                "{} is malformed. Details: {}".format(e.file_name, str(e)),
                e.file_name,
                e.line_no,
            )
        except:
