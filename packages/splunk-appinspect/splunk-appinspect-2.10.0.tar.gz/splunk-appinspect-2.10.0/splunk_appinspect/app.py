...
from . import authorize_configuration_file
from . import custom_commands
from . import custom_visualizations
from . import configuration_file
from . import configuration_parser
from . import distsearch_configuration_file
...
class App(object):
    ...
    def __init__(
        ...
        if location is None and package is None:
            logger_output = (
                "splunk_appinspect.App requires either a"
        ...
        self.base_bin_dirs = [
            os.path.relpath(path, self.app_dir)
        ...
        if self.python_analyzer_enable:
            try:
                ...
                self._app_temp_dir = os.path.join(
                    tempfile.mkdtemp(), os.path.basename(self.package.working_app_path)
                )
    ...
    def python_analyzer_client(self):
        if not self.python_analyzer_enable:
            raise Exception(
                "Python analyzer is disabled. To enable, please run checks including ast tag."
            )
    ...
    def get_config(self, name, dir="default", config_file=None):
        ...
        if not self.app_conf_files.get(conf_file_key):
            log_output = (
                "'{}' called '{}' to retrieve the configuration file '{}'"
                " at directory '{}'. App filepath: {}"
            ).format(__file__, "get_config", name, dir, app_filepath)
    ...
    def get_meta(self, name, directory="metadata", meta_file=None):
        ...
        log_output = (
            "'{}' called '{}' to retrieve the configuration file '{}'"
            " at directory '{}'. App filepath: {}"
        ).format(__file__, "get_config", name, directory, app_filepath)
        logger.debug(log_output)
        if not self.file_exists(app_filepath):
        ...
        with open(app_filepath, "rb") as file:
            meta_file = configuration_parser.parse(
                file, meta_file, configuration_parser.configuration_lexer
            )
    ...
    def get_raw_conf(self, name, dir="default"):
        ...
        log_output = (
            "'{}' called '{}' to retrieve the configuration file '{}'"
            " at directory '{}'. App filepath: {}"
        ).format(__file__, "get_raw_conf", name, dir, app_filepath)
        logger.debug(log_output)
    ...
    def _get_app_info(self, stanza, option, app_conf_dir="default"):
        ...
        try:
            ...
            logger_output = ("The `app.conf` file does not exist." " Error: {}").format(
                error_message
            )
            logger.error(logger_output)
            property_to_return = "[MISSING `{}/app.conf`]".format(app_conf_dir)
            raise exception
            ...
            logger.error(logger_output)
            property_to_return = ("[MISSING `{}/app.conf` stanza `{}`]").format(
                app_conf_dir, stanza
            )
            raise exception
            ...
            logger.error(logger_output)
            property_to_return = (
                "[MISSING `{}/app.conf` stanza [{}]" " property `{}`]"
            ).format(app_conf_dir, stanza, option)
            raise exception
                ...
                if current_depth <= recurse_depth:
                    for file in files:
                        ...
                        if (
                            ...
                            if not self._filter_by_trusted_libs(
                                dir_in_app, file, check_name
                            ):
    ...
    def get_filepaths_of_files(
        self, basedir="", excluded_dirs=None, filenames=None, types=None
    ):
        excluded_dirs = excluded_dirs or []
    ...
    def file_exists(self, *path_parts):
        ...
        log_output = (
            "'{}.{}' was called. File path being checked:'{}'." " Does File Exist:{}"
        ).format(__file__, "file_exists", file_path, does_file_exist)
        logger.debug(log_output)
    ...
    def directory_exists(self, *path_parts):
        ...
        log_output = (
            "'{}.{} was called.'. Directory path being checked:'{}'."
            " Does Directory Exist:{}"
        ).format(__file__, "directory_exists", directory_path, does_file_exist)
        logger.debug(log_output)
        return does_file_exist
    ...
    def search_for_patterns(
        ...
        for dir, filename, ext in files_iterator:
            ...
            file_to_inspect = inspected_file.InspectedFile.factory(
                os.path.join(self.app_dir, dir, filename)
            )
            ...
            for (fileref_output, file_match) in found_matches:
                ...
                relative_file_ref_output = "{}:{}".format(
                    relative_filepath, line_number
                )
                matches_with_relative_path.append(
                    (relative_file_ref_output, file_match)
                )
    ...
    def search_for_crossline_patterns(
        ...
        for directory, filename, _ in files_iterator:
            ...
            file_to_inspect = inspected_file.InspectedFile.factory(
                os.path.join(self.app_dir, directory, filename)
            )
            found_matches = file_to_inspect.search_for_crossline_patterns(
                patterns=patterns, cross_line=cross_line
            )
            matches_with_relative_path = []
            for (fileref_output, file_match) in found_matches:
    ...
    def is_text(self, filename):
        ...
        if filename in self.info_from_file:
            return bool(
                re.search(r".* text", self.info_from_file[filename], re.IGNORECASE)
            )
        try:
    ...
    def app_icon(self):
        return file_resource.FileResource(
            os.path.join(self.app_dir, "appserver/static/appIcon.png")
        )

    def setup_xml(self):
        return file_resource.FileResource(
            os.path.join(self.app_dir, "default/setup.xml")
        )
        ...
        try:
            ...
            if os.access(filepath, os.F_OK) and os.access(filepath, os.R_OK):
                with open(filepath, "rb") as f:
                    return self._trusted_libs_manager.check_if_lib_is_trusted(
