...
def display_timespan(display_handler, timespan_handler):
    global CANCEL_SCRIPT
    if CANCEL_SCRIPT:
        return
    timespan = timespan_handler.get_elapsed_time()
    display_handler.timedelta_to_display(timespan)
    Timer(1, display_timespan, [display_handler, timespan_handler]).start()
...
def main():
    ...
    try:
        ...
        display_timespan(display_handler, timespan_handler)
        pause()
