...
class Board:
  ...
  def begin(self):
    ...
    if self.boardname in ["RPI","NEZHA","WIN"]:
      self.connected = True
      return self
    if self.connected:
      return
...
#          port_list_2 = [x[0:2] for x in port_list_2]
  ...
  def detect_firmata(self):
    ...
    if len(res) < 3:
      ...
      minor=0
    if (major,minor) == (2,6):
      if self.boardname in ["HANDPY" , "MICROBIT", "GD32V"]:
        ser.read(ser.in_waiting)
