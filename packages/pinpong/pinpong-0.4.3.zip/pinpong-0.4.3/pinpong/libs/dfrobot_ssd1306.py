...
class SSD1306(object):
    ...
    def __init__(self, width, height, external_vcc):
        ...
        self.framebuf = FRAME_BUF(self.buffer, self.width, self.height, 'MVLSB')
        self.power_on()
        self.init_display()
        self.set_flush_mode(self.FULL)

    def init_display(self):
        count = 0
        for cmd in (
            SSD1306_DISPLAYOFF, #关显示
            SSD1306_SETDISPLAYCLOCKDIV, #设置显示时钟分频、振荡器频率
            0xF0,  # Increase speed of the display max ~96Hz
            SSD1306_SETMULTIPLEX, #duty设置
            0x3F,                 #duty=1/64
            SSD1306_SETDISPLAYOFFSET,#显示偏移
            0x00,                    #不偏移
            SSD1306_SETSTARTLINE,
            SSD1306_CHARGEPUMP,
            0x14,
            SSD1306_MEMORYMODE,#设置内存地址模式
            0x00,#水平地址模式
            SSD1306_SEGREMAP,
            SSD1306_COMSCANINC,
            SSD1306_SETCOMPINS,
...
#        print(self.buffer)
    ...
    def set_font(self,font,width,height,kerning=None,rowledge=None):
        if(kerning==None)&(rowledge!=None):
          self.framebuf.set_font(font,width,height,0,rowledge)
        elif(rowledge==None)&(kerning!=None):
          self.framebuf.set_font(font,width,height,kerning,0)
        elif(rowledge==None)&(kerning==None):
          self.framebuf.set_font(font,width,height,0,0)
        else:
    ...
    def rect(self, x, y, w, h, color):
        self.framebuf.rect(x, y, w, h, color, self)

    def fill_rect(self, x, y, w, h, color):
        self.framebuf.fill_rect(x, y, w, h, color,self)

    def circle(self, x, y, radius, color):
        self.framebuf.circle(x, y, radius, color, self)

    def fill_circle(self, x, y, radius, color):
        self.framebuf.fill_circle(x, y, radius, color, self)

    def text(self, string="Hello World!", x=20, y=22, color=1, word_wrap=True):
        self.framebuf.text(str(string), x, y, color, word_wrap, self)
...
class SSD1306_I2C(SSD1306):
  ...
  def write_data(self, buf):
    ...
    while count < len(buf):
      temp = bytearray()
      temp.append(0x40)
      temp.extend(buf[count:count+16])
      count += 16
