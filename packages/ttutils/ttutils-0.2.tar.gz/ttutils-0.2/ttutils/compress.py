...
from typing import List, Dict, Iterable, Generator, Optional, MutableSequence
...
def breaking_range(values: MutableSequence[int]) -> Generator[int, None, None]:
    ...
    while True:
        try:
            _cur = values.pop(0)
            if _cur is None:
                _min, _max = cur + 1, values.pop(0) + 1
                if abs(_max - _min) > RANGE_LIMIT:
                    raise ValueError('Too long range')
                yield from range(_min, _max)
            else:
                cur = _cur
                yield cur
