...
def create_report(file_name='FT_validation_report.txt'):
    report = os.path.expanduser(f'~\\Desktop\\{file_name}')
    r = open(report, mode='wt')
    return r
...
def bundle(ctx):
    ...
    print()
    ug = click.prompt("Please enter the number of the update group you want to list the bundles for, from the list above: ",
    ...
    for b in first_five_bundles:
        list_top_five_bundles.append(b[0])
    ...
    for x, y in enumerate_package_dict.items():
        print(f'{x}. {y}')
    package = click.prompt("Please enter the number next to the type of package you are looking for from the following list: ",
                        ...
                        if full_or_delta == 'Delta':
                            if click.confirm("Do you want to download previous versions full installer?", abort=True):
                                ...
                                if delta_cmp[0]["Url"]:
                                    ...
                                    delta_package_name = download_url.split('/')[-1]
                                    delta_package_name = packagename.split('.')[0]
                                    save_to_desktop(delta_download_url,
...
def create(ctx, duration, apply, vouchertype):
