...
def info(G):
    ...
    print("Time (G_info):", round(stop-start, 3),"sec")      
...
def LCC_analysis(G):
    ...
    stop = timeit.default_timer()
    print("Time (LCC):", round(stop-start, 3),"sec")      

def demo():
    G = Graph()
    f_node = pkg_resources.resource_stream(__name__, "/nodes_16007.csv")
    reader_node = f_node.readline()
    nodes = []
    while reader_node:
        nodes.append(int(reader_node))
        reader_node = f_node.readline()
    G.add_vertices(nodes)

    f_edge = pkg_resources.resource_stream(__name__, "/links_16007.csv")
    reader_edge = f_edge.readline()
    edges = []
    while reader_edge:
        node_set = reader_edge.split(b',')
        #current_edge = []
        edges.append((int(node_set[0]), int(node_set[1])))
        #print(reader, int(node_set[0]), int(node_set[1]))
        reader_edge = f_edge.readline()
    G.add_edges(edges)
    info(G)
    LCC_analysis(G)
