...
def addOptions(parser):
    ...
    group.add_argument('--scinames',   # TODO seq/elt/line???
                       action="store_true", 
                       dest="annotate:scinames",
                       default=False,
                       help="")
...
def run(config):
    ...
    if config['annotate']['scinames']:
        ...
        for seq in o_view:
            names = []
            #if seq['BEST_MATCH_TAXIDS'] is not None:
            #    taxids = seq['BEST_MATCH_TAXIDS']
            #    for i in range(len(seq['BEST_MATCH_TAXIDS'])):
            #        taxid = taxids[i]
            #        genustaxid = taxo.get_taxon_at_rank(taxid, 'genus')
            #        if genustaxid is not None : #and 'sp.' in tostr(taxo.get_scientific_name(taxid)):
            #            names.append(taxo.get_scientific_name(genustaxid))
            #        else:
            #            names.append(taxo.get_scientific_name(taxid))
            
            taxid = seq['TAXID']
            names = [taxo.get_scientific_name(taxid)]
            
            #names = [sn for sn in [taxo.get_scientific_name(taxo.get_taxon_at_rank(taxid, 'genus')) for taxid in seq['BEST_MATCH_TAXIDS']]]
            #print(type(names[0]), names)
            seq['sc_names'] = names
            
            # Tag chloroplasts/host
            seq['host'] = False
            for name in names:
                if taxo.is_ancestor(taxo.get_taxon_by_name('Viridiplantae').taxid, taxo.get_taxon_by_name(name).taxid):
                    seq['host'] = True
                    break
            ...
            names = [b' '.join(name.split(b' ')[:2]) for name in names]
            #print(names)
            
            derep_names = []
            for i in range(len(names)):
                name = tostr(names[i])
                if name not in derep_names:
                    derep_names.append(name)
            
            if 'sample' in seq.keys():
                sample = seq['sample']
            else:
                sample = 'sample'
            if sample not in namesdict:
                namesdict[sample] = {}
            for name in derep_names:
                if name not in namesdict[sample]:
                    namesdict[sample][name] = {}
                    namesdict[sample][name]['COUNT'] = seq['COUNT']
                    if b'NORM_COUNT' in seq.keys():
                        namesdict[sample][name]['NORM_COUNT'] = seq['NORM_COUNT']
                    namesdict[sample][name]['BEST_IDENTITY'] = [seq['BEST_IDENTITY']]
                    if b'ESTIMATED_GENOME_SIZE' in seq.keys():
                        namesdict[sample][name]['ESTIMATED_GENOME_SIZE'] = [seq['ESTIMATED_GENOME_SIZE']]
                    if b'CONFIDENCE_INTERVAL' in seq.keys():
                        namesdict[sample][name]['CONFIDENCE_INTERVAL'] = [seq['CONFIDENCE_INTERVAL']]
                else:
                    namesdict[sample][name]['COUNT'] += seq['COUNT']
                    if b'NORM_COUNT' in seq.keys():
                        namesdict[sample][name]['NORM_COUNT'] += seq['NORM_COUNT']
                    namesdict[sample][name]['BEST_IDENTITY'].append(seq['BEST_IDENTITY'])
                    if b'ESTIMATED_GENOME_SIZE' in seq.keys():
                        namesdict[sample][name]['ESTIMATED_GENOME_SIZE'].append(seq['ESTIMATED_GENOME_SIZE'])
                    if b'CONFIDENCE_INTERVAL' in seq.keys():
                        namesdict[sample][name]['CONFIDENCE_INTERVAL'].append(seq['CONFIDENCE_INTERVAL'])
            ...
            #     for name in derep_names:
            #         toprint = tostr(name)
            #         toprint += '\t'+tostr(seq['sample'])
            #         toprint += '\t'+str(seq['BEST_IDENTITY'])
            #         toprint += '\t'+str(seq['ESTIMATED_GENOME_SIZE'])
            #         toprint += '\t'+str(seq['CONFIDENCE_INTERVAL'])
            #         print(toprint)
        
        for sample in namesdict.keys():
            for name in namesdict[sample]:
                toprint = tostr(name)
                toprint += '\t'+tostr(sample)
                #toprint += '\t'+str(namesdict[sample][name]['COUNT'])
                if 'NORM_COUNT' in namesdict[sample][name]:
                    toprint += '\t'+str(namesdict[sample][name]['NORM_COUNT'])
                toprint += '\t'+str(statistics.mean(namesdict[sample][name]['BEST_IDENTITY']))
                if 'ESTIMATED_GENOME_SIZE' in namesdict[sample][name]:
                    toprint += '\t'+str(statistics.mean(namesdict[sample][name]['ESTIMATED_GENOME_SIZE']))
                if 'CONFIDENCE_INTERVAL' in namesdict[sample][name]:
                    if len(list(filter(None, namesdict[sample][name]['CONFIDENCE_INTERVAL']))) >= 2:
                        toprint += '\t'+str(statistics.mean(list(filter(None, namesdict[sample][name]['CONFIDENCE_INTERVAL'])))) # TODO None issue
                    else:
                        toprint += '\tNA'
                print(toprint)
