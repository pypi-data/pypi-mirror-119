...
LUNAR_CHANNELS_API_URL = "/v1/channels"
...
class ChannelClient(LunarClient):
    """
    Client for Lunar Recommendation API (`/v1/channels/`).
    ...
    def __init__(self, config: Config):
        ...
        self.api_url = LUNAR_CHANNELS_API_URL if config.ENV == "LOCAL" else f"/api{LUNAR_CHANNELS_API_URL}"
    ...
    async def create_channel_async(self, id: str, experiment_id: str = None) -> Channel:
        ...
        body = await self._request(method="POST", url=self.api_url, data=channel.dict())
    ...
    async def update_channel_async(self, id: str, experiment_id: str = None) -> Channel:
        ...
        body = await self._request(method="PUT", url=f"{self.api_url}/{id}", data=updated_channel.dict())
        return Channel(**body["data"])
    ...
    async def update_channel_partial_async(self, id: str, experiment_id: str = None) -> Channel:
        ...
        body = await self._request(
            method="PATCH", url=f"{self.api_url}/{id}", data=patched_channel.dict(exclude_none=True)
        )
        return Channel(**body["data"])
