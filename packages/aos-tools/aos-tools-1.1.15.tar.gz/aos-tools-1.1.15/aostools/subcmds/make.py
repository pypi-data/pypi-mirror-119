            ...
            if opt.board:
                if ext_scons_existed:
                    if params_str.find("V=1") >= 0:
                        result = os.system("scons --verbose --board=%s %s -j4" % (opt.board, params_str)) >> 8
                    else:
                        result = os.system("scons --board=%s %s -j4" % (opt.board, params_str)) >> 8
                else:
                    result = os.system("make BOARD=%s %s" % (opt.board, params_str)) >> 8
            else:
                if ext_scons_existed:
                    if params_str.find("V=1") >= 0:
                        result = os.system("scons --verbose %s -j4" % params_str) >> 8
                    else:
                        result = os.system("scons %s -j4" % params_str) >> 8
                else:
