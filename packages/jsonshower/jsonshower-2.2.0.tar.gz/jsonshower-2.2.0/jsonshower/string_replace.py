...
def fuzzy_match(needle, hay, flags=None, needle_length_window=0.2):
    ...
    max_sim_val    = 0
    max_sim_string = u""
    for ngram in ngrams(hay.split(), needle_length + int(needle_length_window*needle_length)):
        hay_ngram = u" ".join(ngram)
        similarity = SM(None, hay_ngram, needle).ratio() 
        if similarity > max_sim_val:
            max_sim_val = similarity
            max_sim_string = hay_ngram 
    return re.search(max_sim_string, hay, flags=flags)

def get_start_end_index(achievement, rel_string, case_insensitive=False):
    rel_string = rel_string.strip()
    if case_insensitive:
        matched = fuzzy_match(rel_string, achievement, flags=re.IGNORECASE | re.MULTILINE)
    else:
