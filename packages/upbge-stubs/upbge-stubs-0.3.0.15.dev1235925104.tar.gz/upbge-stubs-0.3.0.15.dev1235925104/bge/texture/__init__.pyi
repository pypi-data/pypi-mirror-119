...
:func:`materialID <bge.texture.materialID>` function, then you create a new texture with dynamic content
...
class ImageMirror:
  ...
  def __init__(self, scene: bge.types.KX_Scene, observer: bge.types.KX_GameObject, mirror: bge.types.KX_GameObject, material: int = 0, width: int = None, height: int = None, samples: int = None) -> None:
    ...
    :type scene:      
      :class:`KX_Scene <bge.types.KX_Scene>`
...
(the object from which the mirror has to be looked at, for example a camera).
    ...
    :type observer:   
      :class:`KX_GameObject <bge.types.KX_GameObject>`
    ...
    :arg width:       
      Off-screen render buffer width (optional).

    :type width:      
      integer

    :arg height:      
      Off-screen render buffer height (optional).
  ...
  Deprecated since version 0.2.2: Use :attr:`bge.texture.ImageMirror.horizon` or :attr:`bge.texture.ImageMirror.zenith` instead.
...
class ImageMix:
  ...
  def getSource(self, id: str) -> typing.Any:
...
otherwise on the default framebuffer.
  ...
  def __init__(self, scene: bge.types.KX_Scene, camera: bge.types.KX_Camera, width: int, height: int, samples: int) -> None:
    ...
    :type camera:     
      :class:`KX_Camera <bge.types.KX_Camera>`
...
This only makes sense with offscreen render on texture target (see :func:`offScreenCreate <bge.render.offScreenCreate>`).
...
To render from a non active camera see :class:`ImageRender <bge.texture.ImageRender>`.
...
provide the texture for both materials/textures.(optional)
    ...
    :type textureObj: 
      :class:`Texture <bge.texture.Texture>`
...
The format of the string is ``<displayMode>[/3D]``.
  ...
  Refer to :class:`VideoDeckLink <bge.texture.VideoDeckLink>` to get the list of acceptable ``<displayMode>``.
...
class FilterRGB24:
  ...
  Returns a new input filter object to be used with :class:`ImageBuff <bge.texture.ImageBuff>` object when the image passed
to the :meth:`load <bge.texture.ImageBuff.load>` function has the 3-bytes pixel format BGR.
...
def imageToArray(image: typing.Any, mode: str) -> bgl.Buffer:
  ...
  Returns a :class:`Buffer <bgl.Buffer>` corresponding to the current image stored in a texture source object.
...
def materialID(object: bge.types.KX_GameObject, name: str) -> int:
  ...
  Returns a numeric value that can be used in :class:`Texture <bge.texture.Texture>` to create a dynamic texture.
