...
class S3ObjectStoreTest(TypedObject):
    """
    *(extends* :py:class:`delphixpy.web.vo.TypedObject` *)* An Amazon S3 object
    store connectivity test object.
    """
    def __init__(self, undef_enabled=True):
        super(S3ObjectStoreTest, self).__init__()
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        obj = super(S3ObjectStoreTest, cls).from_dict(data, dirty, undef_enabled)
        ...
        if obj._region[0] is not None and obj._region[0] is not obj.__undef__:
            assert isinstance(obj._region[0], TEXT_TYPE), ("Expected one of ['string'], but got %s of type %s" % (obj._region[0], type(obj._region[0])))
        ...
        if obj._bucket[0] is not None and obj._bucket[0] is not obj.__undef__:
            assert isinstance(obj._bucket[0], TEXT_TYPE), ("Expected one of ['string'], but got %s of type %s" % (obj._bucket[0], type(obj._bucket[0])))
    ...
    def to_dict(self, dirty=False, belongs_to_parent=False):
        dct = super(S3ObjectStoreTest, self).to_dict(dirty, belongs_to_parent)
        ...
        if "region" == "type" or (self.region is not self.__undef__ and (not (dirty and not self._region[1]) or isinstance(self.region, list) or belongs_to_parent)):
            dct["region"] = dictify(self.region)
        if "bucket" == "type" or (self.bucket is not self.__undef__ and (not (dirty and not self._bucket[1]) or isinstance(self.bucket, list) or belongs_to_parent)):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, S3ObjectStoreTest):
            return False
        return super(S3ObjectStoreTest, self).__eq__(other) and \
