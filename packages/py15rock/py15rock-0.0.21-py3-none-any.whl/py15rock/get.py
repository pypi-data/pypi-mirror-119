...
def requestCompanyReturning(ticker, endpoint):
    r = httpx.get(f'{config.api_endpoint}/company/{ticker}/{endpoint}', headers={'Authorization': f'{config.api_key}'})
    df = pd.read_json(r.content)
    return df

#api calls for endpoints that require years 
def requestCompanyReturningYears(ticker, years, endpoint, token):
    r = httpx.get(f'{config.api_endpoint}/company/{ticker}/{endpoint}/{years}', headers={'Authorization': f'{config.api_key}'})
    df = pd.read_json(r.content)
    return df

#api call for fund level analytics
def requestFundAnalytics(fund_ticker, endpoint):
    r = httpx.get(f'{config.api_endpoint}/fund/{fund_ticker}/{endpoint}', headers={'Authorization': f'{config.api_key}'})
    df = pd.read_json(r.content)
    return df

#api call for portflio level analytics
def requestPortflioAnalytics(data, endpoint):
    r = httpx.post(f'{config.api_endpoint}/portfolio/analytics/{endpoint}', headers={'Authorization': f'{config.api_key}'}, data=data)
    df = pd.read_json(r.content)
    return df

#api call for country level analytics
def requestCountryAnalytics(countryName, endpoint):
    r = httpx.get(f'{config.api_endpoint}/country/{countryName}/{endpoint}', headers={'Authorization': f'{config.api_key}'})
    df = pd.read_json(r.content)
    return df
