...
class DicsriminatorTail(nn.Module):
    def __init__(
        self, nf_mult, n_layers, ndf=64, norm_layer=nn.BatchNorm2d, use_parallel=True
    ):
...
class MultiScaleDiscriminator(nn.Module):
    def __init__(
        self, input_nc=3, ndf=64, norm_layer=nn.BatchNorm2d, use_parallel=True
    ):
...
def get_discriminator(model_config):
    ...
    if discriminator_name == "no_gan":
        ...
        model_d = NLayerDiscriminator(
            n_layers=model_config["d_layers"],
            norm_layer=get_norm_layer(norm_type=model_config["norm_layer"]),
            use_sigmoid=False,
        )
        ...
        raise ValueError(
            "Discriminator Network [%s] not recognized." % discriminator_name
        )
