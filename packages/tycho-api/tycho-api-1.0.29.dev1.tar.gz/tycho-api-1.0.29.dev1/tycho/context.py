...
class TychoContext:
    ...
    def get_definition(self, app_id):
        ...
        print(f"APP DEFINITION =======================================> {app_definition}")
        if not app_definition:
            try:
                logger.debug (f"-- resolving definition for {app_id}")
                print(f"APP ID EVERYTHING ==============================> {self.apps[app_id]}")
    ...
    def start (self, principal, app_id, resource_request):
        ...
        """Instance count per app"""
        count = self.apps.get(app_id).get("count", 1)
        spec["services"][app_id]["count"] = count
