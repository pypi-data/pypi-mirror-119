...
class GlueKernel(IPythonKernel):
    ...
    async def do_execute(self, code: str, silent: bool, store_history=True, user_expressions=None,
        ...
        try:
            if not self.glue_client:
                ...
                self.glue_client = self.authenticate(glue_role_arn=self.get_glue_role_arn(),
        ...
        try:
            ...
            try:
                while time.time() - start_time <= self.time_out:
                    statement = self.glue_client.get_statement(SessionId=self.get_session_id(), Id=statement_id)[
                        "Statement"]
    ...
    def _authenticate_with_profile(self):
        ...
        self.sts_client = boto3.Session(profile_name=self.get_profile()).client('sts',
                                                                                region_name=self.get_region())
    ...
    def delete_session(self):
        if self.get_session_id():
            try:
                ...
                self.ipython_display.send_error(
                    f'Exception encountered while deleting session {self.get_session_id()}: {e} \n')
                self._print_traceback(e)
    ...
    def _cancel_statement(self, statement_id: str):
        ...
        try:
            ...
            while time.time() - start_time <= self.time_out and not is_ready:
                status = \
                self.glue_client.get_statement(SessionId=self.get_session_id(), Id=statement_id)["Statement"][
                    "State"]
    ...
    async def _execute_cell(self, code, silent, store_history=True, user_expressions=None, allow_stdin=False):
        reply_content = await self._execute_cell_for_user(code, silent, store_history, user_expressions,
                                                          allow_stdin)
        return reply_content

    async def _execute_cell_for_user(self, code, silent, store_history=True, user_expressions=None,
                                     allow_stdin=False):
        result = await super(GlueKernel, self).do_execute(code, silent, store_history, user_expressions,
                                                          allow_stdin)
