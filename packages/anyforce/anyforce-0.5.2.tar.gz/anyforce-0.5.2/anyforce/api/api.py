...
class API(Generic[UserModel, Model, CreateForm, UpdateForm]):
    ...
    def translate_condition(self, user: UserModel, k: str, v: Any) -> Any:
        if isinstance(v, List):
            rs: List[Union[str, float]] = []
            for e in cast(List[Any], v):
                r = self.translate_condition(user, k, e)
                ...
                if condition:
                    for raw in condition:
                        ...
                        for k, v in kv.items():
                            ...
                            v = self.translate_condition(current_user, k, v)
                            if isinstance(v, Dict):
                                q_kvs.update(cast(Dict[str, Any], v))
                            else:
                                q_kvs[k] = v
                        condition_q = Q(
                            **q_kvs,
