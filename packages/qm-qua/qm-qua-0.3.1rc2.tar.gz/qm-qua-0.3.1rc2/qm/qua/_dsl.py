...
def update_frequency(element, new_frequency, units="Hz", keep_phase=False):
    ...
    The behavior of the phase (continuous vs. change only frequency) is controlled by the ``keep_phase`` parameter and
    is discussed in :ref:`Frequency and phase transformations`.

    .. note::

        Starting from version 0.6, this statement occurs in parallel with pulse playing, and the latency
        associated with ``update_frequency()`` depends on the duration of the play/wait statement that is played
        in parallel to it.

    :param element: The quantum element associated with the oscillator whose frequency will be changed
    :type element: str
    :param new_frequency: The new frequency value to set in units set by ``units`` parameter. Range: (0 to 5000000) in steps of 1.
    :type new_frequency: int
    :param units: units of new frequency. Useful when sub-Hz precision is required.
    :type units: str
    :param keep_phase: Determine whether phase will be continuous through the change (if ``True``) or only frequency will change (if ``False``).
...
def set_dc_offset(element, input_reference, offset):
    ...
    :param offset: offsets to set - one offset for a single element and up to two
    offsets for a mixed element using the following template (I dc offset, Q dc offset)
...
def declare(t, **kwargs):
    ...
    prog = scope.program()
    if t == int:
        prog.declare_int(var, memsize, value, dim)
    elif t == bool:
        prog.declare_bool(var, memsize, value, dim)
    elif t == float:
        prog.declare_real(var, memsize, value, dim)
    elif t == fixed:
        prog.declare_real(var, memsize, value, dim)
    else:
...
class DualAccumulationMethod(AccumulationMethod):
    ...
    def full(self, iw1, element_output1, iw2, element_output2, target):
        """
        Perform an ordinary dual demodulation/integration. See :ref:`Dual demodulation`.
...
class _Functions(object):
    ...
    @staticmethod
    def demod(frequency, iw_cos, iw_sin):
        """
            demodulating the acquired data from the indicated stream at the frequency
            and using integration weights defined in the function parameters
        :param frequency: frequency for demodulation calculation
        :param iw_cos: cosine integration weight. Integration weight can be either a scalar
                        for constant integration weight, or a python iterable for arbitrary integration weights
        :param iw_sin: sine integration weight. Integration weight can be either a scalar
                        for constant integration weight, or a python iterable for arbitrary integration weights
        :return: stream object
        """
        ...
        if hasattr(iw_sin, "__len__"):
            ...
            iw_sin = str(iw_sin)
        return ["demod", str(frequency), iw_cos, iw_sin]
...
class _ResultStream:
    ...
    def map(self, function) -> "_ResultStream":
        """
        transform the item by applying a function, such as average, to it
