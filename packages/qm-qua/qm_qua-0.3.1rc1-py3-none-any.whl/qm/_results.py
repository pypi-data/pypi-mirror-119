...
from qm._errors import InvalidStreamMetadataError
...
class BaseNamedJobResult:
    ...
    def has_dataloss(self) -> bool:
        ...
        """
        return self._get_job_state().has_dataloss
    ...
    def _get_job_state(self, **kwargs) -> JobStreamingState:
        request = GetJobStateRequest()
        request.jobId = self._job_id
        response = self._service.GetJobState(request)
...
class JobResults:
    ...
    def _get_job_state(self) -> JobStreamingState:
        request = GetJobStateRequest()
        request.jobId = self._job_id
        response = self._service.GetJobState(request)
        return JobStreamingState(
