...
def multiclass_roccurve(C_mod, key_list, target_line,target_label=None):
    ...
    for tl1 in target_label:
        ...
        for count in range(len_tar_l):
            ax = fig.add_subplot(1, len_tar_l, count)
            for count2 in range(len_key_l):
                roc = C_mod.Cal_s.roc_curve(key_list[count2], target_line[count])
                ax.plot(roc[0], roc[1], label=labels[count], color=color_list[count])
            ax.set_title(tl1)
            ax.xlabel('FPR: False positive rate')
            ax.ylabel('TPR: True positive rate')
            ax.grid(True)
