...
def multiclass_class_check(C_mod,key_list, target_line, value_type="number",target_label=None):
    ...
    for tl1 in target_label:
        ...
        for count in range(len_tar_l):
            ...
            c_col = np.full_like(c_col, "", dtype=object)
            for ii in range(max_n):
                for jj in range(max_n):
                    if ii == jj:
                        c_col[ii, jj] = [int(250 * class_data2[ii * max_n + jj]),0,0]
                    else:
                        c_col[ii, jj] = [0,0,int(250 * class_data2[ii * max_n + jj])]



            ax.axis('off')
            ax.table(cellText=cd_df.values, colLabels=cd_df.columns, rowLabels=cd_df.index, loc="center",cellColours=c_col)
            ax_list.append(ax)
        plt.tight_layout()
        plt.show()
