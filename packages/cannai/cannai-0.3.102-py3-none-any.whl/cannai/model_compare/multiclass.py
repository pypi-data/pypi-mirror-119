...
def multiclass_class_check(C_mod,key_list, target_line, value_type="number",target_label=None):
    ...
    for tl1 in target_label:
        ...
        for count in range(len_tar_l):
            ...
            class_datas = []
            for count2 in range(len_key_l):
                class_data, max_n = C_mod.Cal_s.cal_classtering(key_list[count2], target_line, nv)
            ...
            print(cd_df)
            ax.table(cellText=cd_df.values, colLabels=cd_df.columns, rowLabels=cd_df.index, loc="center")
            ax_list.append(ax)
        plt.tight_layout()
        plt.show()
