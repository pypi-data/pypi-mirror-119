...
class Cal_trained_score:
    ...
    def check_binary(self, e_score, ll):
        if e_score in binary_score_list:
            ...
            if type(ll) == pd.core.series.Series: ll2 = ll.map(zero_one)
    ...
    def cal_classtering(self, key, target_line, nv):
        out_l, ans_l = self.get_inout(key, target_line)
        if self.parent.class_type == "m": out_l = self.get_highest_col(out_l)
        elif self.parent.class_type == "b": out_l = out_l.map(over_half)
        ...
        if nv == 1: taisyou_list = [0.0 for aa1 in range(max_n*max_n +1)]
        else: taisyou_list = [0 for aa1 in range(max_n*max_n +1)]

        for ii in range(len_l):
            taisyou_list[out_l[ii] * max_n + ans_l[ii]] += 1

        if nv == 1:
            al_c = ans_l.value_counts()
            al_c.index.astype('str')
            for ii in range(max_n):
                if str(ii) in len_l.index:
                    alc_v = float(al_c[str(ii)])
                    for jj in range(max_n): taisyou_list[ii * max_n + jj] /= alc_v
                else: taisyou_list[ii * max_n + jj] = 0.0

        return taisyou_list, max_n