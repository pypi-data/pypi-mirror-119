...
class S3Oper(object):
    ...
    def _upload_i(self,  remote_file, local_file, retry_count, is_oss_file_to_disk=False):
        """
        @des: 多线程批量上传
        """
        ctx = self.ctx
        ...
        try:
            ...
            oss盘的文件复制到本地
            """
            if is_oss_file_to_disk:
                tmp_data_path = "/tmp/data"
                os.makedirs(tmp_data_path, exist_ok=True)
                #
                disk_local_file = os.path.join(
                    tmp_data_path, f"{uuid.uuid1().hex}{time.time()}")
                shutil.copyfile(local_file, disk_local_file)
            else:
                disk_local_file = local_file
            """
            开始上传
            """
            for i in range(retry_count):  # 最多重试三次，由于网络不稳定等问题
                try:
                    ...
                    _ = s3_client.upload_file(
                        disk_local_file, self.bucket, remote_file)  # 返回值为None
                    _e = time.time()
                    BMOBJ.clog(
            ...
            if is_oss_file_to_disk:
                try:
                    BMOBJ.remove_file(disk_local_file)
                except:
                    pass
        ...
        if subfiles:
            ...
            for subfile in subfiles:
                ...
                task_i = executor.submit(
                    self._upload_i, *(remote_file, local_file, retry_count, is_oss_file_to_disk))
