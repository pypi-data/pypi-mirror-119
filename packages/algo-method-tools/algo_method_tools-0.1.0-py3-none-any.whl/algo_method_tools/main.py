...
def create_main(
        task_no,
        base_dir: Path=Path('~/algo-methods/').expanduser(),
        template_file: Optional[str]=None):
    soup = fetch_task_soup(task_no)
    json_data = load_json(soup)
    lecture_group_id = json_data['props']['pageProps']['tasks']['lecture_group_id']
    ins, outs = compute_ins_and_outs(soup)
    write_task_in_outs(ins, outs, base_dir / str(lecture_group_id) / task_no)
    main_file = base_dir / str(lecture_group_id) / task_no / 'main.py'
    if template_file:
        template_file = Path(template_file).absolut()
        copyfile(template_file, src(main_file))
    else:
