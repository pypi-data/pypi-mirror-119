...
class Likelihood(AnalysisObject):
    ...
    def __init__(self, filename:str, data_name:str='combData', 
        ...
        self.poi_bestfit = None
        
    def evaluate(self, poi_val:float, poi_name:Optional[str]=None, vmin:float=10., vmax:float=10., unconditional=False,
                 hesse:bool=False, minos:bool=True):
        
        self.model.workspace.saveSnapshot("tmp", self.model.workspace.allVars())
        
        # setup the POI
        poi = self.model.get_poi(poi_name)
        poi_min, poi_max = poi_val - abs(vmin), poi_val + abs(vmax)
        poi.setRange(poi_min, poi_max)
        ...
        if minos:
            minimizer_options['minos_set'] = ROOT.RooArgSet(poi)
        
        # perform the fit
        self.minimizer.minimize(**minimizer_options)
        
        self.poi_bestfit = poi.getVal()
        
        self.model.workspace.loadSnapshot("tmp")
        return self.minNll