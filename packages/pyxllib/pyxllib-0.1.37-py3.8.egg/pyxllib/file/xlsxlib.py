...
class EnchantCell(EnchantBase):
    ...
    @RunOnlyOnce
    def enchant(cls):
        names = cls.check_names([openpyxl.cell.cell.Cell, openpyxl.cell.cell.MergedCell])

        for name in names:
            setattr(openpyxl.cell.cell.Cell, name, getattr(cls, name))
            setattr(openpyxl.cell.cell.MergedCell, name, getattr(cls, name))
...
class EnchantWorksheet:
    ...
    def enchant(cls):
        ...
        for name in names:
            setattr(openpyxl.worksheet.worksheet.Worksheet, name, getattr(cls, name))
...
class EnchantWorkbook:
    @classmethod
    @RunOnlyOnce
    def enchant(cls):
        def check_names():
            exist_names = {'openpyxl.Workbook': set(dir(openpyxl.Workbook))}
            names = {x for x in dir(cls) if x[:2] != '__'} - {'enchant'}
        ...
        for name in names:
            setattr(openpyxl.Workbook, name, getattr(cls, name))
