...
class UtoolsText(UtoolsBase):
    ...
    def browser(self):
        ...
        from pyxllib.robot.win32lib import get_win32_app, wd_save_format
        ...
        def _func(type_='html', close=None, visible=None, quit=None):
            ...
            word = get_win32_app('word')
            if visible is not None:
                word.Visible = visible  # 后台运行
            word.DisplayAlerts = 0  # 不显示，不警告

            doc = word.Documents.Add()  # 创建新的word文档
            word.Selection.PasteAndFormat(_c.wdFormatOriginalFormatting)
            # 复杂格式可能无法完美支持所有功能。比如复杂的pdf无法使用SaveAs2实现，要用ExportAsFixedFormat。
            doc.SaveAs2(str(f), wd_save_format(type_))  # 另存为
            if close:
    ...
    def chrome_history(self):
        ...
        def chrome_history(visible=None, drop_duplicates=True, summary=False):
            ...
            # 1 初始化
            from pyxllib.robot.win32lib import get_win32_app
            word = get_win32_app('word', visible=visible)
            doc = word.Documents.Add()
            word.Selection.PasteAndFormat(_c.wdFormatOriginalFormatting)  # 从剪切板粘贴数据到word中

            # 2 遍历word的主要文本内容，可以借助Characters获取content
            # 有特殊换行，ch.Text可能会得到 '\r\x07'，为了位置对应，只记录一个字符
            content = ''.join([ch.Text[0] for ch in doc.Characters])

            # 3 获取content后，就可以在py做正则等各种复杂处理
            ms = list(re.finditer(r'⨯\d{2}:\d{2}:\d{2}', content))
            for m in reversed(ms):
                doc.Characters.Item(m.start() + 1).Select()  # 通过Characters.Item可以找到匹配的内容在文档中的实际range位置
                word.Selection.MoveLeft()  # 这里是找到“⨯21:58:17"的标记
                word.Selection.TypeParagraph()  # 换行，方便后面做解析，信息提取
