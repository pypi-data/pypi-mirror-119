...
class xlpil(EnchantBase):
    ...
    def enchant(cls):
        ...
        # 0 check
        all_names = cls.check_enchant_names([cv2, np.ndarray, PIL.Image, PIL.Image.Image])

        # 1 绑定到模块下的方法
        pil_names = 'read read_from_buffer read_from_url'.split()
        cls._enchant(PIL.Image, pil_names)

        # 2 绑定到PIL.Image.Image下的方法
        # 2.1 属性类
        attrs_names = 'imsize n_channels'.split()
        cls._enchant(PIL.Image.Image, attrs_names, 'property')

        # 2.2 其他均为方法类
        # cls._enchant(PIL.Image.Image, all_names - pil_names - attrs_names)
