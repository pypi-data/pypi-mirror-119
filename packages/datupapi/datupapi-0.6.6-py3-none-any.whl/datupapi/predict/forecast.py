...
class Forecast(Config):
    ...
    def prediction(self,data,models, data_train_list, scalers_list,n_features, n_train):
        ...
        for i in range(self.n_backtests+1):
            ...
            #if params["trdata"]:
            predict_input =data_train_list[i].iloc[:,n_train:].tail(self.n_steps_in)
            #......................................................................
            ...
            intervals[i]=self.predict_with_uncertainty(f=predict_with_dropout, x=input_data, n_iter=self.n_iter, n_steps_out=self.n_steps_out, n_features=n_features)          
            #-----------------------------------------------------------------------------------
            #Make predictions without dropout to compare the results
            #.......................................................................................
            #if params["trdata"]:
            predict_input =predict_input.reshape((1, self.n_steps_in, len(data_train_list[i].iloc[:,n_train:].columns)))
            #else:
        ...
        for i in range(self.n_backtests+1):
            ...
            for j in range(n_features):
                for k in range(self.n_steps_out):
                    ...
                    ci = 0.95
                    p[j*self.n_steps_out+k][1]=(np.quantile(predictions[i][n_features*k+j,:], 0.5+ci/2)-p50)*mult[j][0]
                    p[j*self.n_steps_out+k][2]=(np.quantile(predictions[i][n_features*k+j,:], 0.5-ci/2)-p50)*mult[j][0]              
                    ci=0.6
                    p[j*self.n_steps_out+k][3]=(np.quantile(predictions[i][n_features*k+j,:], 0.5+ci/2)-p50)*mult[j][2]
                    p[j*self.n_steps_out+k][4]=(np.quantile(predictions[i][n_features*k+j,:], 0.5-ci/2)-p50)*mult[j][2]
                    ci=0.8
                    p[j*self.n_steps_out+k][5]=(np.quantile(predictions[i][n_features*k+j,:], 0.5+ci/2)-p50)*mult[j][1]
                    p[j*self.n_steps_out+k][6]=(np.quantile(predictions[i][n_features*k+j,:], 0.5-ci/2)-p50)*mult[j][1] 
    ...
    def predict_with_simulation(self, data,scalers_list, mult=[0.9,1,1.1], item_sim="brentcrudeusd", n_features=1, n_train=1):
        ...
        for j in range(len(mult)):
            ...
            ids= list(itertools.chain.from_iterable(itertools.repeat(x, self.n_steps_out) for x in list_items))

    
            old_values=data_test[item_sim].values[:-self.n_steps_out]
            main_value=old_values[-1]

            new_values[j]=np.repeat(mult[j]*main_value,self.n_steps_out)
            ...
            predict_input =data_test.iloc[:,n_train:].tail(self.n_steps_in)
    
            predict_input =predict_input.to_numpy()[0:self.n_steps_in]
            model=tf.keras.models.load_model('../tmp/model'+str(0)+'.h5')
            predict_input =predict_input.reshape((1, self.n_steps_in, len(data_test.iloc[:,n_train:].columns)))
            predict_out = model.predict(predict_input, verbose=2)
