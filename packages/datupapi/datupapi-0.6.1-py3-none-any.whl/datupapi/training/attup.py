...
class Attup(Config):
    ...
    def training(self, data_m,n_features, n_train, verbose=0):
        ...
        for i in range(self.n_backtests+1):
            ...
            earlyStopping=tf.keras.callbacks.EarlyStopping( monitor='mae', min_delta=0.01, patience=25, verbose=verbose,    mode='auto')
    ...
    def join_related_dataset(self,Qfwd, data_date, data):
        if ("location" in Qfwd.columns) or ("Location" in Qfwd.columns):
            ...
            Qfwd=Qfwd[["timestamp","item_id",Qfwd.columns[2]]]
        Qfwd.columns=["timestamp","item_id","demand"]
        data_date_fwd,_=self.transform_to_matrix(df=Qfwd, value=0)
        data_date_fwd=data_date_fwd.set_index("Date")
        data_date_fwd =data_date_fwd.reindex(sorted(data_date_fwd.columns), axis=1)
        data_date_fwd=data_date_fwd.add_suffix('-fwd')
        data_date_fwd=data_date_fwd.reset_index()
        data_date_fwd.Date=pd.to_datetime(data_date_fwd.Date)
