...
class FusionNet(nn.Module):
    ...
    def forward(self, img0, img1, flow, c0, c1, flow_gt):
        ...
        if flow_gt == None:
            ...
            warped_img0_gt = warp(img0, flow_gt[:, :2], self.device)
            warped_img1_gt = warp(img1, flow_gt[:, 2:4], self.device)
...
class Model:
    def __init__(self, device, local_rank=-1):
        ...
        self.fusionnet = FusionNet(device)
        self.device()
    ...
    def device(self):
        ...
        self.contextnet.to(self.torch_device)
        self.fusionnet.to(self.torch_device)
    ...
    def load_model(self, path, rank):
        ...
        if rank <= 0:
            ...
            self.contextnet.load_state_dict(
                convert(torch.load('{}/contextnet.pkl'.format(path), map_location=self.torch_device)))
            self.fusionnet.load_state_dict(
                convert(torch.load('{}/unet.pkl'.format(path), map_location=self.torch_device)))
