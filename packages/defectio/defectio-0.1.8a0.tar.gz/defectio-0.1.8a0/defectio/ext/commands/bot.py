...
def when_mentioned(bot: Union[Bot], msg: Message) -> List[str]:
...
def when_mentioned_or(
    *prefixes: str,
) -> Callable[[Union[Bot], Message], List[str]]:
...
class BotBase(GroupMixin):
    ...
    async def is_owner(self, user: defectio.User) -> bool:
        ...
        Checks if a :class:`~defectio.User` or :class:`~defectio.Member` is the owner of
        this bot.
    ...
    def add_cog(self, cog: Cog, *, override: bool = False) -> None:
        ...
        if existing is not None:
            if not override:
                raise defectio.ClientException(f"Cog named {cog_name!r} already loaded")
        ...
        if isinstance(prefix, str):
            ...
            try:
                ...
                if message.content.startswith(tuple(prefix)):
                    invoked_prefix = defectio.utils.find(view.skip_string, prefix)
                else:
                    return ctx
...
class Bot(BotBase, defectio.Client):
    """Represents a defectio bot.

    This class is a subclass of :class:`defectio.Client` and as a result
    anything that you can do with a :class:`defectio.Client` you can do with
    this bot.
    ...
    command_prefix
        ...
        as its first parameter and :class:`defectio.Message` as its second
