...
import pandas as pd
import pathlib
from aimltools.ts import DateTimeUtils
...
class OSIPIReader:
    ...
    def __init__(self, servername, datetime_format, datetime_format_pi):
    ...
    def read_tag_interpolated(self, tagname, ts_list):
        ...
        li = List[AFTime]()
        for ts in ts_list:
            ts_epoch = DateTimeUtils.string_to_epoch_ciso8601(ts)
    ...
    def read_batch_tree(self, modulepath, starttime, endtime):
        start_time_dt = DateTimeUtils.string_to_datetime_ciso8601(starttime)
        end_time_dt = DateTimeUtils.string_to_datetime_ciso8601(endtime)
    ...
    def __read_tag(self, tagname, starttime, endtime, boundary_type):
        starttime_utc_secs = DateTimeUtils.string_to_epoch_ciso8601(starttime)
        endtime_utc_secs = DateTimeUtils.string_to_epoch_ciso8601(endtime)
...
class PIDataUBSerializer:
    ...
    def process_batches(self, pidata):
        ...
        df_ub[PIDataUBSerializer.COL_UB_UTCSTART] = [DateTimeUtils.datetime_to_epoch(val) for val in self.__ub_dict[PIDataUBSerializer.COL_UB_STARTTIME]]
        df_ub[PIDataUBSerializer.COL_UB_UTCEND] = [DateTimeUtils.datetime_to_epoch(val) for val in self.__ub_dict[PIDataUBSerializer.COL_UB_ENDTIME]]

        df_sb = pd.DataFrame.from_dict(self.__sb_dict)
        df_sb[PIDataUBSerializer.COL_SB_UTCSTART] = [DateTimeUtils.datetime_to_epoch(val) for val in self.__sb_dict[PIDataUBSerializer.COL_SB_STARTTIME]]
        df_sb[PIDataUBSerializer.COL_SB_UTCEND] = [DateTimeUtils.datetime_to_epoch(val) for val in self.__sb_dict[PIDataUBSerializer.COL_SB_ENDTIME]]
