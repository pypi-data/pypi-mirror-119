...
class ChoiceModel(ABC):
    ...
    def _post_fit(self, optimization_res, coeff_names, sample_size, hess_inv=None, verbose=1):
        ...
        # TODO: check for latent class models. Adjust if want transform + latent
    ...
    def summary(self):
        ...
        if hasattr(self, 'is_latent_class'):
            ...
            print("-"*75)
            print("{:19} {:>13} {:>13} {:>13} {:>13}"
                  .format("Class Member Coefficient", "Estimate", "Std.Err.", "z-val", "P>|z|"))
            print("-"*75)
            for i in range(len(self.class_x)):
                signif = ""
                if pvalues[i] < 0.001:
                    signif = "***"
                elif pvalues[i] < 0.01:
                ...
                print(fmt.format(self.coeff_names[i+self.num_params][:19], self.class_x[i],
                                self.class_x_stderr[i], zvalues[i], pvalues[i],
                                signif
                                ))
    ...
    def ind_pred_alts(self, verbose=True):
        ...
        if verbose:
            ...
            print()
        # print("Number of each altenative observed (individual):")
        # for alt in self.alternatives:
        #     print(fmt.format(alt), end='  ')
        # print()
        # for pred in self.obs_num_alts:
        #     print(fmt.format(pred), end='  ')
        # print()
        return pred_alts

    # def panel_pred_alts(self):
    #     pred_alts = []
    #     for ind_row in self.
    #     pass
    ...
    def fitted(self, type="parameters"):
        ...
        for k in range(self.Kr):
            ...
            # print('s_jn', s_jn/np.sqrt(self.N))
            # print('s_jn2', s_jn2/np.sqrt(self.N))

        # stdevs = self._get_stddev()
        if type == "parameters":
