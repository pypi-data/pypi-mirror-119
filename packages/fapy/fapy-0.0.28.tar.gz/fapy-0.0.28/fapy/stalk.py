...
from fapy.net import check_path as _chp, http_connection, http_request
...
def check_spa(host, port=80, ssl=False, timeout=2):
    path = random_path()
    code, _ = _chp(host, port, path, ssl=ssl, timeout=timeout)
    return 200 <= code < 300
...
def path_checker(path, port=80, ssl=False, timeout=2, inc_re='', exc_re=''):
    ...
    def f(host):
        _path = random_path()
        _c = http_connection(host, port, ssl, timeout=timeout)
        ...
        # check target path
        code, body = http_request(_c, path)
