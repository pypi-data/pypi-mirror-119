...
class ImagePhantomBase:
    ...
    def _sample_high_contrast_rois(self, roi_settings=None) -> List[HighContrastDiskROI]:
...
class StandardImagingQC3(ImagePhantomBase):
    ...
    def phantom_ski_region(self) -> RegionProperties:
        ...
        for phantom_idx, region in enumerate(regions):
            ...
            is_at_145cm = np.isclose(region.bbox_area, phantom_size_pix/(1.45**2), rtol=0.08)
            centered = np.allclose(region.centroid, img_center, rtol=0.1)
            if (is_at_iso or is_at_145cm) and centered:
                blobs.append(phantom_idx)
...
class PTWEPIDQC(ImagePhantomBase):
    ...
    def _sample_high_contrast_rois(self, roi_settings) -> List[HighContrastDiskROI]:
        """Sample the high-contrast line pair regions."""
        hc_rois = []
    ...
    def _sample_low_contrast_rois(self, roi_settings) -> List[LowContrastDiskROI]:
        """Sample the low-contrast sample regions for calculating contrast values."""
        lc_rois = []
        for stng in roi_settings.values():
            roi = LowContrastDiskROI(self.image,
