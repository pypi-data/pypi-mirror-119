    ...
    def merge_data(self, target_data, source_data, field_key, is_master_data=False):
        if source_data:
            ...
            # if suggest and value is not None:
                ...
                # lst_province = [x.value for x in ProvinceEnum]
                # province_code = next(
                #     (x for x in lst_province if x.code == value), None
                # )
                # if province_code:
                #     target_data[field_key] = CommonHelper.create_simple_data_type(
                #         _id=province_code.code, _name=province_code.name
                #     )
