...
class optimization(ABC):
    ...
    def perform_optimization(self, data_opt, P_PV, P_load, unit_load_cost):
        ...
        The LP problem is posed to minimize the following obtective function: \
        .. math::
            \sum_{i=1}^{optTimeDelta/timeStep} 0.001*timeStep(unitLoadCost_i*(P_{load_i}+P_{defSum_i})+prodSellPrice*P_{gridNeg_i})
        where :math:`optTimeDelta` is the total period of optimization in hours, \
        :math:`timeStep` is the optimization time step in hours, :math:`unitLoadCost` \
        is the cost of the energy from the utility, :math:`P_{load}` is the \
        electricity load consumption, :math:`P_{defSum}` is the sum of the deferrable \
        loads defined, :math:`prodSellPrice` is the price of the energy sold to \
        ...
        The problem constraints are written as follows: \
            ...
            and negative components of the battery pozer (charge)
        ...
        opt_model = plp.LpProblem("LP_Model", plp.LpMinimize)
