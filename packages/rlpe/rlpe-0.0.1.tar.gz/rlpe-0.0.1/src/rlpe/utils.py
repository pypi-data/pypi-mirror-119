from src.rlpe.Observer.lunar_lander_expert import LunarLanderExpert
from src.rlpe.Observer.single_taxi_expert import SingleTaxiExpert
from src.rlpe.Observer.taxi_expert import Taxi_Expert
from src.rlpe.Transforms.transform_constants import *
from src.rlpe.save_load_utils import *
...
def get_env(env_name, number_of_agents=1):
    ...
    """
    if env_name == TAXI:
        from src.rlpe.Environments.taxi_environment_wrapper import TaxiSimpleEnv
        return TaxiSimpleEnv()
    elif env_name == TAXI_EXAMPLE:
        from src.rlpe.Environments.taxi_environment_wrapper import TaxiSimpleExampleEnv
        return TaxiSimpleExampleEnv()
    elif env_name == SINGLE_TAXI_EXAMPLE:
        from src.rlpe.Environments.SingleTaxiEnv.single_taxi_wrapper import SingleTaxiSimpleEnv
        return SingleTaxiSimpleEnv()
    elif env_name == SINGLE_FROZEN_EXAMPLE:
        from src.rlpe.Environments.frozenlake_environment import FrozenLakeEnv
        return FrozenLakeEnv()
    elif env_name == LUNAR_LANDER:
        from src.rlpe.Environments.lunar_lander_wrapper import LunarLenderWrapper
        ...
        from src.rlpe.Environments.ApplePicking.apple_picking_env import ApplePickingEnv
...
def get_transformed_env(env_name):
    if env_name == TAXI_EXAMPLE:
        from src.rlpe.Transforms.taxi_transforms import TaxiTransformedEnv
        return TaxiTransformedEnv
    elif env_name == SINGLE_TAXI_EXAMPLE:
        from src.rlpe.Transforms.single_taxi_transforms import SingleTaxiTransformedEnv
        return SingleTaxiTransformedEnv
    elif env_name == LUNAR_LANDER:
        from src.rlpe.Transforms.lunar_lander_transforms import LunarLanderTransformedEnv
...
def get_transform_name(env_name, bool_params):
    if env_name == TAXI_EXAMPLE:
        from src.rlpe.Transforms.taxi_transforms import get_taxi_transform_name
        return get_taxi_transform_name(bool_params)
    elif env_name == SINGLE_TAXI_EXAMPLE:
        from src.rlpe.Transforms.single_taxi_transforms import get_single_taxi_transform_name
        return get_single_taxi_transform_name(bool_params)
    elif env_name == LUNAR_LANDER:
        from src.rlpe.Transforms.lunar_lander_transforms import get_lunar_lander_transform_name
