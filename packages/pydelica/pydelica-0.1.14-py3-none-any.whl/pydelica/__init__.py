...
class Session:
    ...
    def use_library(self, library_name: str, library_version: str = "") -> None:
    ...
    def fail_on_assert_level(self, assert_level: str) -> None:
        ...
        """
        if assert_level not in ['info', 'warning', 'error', 'never']:
    ...
    def simulate(self,
        model_name: str = None,
        verbosity: Optional[OMLogLevel]=None
        ) -> None:
        ...
        pde.parse_error_string_simulate(
            _run_sim.stdout,
            self._assert_fail_level
        )
