import sys
import argparse
import terragit.terracommand
import terragit.terradocs
...
def main():
    ...
    if sys.argv[1] in ["changes","validate","plan","apply","output"]:
        ...
        if args.destroy :
            ...
            if sys.argv[1] == "apply" :
                sys.arg[1] = "destroy"
        comm =terracommand.terracommand(args.project_id, args.commit_id, args.mr_id, args.gitlab_token, args.gitlab_url, args.directory, args.verbose, args.commit_title)

        comm.terragruntCommand(sys.argv[1])
    if sys.argv[1] =="docs":
        parser.add_argument("-m", "--module", dest = "module",default = "", help="module")
        parser.add_argument("-o", "--output", dest = "output_path",default = "", help="output path")

        parser.add_argument(sys.argv[1])
        args = parser.parse_args()

        terradoc =terradocs.terradoc(args.output_path,args.gitlab_url,args.gitlab_token)
        subgrouplist=terradoc.getSubgroupList(args.module)
        terradoc.getModule(subgrouplist,args.module)
        terradoc.f.close()
