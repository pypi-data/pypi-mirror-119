...
def active_cropping(x, y, ly, size, scale, p, tries):
	...
	if x.shape[0] * scale != y.shape[0] or x.shape[1] * scale != y.shape[1]:
		raise ValueError("Scaled shape must be equal ({}, {})".format(x.shape[:1], y.shape[:1]))
	...
	if np.random.uniform() < p:
		...
		for i in range(tries):
			...
			crop_x = x[ph // scale : ph // scale + size_x, pw // scale : pw // scale + size_x, :]
			crop_ly = ly[ph // scale : ph // scale + size_x, pw // scale : pw // scale + size_x, :]
...
def pairwise_transform(src, cfg):
	...
	patch_x = np.zeros((cfg.patches, cfg.ch, cfg.in_size, cfg.in_size), dtype=np.uint8)
	patch_y = np.zeros((cfg.patches, cfg.ch, cfg.out_size, cfg.out_size), dtype=np.uint8)
