...
class GearsApi(object):
    ...
    def prepare_add_gear(self, gear, **kwargs):  # noqa: E501
        """Prepare a gear upload

        This method makes a synchronous HTTP request by default.

        :param GearDoc gear: (required)
        :param bool async_: Perform the request asynchronously
        :return: GearReturnTicket
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_'):
            return self.prepare_add_gear_with_http_info(gear, **kwargs)  # noqa: E501
        else:
            (data) = self.prepare_add_gear_with_http_info(gear, **kwargs)  # noqa: E501
            if data and hasattr(data, 'return_value'):
                return data.return_value()
            return data


    def prepare_add_gear_with_http_info(self, gear, **kwargs):  # noqa: E501
        """Prepare a gear upload
        ...
        for key, val in six.iteritems(params['kwargs']):
            ...
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'gear' is set
        if ('gear' not in params or
                params['gear'] is None):
            raise ValueError("Missing the required parameter `gear` when calling `prepare_add_gear`")  # noqa: E501
        ...
        body_params = None
        if 'gear' in params:
            body_params = flywheel.models.GearDoc.positional_to_model(params['gear'])
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501
        ...
        return self.api_client.call_api(
            ...
            post_params=form_params,
            files=local_var_files,
            response_type='GearReturnTicket',  # noqa: E501
            auth_settings=auth_settings,
            async_=params.get('async_'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
