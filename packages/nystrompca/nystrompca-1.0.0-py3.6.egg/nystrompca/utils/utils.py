...
def get_inverse(M:    np.ndarray,
    ...
    func : Callable, default identity
        Tranformation to apply to the eigenvalues before
        taking the inverse
...
def flip_dimensions(scores:     np.ndarray,
                    components: np.ndarray) -> (np.ndarray, np.ndarray):
    """
    ...
    lies mostly within positive values. Specifically, we flip dimensions so that
    the mid point of the range of values is positive.
