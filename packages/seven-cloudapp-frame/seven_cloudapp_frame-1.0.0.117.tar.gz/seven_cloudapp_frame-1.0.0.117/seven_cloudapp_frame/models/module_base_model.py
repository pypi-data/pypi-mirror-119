...
@LastEditTime: 2021-09-06 19:40:06
...
class ModuleBaseModel():
    ...
    def get_act_module_list(self,app_id,act_id,module_name,is_del,page_size,page_index,is_cache=True):
        ...
        if is_cache:
            page_list, total = ActModuleModel(context=self.context).get_cache_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by, params=params,dependency_key=f"act_module_list:actid_{act_id}")
        else:
