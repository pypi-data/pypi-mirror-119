...
@LastEditTime: 2021-09-14 19:46:58
...
class SaveActModuleHandler(TaoBaseHandler):
    ...
    def post_async(self):
        ...
        module_name = self.get_request_param("module_name")
        module_sub_name = self.get_request_param("module_sub_name")
        start_date = self.get_request_param("start_date", "1900-01-01 00:00:00")
        end_date = self.get_request_param("end_date", "1900-01-01 00:00:00")
        module_pic = self.get_request_param("module_pic")
        module_desc = self.get_request_param("module_desc")
        price = self.get_request_param("price")
        price_gear_id = int(self.get_request_param("price_gear_id",0))
        ip_id = int(self.get_request_param("ip_id", 0))
        join_ways = int(self.get_request_param("join_ways",0))
        is_fictitious = int(self.get_request_param("is_fictitious",0))
...
class ActModuleListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        page_size = int(self.get_request_param("page_size", 10))
        is_del = int(self.get_request_param("is_del", -1))
        module_name = self.get_request_param("module_name")
        start_date = self.get_request_param("start_date")
        end_date = self.get_request_param("end_date")

        if not app_id or not act_id:
            return self.response_json_success({"data": []})
        module_base_model = ModuleBaseModel(context=self)
        page_list, total = module_base_model.get_act_module_list(app_id, act_id, module_name, start_date, end_date, is_del, page_size, page_index, False)
...
class UpdateActModulePriceHandler(TaoBaseHandler):
    """
    ...
    def get_async(self):
        ...
        app_id = self.get_app_id()
        act_id = int(self.get_request_param("act_id", 0))
        prize_gear_id = int(self.get_request_param("prize_gear_id", 0))
