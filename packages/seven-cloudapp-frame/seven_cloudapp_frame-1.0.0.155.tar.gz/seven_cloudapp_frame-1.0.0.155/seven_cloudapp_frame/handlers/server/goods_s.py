...
@LastEditTime: 2021-09-14 19:46:33
...
class GoodsListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        goods_name = self.get_request_param("goods_name")
        order_tag = self.get_request_param("order_tag", "list_time")
        order_by = self.get_request_param("order_by", "desc")
        page_index = int(self.get_request_param("page_index", 0))
        page_size = self.get_request_param("page_size", 20)
...
class GoodsListByGoodsIDHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        """
        access_token = self.get_taobao_param().access_token
        goods_ids = self.get_request_param("goods_ids")
        is_log = bool(self.get_request_param("is_log",False))
...
class BenefitDetailHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        """
        access_token = self.get_taobao_param().access_token
        right_ename = self.get_request_param("right_ename")
        is_log = bool(self.get_request_param("is_log",False))
...
class InventoryGoodsListHandler(TaoBaseHandler):
    """
...
class SpecialGoodsListHandler(TaoBaseHandler):
    """
...
class BindSpecialGoodsHandler(TaoBaseHandler):
    """
