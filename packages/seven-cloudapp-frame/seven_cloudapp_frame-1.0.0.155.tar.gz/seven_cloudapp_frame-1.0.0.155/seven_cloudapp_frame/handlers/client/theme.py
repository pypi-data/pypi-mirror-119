...
class ThemeInfoHandler(TaoBaseHandler):
    """
    ...
    def get_async(self):
        ...
        ver_no = self.get_request_param("ver_no")
...
class SaveThemeHandler(TaoBaseHandler):
    """
    :description: 保存主题
    """
    ...
    def post_async(self):
        ...
        theme_name = self.get_request_param("theme_name")
        client_json = self.get_request_param("client_json")
        server_json = self.get_request_param("server_json")
        out_id = self.get_request_param("out_id")
        ver_no = self.get_request_param("ver_no")
...
class SaveSkinHandler(TaoBaseHandler):
    """
    :description: 保存皮肤
    """
    ...
    def post_async(self):
        ...
        """
        app_id = self.get_taobao_param().source_app_id
        skin_name = self.get_request_param("skin_name")
        client_json = self.get_request_param("client_json")
        server_json = self.get_request_param("server_json")
        theme_out_id = self.get_request_param("theme_out_id")
        skin_out_id = self.get_request_param("skin_out_id")
