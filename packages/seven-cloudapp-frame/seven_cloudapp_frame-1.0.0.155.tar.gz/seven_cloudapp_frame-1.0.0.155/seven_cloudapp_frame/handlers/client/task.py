...
class TaskInfoListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        task_types = self.get_request_param("task_types")
        is_log = self.get_request_param("is_log", False)
...
class ReceiveRewardHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        task_sub_type = self.get_request_param("task_sub_type")
        task_base_model = TaskBaseModel(context=self)
...
class InviteJoinMemberHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        continue_request_expire = invoke_result_data.data["continue_request_expire"] if invoke_result_data.data.__contains__("continue_request_expire") else 5
        app_key, app_secret = self.get_app_key_secret()
        invoke_result_data = task_base_model.process_invite_join_member(app_id, act_id, module_id, user_id, login_token, invite_user_id, self.__class__.__name__, self.get_taobao_param().access_token, app_key, app_secret, check_user_nick, continue_request_expire)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        invoke_result_data = self.business_process_executed(invoke_result_data, app_id, act_id, user_id, module_id, login_token, invite_user_id, self.request_params)
        if invoke_result_data.success == False:
...
class BrowseSiteHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        user_id = int(self.get_request_param("tb_user_id", 0))
        module_id = int(self.get_request_param("module_id", 0))
        task_type = int(self.get_request_param("task_type", 0))
        task_sub_type = self.get_request_param("task_sub_type")
        login_token = self.get_request_param("login_token")
        task_base_model = TaskBaseModel(context=self)
