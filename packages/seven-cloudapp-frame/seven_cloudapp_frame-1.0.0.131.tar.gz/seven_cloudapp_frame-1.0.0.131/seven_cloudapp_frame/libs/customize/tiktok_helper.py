...
@LastEditTime: 2021-09-08 11:13:14
...
class TikTokError(Exception):
    def __init__(self, msg):
        super(TikTokError, self).__init__(msg)


# class DouYinPayRequest(object):
#     """配置账号信息"""
#     # =======【基本信息设置】=====================================
...
#         :last_editors: HuangJingCan
#         """
#         payconfig = config.get_value("douyin_pay")
#         self.APPID = config.get_value("app_id")
#         self.APIKEY = payconfig['api_key']
#         self.CPEXTRA = payconfig['cp_extra']
#         self.NOTIFYURL = payconfig['notify_url']
...
#         redis_key = "douyin_order_id:" + str(out_order_no)
#         order_id = DouYinHelper.redis_init().get(redis_key)
#         if order_id:
#             return order_id
...
#                 DouYinHelper.redis_init().set(redis_key, order_id, ex=3600 * 1)
#                 return resp
#             else:
#                 self.logger_error.error(f"【{out_order_no},统一下单获取order_id】" + resp['err_tips'])
#                 return TikTokError(resp['err_tips'])
#         else:
#             self.logger_error.error(f"【{out_order_no},统一下单获取order_id】" + response.text)
#             return TikTokError("支付接口报错，请重试")
...
#                 return TikTokError("查询订单出现异常，请重试")
#         except Exception as ex:
...
#         """
#         :description:支付/退款/结算回调
#         :param params: 字典 由抖音端请求,通过params = json.loads(self.request.body)获得
#         :last_editors: HuangJingCan
#         """
#         # data = {
#         #     'timestamp': '1620464599',
#         #     'nonce': '6115',
#         #     'msg':
#         #     '{"appid":"tt3a194ca4a5233849","cp_orderno":"2021050816471823375231","cp_extra":"hello world","way":"2","channel_no":"2021050822001459941406250991","channel_gateway_no":"12105080123729621931","payment_order_no":"2021050822001459941406250991","out_channel_order_no":"","total_amount":1}',
#         #     'msg_signature': '69cbf84dff685cc43df65e288bf1c2607bc318ab',
#         #     'type': 'payment'
#         # }

#         if data:
#             #计算服务端sign
#             service_signature = self.get_tt_callback_sign(data)
#             #验证成功,进行业务处理
#             log_msg = f'客户端签名:{data["msg_signature"]},服务端签名:{service_signature}'
#             # logger_info.info(log_msg)
#             if service_signature == data["msg_signature"]:
#                 #根据支付/退款/分账等回调json内容 在这边写业务逻辑
#                 #检测type字段判断是支付回调还是退款回调
#                 # type:payment 支付成功回调,refund退款成功回调,settle分账成功回调
#                 return  #to do
