...
@LastEditTime: 2021-09-14 19:29:48
...
class WeChatHelper:
    ...
    @classmethod
    def code2_session(self,app_id, app_secret, code, grant_type="authorization_code"):
        """
    ...
    @classmethod
    def get_access_token(self, app_id, app_secret, grant_type="client_credential"):
        """
    ...
    @classmethod
    def decrypt_data_by_code(self, app_id, app_secret, open_id, code, encrypted_Data, iv):
        """
        ...
        data = None
        if code:
            code2_session_dict = self.code2_session(app_id, app_secret,code)
            if code2_session_dict:
    ...
    def get_sign(self, params):
        ...
        string_sign_temp = string_a + '&key=' + self.api_key  # APIKEY, API密钥，需要在商户后台设置
...
class WeChatPayRequest(object):
    """
    :description: 微信支付请求类
    :description: 配置文件内容 "wechat_pay": {"api_key": "","mch_id": "","certificate_url":""}
    """
    """配置账号信息"""
    # =======【基本信息设置】=====================================
    ...
    def get_prepay_id(self, unifiedorder_url, params):
        ...
        respone = requests.post(unifiedorder_url, self._get_request_xml(params), headers={'Content-Type': 'application/xml'})
        ...
        if response_data['return_code'] == 'SUCCESS':
            if response_data['result_code'] == 'SUCCESS':
                ...
                self.logger_error.error("【预支付单号】" + SevenHelper.json_dumps(response_data))
                invoke_result_data.success = False
    ...
    def create_order(self, pay_order_no, body, total_fee, spbill_create_ip, notify_url, open_id="", time_expire="", trade_type="JSAPI"):
        ...
        invoke_result_data = self.get_prepay_id(unifiedorder_url, params)
        ...
        invoke_result_data.data = WeChatHelper.get_sign(sign_again_params)
    ...
    def query_order(self, pay_order_no="", transaction_id=""):
        ...
        try:
            ...
            request_xml = self._get_request_xml(params)
    ...
    def create_refund(self, refund_no, pay_order_no, notify_url, refund_fee, total_fee):
        ...
        params = WeChatHelper.get_sign(params)
        refund_url = 'https://api.mch.weixin.qq.com/secapi/pay/refund'
        respone = post(url=refund_url, data=self._get_request_xml(params), headers={'Content-Type': 'application/xml'}, pkcs12_filename=self.certificate_url, pkcs12_password=self.mch_id)
        response_data = xmltodict.parse(respone.text.encode('ISO-8859-1').decode('utf-8'))['xml']
        if response_data['return_code'] == 'SUCCESS':
            if response_data['result_code'] == 'SUCCESS':
                ...
                self.logger_error.error("【创建退款单】" + SevenHelper.json_dumps(response_data))
                invoke_result_data.success = False
                invoke_result_data.error_code="error"
                invoke_result_data.error_message = response_data['err_code_des']
                return invoke_result_data
    ...
    def _get_request_xml(self, params):
        """
        :description:拼接XML
        :return: 
        :last_editors: HuangJianYi
...
class WeChatPayReponse(object):
    ...
    def check_sign(self):
        ...
        del params['sign']
        sign = WeChatHelper.get_sign(params)  # 本地签名
    ...
    def response_xml(self, msg, ok=True):
        """
