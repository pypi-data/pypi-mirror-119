...
@LastEditTime: 2021-09-14 19:41:31
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def get_request_param(self, param_name, default="", strip=True):
        """
...
class TaoBaseHandler(FrameBaseHandler):
    ...
    def prepare(self):
        ...
        dict_par["app_id"] = self.get_request_param("app_id")
    ...
    def get_taobao_param(self):
        ...
        :last_editors: HuangJianYi
        """
        app_key = self.get_request_param("app_key")
        user_nick = self.get_request_param("user_nick")
        open_id = self.get_request_param("open_id")
        env = self.get_request_param("env")
        mini_app_id = self.get_request_param("mini_app_id")
        access_token = self.get_request_param("access_token")
        sign = self.get_request_param("sign")
        mix_nick = self.get_request_param("mix_nick")
        tb_user_id = self.get_request_param("tb_user_id")
        main_user_id = self.get_request_param("main_user_id")
        source_app_id = self.get_request_param("source_app_id")
        request_id = self.get_request_param("request_id")
    ...
    def get_app_id(self):
        ...
        :last_editors: HuangJianYi
        """
        app_id = self.get_request_param("app_id")
        if app_id == "":
            ...
            if user_nick:
                ...
                app_id = config.get_value("test_source_app_id")
        return app_id
    ...
    def create_operation_log(self, operation_type=1, model_name="", handler_name="", old_detail=None, update_detail=None, operate_user_id="",operate_user_name=""):
        ...
        operation_log.act_id = int(self.get_request_param("act_id", 0))
...
class ClientBaseHandler(FrameBaseHandler):
    ...
    def prepare_ext(self):
        ...
        dict_par = {}
        dict_par["open_id"] = self.get_request_param("user_id")
        dict_par["app_id"] = self.get_request_param("app_id","1")

        try:
            # 将设备信息字符串转为字典类型
            clientheaderinfo_string = self.request.headers._dict.get("Clientheaderinfo")
            if clientheaderinfo_string:
                self.device_info_dict = self.device_info_dict(clientheaderinfo_string)

            user_id = self.get_request_param("user_id")
            if user_id:
                # 频繁请求校验
                is_continue_request, error_message = self.check_continue_request(self.__class__.__name__, dict_par["app_id"], user_id)
                if is_continue_request:
                    return self.response_json_error("error", error_message)
                #每分钟流量UV计数,用于登录接口限制登录
                self.add_request_user(str(dict_par["app_id"]), user_id)

        except Exception as ex:
            self.logging_link_error("【客户端handler基类】" + str(ex))


    def device_info_dict(self, device_str):
        """
...
def filter_check_params_sign(is_check=True, no_check_params=None):
    ...
    def check_params(handler):
        def wrapper(self, **args):
            encrypt_key = config.get_value("encrypt_key", "3924e337a476475e95b2b341eaba8c1c")
            is_check_params = config.get_value("is_check_params", True)
