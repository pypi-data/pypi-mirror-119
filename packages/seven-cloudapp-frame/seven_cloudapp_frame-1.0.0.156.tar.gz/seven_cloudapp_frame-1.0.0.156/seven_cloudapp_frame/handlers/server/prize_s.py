...
@LastEditTime: 2021-09-14 19:47:41
...
class SaveActPrizeHandler(TaoBaseHandler):
    ...
    def post_async(self):
        ...
        prize_id = int(self.get_request_param("prize_id", 0))
        prize_name = self.get_request_param("prize_name")
        prize_title = self.get_request_param("prize_title")
        prize_pic = self.get_request_param("prize_pic")
        prize_detail_json = self.get_request_param("prize_detail_json")
        goods_id = self.get_request_param("goods_id")
        goods_code = self.get_request_param("goods_code")
        goods_code_list = self.get_request_param("goods_code_list")
        goods_type = int(self.get_request_param("goods_type", 0))
        prize_type = int(self.get_request_param("prize_type", 0))
        prize_price = self.get_request_param("prize_price")
        probability = self.get_request_param("probability")
        chance = self.get_request_param("chance")
        prize_limit = int(self.get_request_param("prize_limit", 0))
        is_prize_notice = int(self.get_request_param("is_prize_notice", 0))
        prize_total = int(self.get_request_param("prize_total", 0))
        is_surplus = int(self.get_request_param("is_surplus", 0))
        lottery_type = int(self.get_request_param("lottery_type", 0))
        tag_name = self.get_request_param("tag_name")
        tag_id = int(self.get_request_param("tag_id", 0))
        is_sku = int(self.get_request_param("is_sku", 0))
        sku_json = self.get_request_param("sku_json")
        ascription_type = int(self.get_request_param("ascription_type", 0))
        sort_index = int(self.get_request_param("sort_index", 0))
        is_release = int(self.get_request_param("is_release", 0))
        i1 = int(self.get_request_param("i1", 0))
        i2 = int(self.get_request_param("i2", 0))
        i3 = int(self.get_request_param("i3", 0))
        i4 = int(self.get_request_param("i4", 0))
        i5 = int(self.get_request_param("i5", 0))
        s1 = self.get_request_param("s1")
        s2 = self.get_request_param("s2")
        s3 = self.get_request_param("s3")
        s4 = self.get_request_param("s4")
        s5 = self.get_request_param("s5")
        d1 = self.get_request_param("d1", "1900-01-01 00:00:00")
        d2 = self.get_request_param("d2", "1900-01-01 00:00:00")
        ...
        if invoke_result_data.data["is_add"] == True:
            # 记录日志
            self.create_operation_log(OperationType.add.value, invoke_result_data.data["new"].__str__(), "SaveActPrizeHandler", None, self.json_dumps(invoke_result_data.data["new"]), self.get_taobao_param().open_id, self.get_taobao_param().user_nick)
        else:
            self.create_operation_log(OperationType.update.value, invoke_result_data.data["new"].__str__(), "SaveActPrizeHandler", self.json_dumps(invoke_result_data.data["old"]), self.json_dumps(invoke_result_data.data["new"]), self.get_taobao_param().open_id, self.get_taobao_param().user_nick)
...
class ActPrizeListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        module_id = int(self.get_request_param("module_id", 0))
        prize_name = self.get_request_param("prize_name")
        is_del = int(self.get_request_param("is_del", -1))
