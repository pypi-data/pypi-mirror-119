...
@LastEditTime: 2021-09-14 19:46:46
...
class InitLaunchGoodsHandler(TaoBaseHandler):
    """
    :description: 初始化商品投放
    """
    ...
    def get_async(self):
        ...
        source_types = self.get_request_param("source_types","1,2")
...
class ResetLaunchGoodsHandler(TaoBaseHandler):
    """
...
class InitLaunchGoodsCallBackHandler(TaoBaseHandler):
    """
    ...
    def get_async(self):
        ...
        app_id = self.get_app_id()
        act_id = int(self.get_request_param("act_id", 0))
        close_goods_id = self.get_request_param("close_goods_id")
...
class UpdateLaunchGoodsStatusHandler(TaoBaseHandler):
    """
    ...
    def get_async(self):
        ...
        app_id = self.get_app_id()
        act_id = int(self.get_request_param("act_id", 0))
        goods_id = self.get_request_param("update_goods_id")
        if goods_id != "":
...
class LaunchGoodsListHandler(TaoBaseHandler):
    """
    :description: 投放商品列表
    """
    @filter_check_params("act_id")
    def get_async(self):
        ...
        invoke_result_data = launch_base_model.get_launch_goods_list(app_id, act_id, page_size, page_index, self.get_taobao_param().access_token, app_key, app_secret)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        return self.response_json_success(invoke_result_data.data)


class AsyncLaunchGoodsHandler(TaoBaseHandler):
    """
    ...
    def get_async(self):
        ...
        launch_base_model = LaunchBaseModel(context=self)
        app_key, app_secret = self.get_app_key_secret()
        invoke_result_data = launch_base_model.async_launch_goods(app_id, act_id, online_url, self.get_taobao_param().access_token, app_key,app_secret)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
