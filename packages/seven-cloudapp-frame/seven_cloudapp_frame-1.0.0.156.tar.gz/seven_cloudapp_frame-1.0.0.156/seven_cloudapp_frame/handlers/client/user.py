...
@LastEditTime: 2021-09-14 19:45:56
...
class LoginHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        avatar = self.get_request_param("avatar")
...
class UpdateUserInfoHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        avatar = self.get_request_param("avatar")
        is_member_before = int(self.get_request_param("is_member_before", -1))
        is_favor_before = int(self.get_request_param("is_favor_before", -1))
...
class ApplyBlackUnbindHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        act_id = int(self.get_request_param("act_id", 0))
        user_id = int(self.get_request_param("tb_user_id", 0))
        reason = self.get_request_param("reason", "误封号,申请解封")
...
class GetCouponPrizeHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        app_id = self.get_taobao_param().source_app_id
        open_id = self.get_taobao_param().open_id
        user_prize_id = int(self.get_request_param("user_prize_id", 0))
        is_log = bool(self.get_request_param("is_log", False))
        app_base_model = AppBaseModel(context=self)
        app_info_dict = app_base_model.get_app_info_dict(app_id)
        if not app_info_dict:
