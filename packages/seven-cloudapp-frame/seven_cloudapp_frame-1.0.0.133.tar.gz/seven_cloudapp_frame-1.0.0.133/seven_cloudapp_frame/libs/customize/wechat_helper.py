...
@LastEditTime: 2021-09-08 14:36:29
...
class WeChatPayRequest(object):
    ...
    def get_prepay_id(self, unifiedorder_url, params):
        ...
        if xmlresp['xml']['return_code'] == 'SUCCESS':
            if xmlresp['xml']['result_code'] == 'SUCCESS':
                ...
                self.logger_error.error("【{params['out_trade_no']},统一下单获取prepay_id】" + xmlresp['xml']['err_code_des'])
                return WeChatError(xmlresp['xml']['err_code_des'])
        else:
            self.logger_error.error("【{params['out_trade_no']},统一下单获取prepay_id】" + xmlresp['xml']['return_msg'])
            return WeChatError(xmlresp['xml']['return_msg'])
    ...
    def create_order(self, out_trade_no, body, total_fee, spbill_create_ip, notify_url, open_id="", time_expire="", trade_type="JSAPI"):
        ...
        if trade_type == "JSAPI":
            if open_id == "":
                return WeChatError("缺少必填参数open_id")
            else:
    ...
    def query_order(self, out_trade_no="", transaction_id=""):
        ...
        if transaction_id == "" and out_trade_no == "":
            return WeChatError("缺少必填参数transaction_id或out_trade_no")
        xmlresp = {}
        xml = ""
        try:
            params = {
    ...
    def close_order(self, out_trade_no=""):
        ...
        """
        if out_trade_no == "":
            return WeChatError("缺少必填参数out_trade_no")
        xmlresp = {}
        try:
            params = {
                'appid': self.app_id,  # appid
