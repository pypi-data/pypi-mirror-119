...
@LastEditTime: 2021-09-10 14:48:57
...
class TikTokHelper:
    ...
    def get_access_token(self, grant_type="client_credential"):
        ...
        try:
            resp = requests.get(requset_url, params=param)
            redis_init.set(redis_key, resp, ex=60 * 60)
            res_result = json.loads(resp.text)
            if res_result['error'] != 0:
                return None
            return res_result
        except Exception as ex:
            self.logger_error.error(str(ex) + "【get_access_token】" + str(resp.text))
            return None
