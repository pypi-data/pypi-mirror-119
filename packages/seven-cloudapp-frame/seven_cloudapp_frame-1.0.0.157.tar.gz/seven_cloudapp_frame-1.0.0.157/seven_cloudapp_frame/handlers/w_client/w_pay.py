...
@LastEditTime: 2021-09-14 18:52:50
...
class VoucherOrderHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        try:
            ...
            invoke_result_data = WeChatPayRequest(pay_config["app_id"], pay_config["api_key"], pay_config["mch_id"]).create_order(pay_order_no, body, total_fee, ip, pay_notify_url, pay_order.open_id, time_expire)
...
class PayNotifyHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        try:
            pay_config = config.get_value("wechat_pay")
            wechat_pay_reponse = WeChatPayReponse(xml_params, pay_config["api_key"])  # 创建对象
...
class RefundNotifyHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        try:
            pay_config = config.get_value("wechat_pay")
            wechat_refund_reponse = WeChatRefundReponse(xml_params,pay_config["api_key"])  # 创建对象
