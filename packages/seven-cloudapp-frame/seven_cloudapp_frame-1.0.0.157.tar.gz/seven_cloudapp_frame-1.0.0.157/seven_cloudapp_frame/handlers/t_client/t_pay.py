...
@LastEditTime: 2021-09-14 19:33:12
...
class VoucherOrderHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        try:
            ...
            invoke_result_data = TikTokPayRequest(pay_config["app_id"], pay_config["api_key"], pay_config["cp_extra"], pay_config["token"], pay_config["salt"]).create_order(out_order_no=pay_order_no, notify_url=pay_notify_url, total_amount=total_amount, subject=subject, body=body, valid_time=time_expire)
    ...
    def business_process_executing(self, request_params):
        ...
        """
        invoke_result_data = InvokeResultData()
        invoke_result_data.data = {"pay_notify_url": "", "time_expire": 0}
...
class PayNotifyHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        try:
            pay_config = config.get_value("tiktok_pay")
            tiktok_reponse = TikTokReponse(json_params, pay_config["token"])
    ...
    def business_process_executed(self, pay_order, respnse_data):
        ...
        if pay_order.order_status == 0 or pay_order.order_status == 2:
            ...
            order_id = ""
            pay_config = config.get_value("tiktok_pay")
            tiktok_pay_request = TikTokPayRequest(pay_config["app_id"], pay_config["api_key"], pay_config["cp_extra"], pay_config["token"], pay_config["salt"])
