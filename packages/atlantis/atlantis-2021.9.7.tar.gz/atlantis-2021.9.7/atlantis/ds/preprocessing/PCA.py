...
class PCA:
	...
	def fit(self, X):
		...
		if self._scale:
			...
			self._pca = PCA(k=self._k, inputCol='scaled_features', outputCol='pca_features').fit(scaled)
			...
			self._pca = PCA(k=self._k, inputCol='features', outputCol='pca_features').fit(assembled)
