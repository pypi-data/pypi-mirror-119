...
class ClusteringOptimizer:
	def __init__(
			...
			external_parallelization=True, n_external_jobs=None, n_internal_jobs=None, **kwargs
	):
		self._min_k = min_k
		self._max_k = max_k
		self._n_jobs = n_jobs
		...
		self._n_external_jobs = n_external_jobs
		self._n_internal_jobs = n_internal_jobs
	...
	def fit(self, X, echo=1, raise_timeout=True, y=None):
		...
		def get_scores(k, n_jobs):
			kmeans = KMeans(
				n_clusters=k, n_jobs=n_jobs, timeout=self._timeout, raise_timeout=raise_timeout, **self._kwargs
			)
			kmeans.fit(X=X)
			if kmeans.timedout:
		...
		if self._external_parallelization and self._n_jobs > 1:

			n_ks = self._max_k - self._min_k + 1
			n_external_jobs = self._n_external_jobs or min(n_ks, self._n_jobs)
			n_remaining_jobs = self._n_jobs - n_external_jobs
			n_internal_jobs = self._n_internal_jobs or max(1, int(round(n_remaining_jobs / n_ks)))
			print(f'n_ks = {n_ks} n_external_jobs = {n_external_jobs} n_internal_jobs = {n_internal_jobs}')

			processor = Parallel(n_jobs=n_external_jobs, backend='threading', require='sharedmem')
			result = processor(
				delayed(get_scores)(k=k, n_jobs=n_internal_jobs)
			...
			for k in range(self._max_k, self._min_k - 1, -1):
				...
				k, model, inertia, distortion, silhouette_score = get_scores(k=k, n_jobs=self._n_jobs)
				if inertia is not None or distortion is not None or silhouette_score is not None:
					if self._keep_models:
						self._models[k] = model
					self._distortions[k] = distortion
