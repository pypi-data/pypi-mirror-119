...
class FiberDynamicFields(InternetDynamicFields):
    ...
    def _build_specific_broadband_service_dynamic_fields(self):
        ...
        return [
            self._df_old_contract_pon(),
            self._df_old_contract_fiber_speed()
        ]

    def _df_old_contract_pon(self):
        return DynamicField(name="ponAntic", value=self.service_data.old_contract_pon)

    def _df_old_contract_fiber_speed(self):
        return DynamicField(
            name="velocitatSollicitadaContracteAnteriorCU",
            value=self.service_data.old_contract_fiber_speed
        )
