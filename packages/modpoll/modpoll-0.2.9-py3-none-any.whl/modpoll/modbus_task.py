        ...
        if args.diagnostics_rate > 0:
            ...
            if now > self.next_due:
                ...
                try:
                    ...
                    error_rate = 0
                mqttc_publish(
                    f"{args.mqtt_topic}{self.name}/diagnostics/error_rate", str(error_rate))
                mqttc_publish(
                    f"{args.mqtt_topic}{self.name}/diagnostics/total_poll", str(self.pollCount))
                self.pollCount = 0
                self.errorCount = 0
...
class Poller:
    ...
    def publish_data(self, timestamp=None, on_change=False):
        ...
        topic = f"{args.mqtt_topic}/{self.device.name}/state"
...
class Reference:
    ...
    def update_value(self, v):
        ...
        #     name = f"{args.mqtt_name}{self.device.name}/state/{self.name}"
        #     mqttc_publish(name, str(val))
...
def modbus_setup():
    ...
    if args.rtu:
        ...
        master = TCPModbusClient(
            args.tcp, args.tcp_port, client_id="modpoll", clean_session=False)
    else:
