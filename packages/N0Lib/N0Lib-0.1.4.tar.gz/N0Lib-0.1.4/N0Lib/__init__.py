...
def N0HELP(_function='$n0p'):
    ...
    HELPS={
        ...
        'followSay':'紧跟在上一次SAY的后面输出。(字符串,[颜色])',
...
def colorTest(limit=256):
    retStr='0 '
    retColorStr='1|0'
    if limit!='$n0p':
        limit=beNumber(limit,1024)
        if limit>1024:
            limit=1024
        if limit=='$n0p' or limit<1:
            consoleOut('color','Wrong number',_red,_white)
            limit=256
    else:
        limit=256
    for i in range(1,limit):
        retColorStr+=','+str(len(str(i))+1)+'|'+str(i)
        retStr+=str(i)+' '
    consoleOut('color',retStr,'SPACE',colorStr=retColorStr,COLORFILTER=False)
...
def ENTER(_x,_y,limitLength,color=_white,backStr='Please input.',backColor=_grey,keyList='$n0p',default='$n0p',history='$n0p',clear=True,hideChar='$n0p',maxLine=1,matchConfig='$n0p',__DEEP__=0,STDOUT='$n0p'):
    _x,_y,limitLength,maxLine=_legalSize(_x,_y,limitLength,maxLine)
    if STDOUT=='$n0p':
        STDOUT=__SYSTEM_VARS__['__STDOUT__']
    def saveHistory(_str):
        if history!='$n0p':#保留历史记录
                historyLength=len(history)
                if _str!='':
                    if historyLength>0:
                        if _str!=history[historyLength-1]:
                            history.append(_str)
                    else:
                        history.append(_str)
    retStr,flags=_ENTERLooper(_x,_y,limitLength,color,backStr,backColor,keyList,default,history,clear,hideChar,maxLine,matchConfig,0,STDOUT=STDOUT)
    if flags=='$enter':
        saveHistory(retStr)
        return retStr
    else:
        return "N0Shell doesn't know what happend.Maybe you should ask for N0P3."
def _ENTERLooper(_x,_y,limitLength,color=_white,backStr='Please input.',backColor=_grey,keyList='$n0p',default='$n0p',history='$n0p',clear=True,hideChar='$n0p',maxLine=1,matchConfig='$n0p',__DEEP__=0,keyWords=list(),STDOUT='$n0p'):
    def getSelection(_PreExcute):
        if _PreExcute!='':
            retSelect=_PreExcute[0]
            _PreExcute=_PreExcute[1:]
            #consoleOut('PreExcute',_PreExcute,'green','white')
        else:
            try:
                retSelect=_consoleWin.get_wch()
            except Exception as e:
                #debugOut('N0SHELL_WARNING',str(e),_yellow,_white)
                retSelect=curses.KEY_RIGHT
        #echo(retSelect)
        return _PreExcute,retSelect
    def createPreExcute(_str):
        if _str!='$n0p':
            _PreExcute=''
            while True:
                if len(_str)>limitLength:
                    _PreExcute+=_str[:limitLength]
                    _str=_str[limitLength+1:]
                    _PreExcute+='\\'+chr(10)
                else:
                    _PreExcute+=_str
                    break
    ...
    def hideClear():
        cursor.hide()
        drawLine(_x,_y,' ',limitLength+1)
        cursor.show()
    def retHistory(_index,_history,up=True):
        nonlocal indexX,retStr
        if up:
            if _index>0:
                _index-=1
        else:
            if _index<len(_history):
                _index+=1
        try:
            if _index!=len(_history):
                _PreExcute=createPreExcute(_history[_index])
            else:
                _PreExcute=''
            indexX=0
            retStr=''
    ...
    if matchConfig!='$n0p':#匹配设置
        matchConfig=patchDict(defaultMatchConfig,matchConfig)
    PreExcute=createPreExcute(default)#预执行命令
    ...
    if clear:
        hideClear()#清空输入区
    say(_x,_y,backStr,backColor)
    say(_x,_y,'')
    while True:
        cursor.show()
        PreExcute,selection=getSelection(PreExcute)
        #consoleOut('press',chr(selection),_white,_green)
        try:
            ordSelection=ord(selection)
        except:
            ordSelection=263
        if selection=='\\':
            say(_x+indexX,_y,'\\',_grey)
            PreExcute,selection=getSelection(PreExcute)
            if selection == '\n':#换行
                #retStr+=(limitLength-len(retStr))*' '#换行部分补空格
                #indexX=limitLength
                if __DEEP__<maxLine-1:
                    enterRet,flags=_ENTERLooper(_x,_y+1,limitLength,color,'',_grey,keyList,PreExcute,history,clear,hideChar,maxLine,matchConfig,__DEEP__+1,keyWords,STDOUT)
                    PreExcute=''
                    #debugOut('ENTER','enterRet '+enterRet)
                    if flags=='$del':#通过删除键返回
                        #debugOut('ENTER','Delete!')
                        say(_x+indexX,_y,' ')
                        say(_x,_y,retStr,color)
                        continue
                    elif flags=='$up':#通过上键返回
                        if __DEEP__>0:
                            retStr+=enterRet
                            hideClear()
                            return retStr,'$up'
                        else:
                            PreExcute,historyIndex=retHistory(historyIndex,history)
                        continue
                    elif flags=='$down':#通过下键返回
                        if __DEEP__>0:
                            hideClear()
                            return '','$down'
                        else:
                            PreExcute,historyIndex=retHistory(historyIndex,history,False)
                        continue
                    else:#通过ENTER返回
                        retStr+=enterRet
                        if clear:
        ...
        if selection == '\n':#ENTER
            ...
            cursor.hide()
            return retStr,'$enter'
        elif selection==curses.KEY_UP:#up
            if __DEEP__>0:
                hideClear()
                return retStr,'$up'
            else:
                PreExcute,historyIndex=retHistory(historyIndex,history)
                keyWord=''
        elif selection==curses.KEY_DOWN:#down
            if __DEEP__>0:
                hideClear()
                return '','$down'
            else:
            ...
            pass
        elif selection==410:#特殊的拖动产生的神秘字符
            pass
        elif ordSelection==27:
            pass
        elif ordSelection==__SYSTEM_VARS__['BACKSPACE']:#backspace
            if indexX>0:
                chrWidth=getWidth(retStr[-1])
                indexX-=chrWidth
                retStr=retStr[:-1]
                if len(keyWord)>0:
                    keyWord=keyWord[:-1]#tab补全用
                else:
                    try:
                        keyWord=keyWords.pop()
                    except:
                        keyWord=''
                say(_x+indexX,_y,' '*chrWidth,color)
                if indexX>0:
                    chrWidth=getWidth(retStr[-1])
                    if hideChar=='$n0p':
                        say(_x+indexX-chrWidth,_y,retStr[-1],color)#Unknow
                    else:
                        say(_x+indexX-chrWidth,_y,hideChar[0]*chrWidth,color)#Unknow
                else:
                    say(_x,_y,backStr,backColor)
                    say(_x,_y,'')
            else:
                if __DEEP__>0:#在深层递归
                    ...
                    pass
        elif selection==chr(__SYSTEM_VARS__['TAB']):
            if keyWord!='':
                if matchConfig!='$n0p':
                    matchItems=_matchThings(keyWord,matchConfig['Words'],matchConfig['Paths'],matchConfig['CurrentPath'],matchConfig['Var'],matchConfig['Command'],matchConfig['PATH'])
                else:
                    matchItems=_matchThings(keyWord)
                debugOut('sMatch',matchItems,STDOUT=STDOUT)
                if len(matchItems)>1:#不止一个匹配项
                    shortest=min(matchItems, key=len)#最短匹配项
                    index=0
                    FINISHED=False
                    debugOut('shortest ',shortest,STDOUT=STDOUT)
                    matchItems.remove(shortest)
                    for i in shortest:
                        for item in matchItems:
                            #debugOut('tab',item[index]+' vs shortest '+i)
                            if item[index]!=i:#某项目该位字符不匹配
                                FINISHED=True
                        if not FINISHED:
                            index+=1
                        else:
                            break
                    matchItems.append(shortest)
                    #consoleOut('tab',index)
                    if index>len(keyWord):
                        PreExcute=shortest[len(keyWord):index]
                        #consoleOut('pre',PreExcute)
                    else:
                        cursor.hide()
                        consoleOut('',','.join(matchItems),'SPACE','white',STDOUT=STDOUT)
                        say(_x+indexX,_y,'')#把光标移回命令行
                        cursor.show()
                elif len(matchItems)==1:
                    
                    result=matchItems.pop()
                    #---支持忽略.py后缀
                    if len(result)>3:
                        #consoleOut('a',result[:-3])
                        #consoleOut('b',result[-3:])
                        resultWithoutPy=result[:-3]+result[-3:].replace('.py','')#把文件结尾的.py去掉
                        #consoleOut('c',resultWithoutPy)
                    else:
                        resultWithoutPy=result
                    if len(resultWithoutPy)>len(keyWord):
                        PreExcute=resultWithoutPy[len(keyWord):]
                    else:
                        PreExcute=result[len(keyWord):]
        elif whiteList:
            try:
                if selection in keyList:
                    if indexX<limitLength:
                        say(_x+indexX,_y,selection,color)
                        chrWidth=getWidth(selection)
                        indexX+=chrWidth
                        retStr+=selection
                        if selection!=' ':
                            keyWord+=selection#tab补全用
                        else:
                            keyWords.append(keyWord)
                            keyWord=''
                    else:#过长
                        
                        say(_x,_y,retStr,_red)
                        time.sleep(0.01)
                        say(_x,_y,retStr,color)
            except Exception as e:
                consoleOut('Input Error1',str(e),red,yellow)
                pass
        else:
            try:
                if indexX<limitLength:
                    ...
                    if hideChar=='$n0p':
                        ...
                        say(_x+indexX,_y,hideChar[0]*chrWidth,color)
                ...
                consoleOut('Input Error2',str(e),red,yellow)
                pass
def _matchThings(target,matchWords='$n0p',matchPaths='$n0p',matchCurrentPath=True,matchVars=True,matchCommand=True,matchPATH=True):
    allItems=list()
    accessItems=list()
    searchPaths=list()
    if matchPATH:
        matchCurrentPath=True
    def searchDir(searchPaths,onlyFile=False):
        retItems=list()
        for path in searchPaths:
                try:
                    if realTargetPath==path:
                        for item in os.listdir(path):
                            if os.path.isfile(path+item):
                                retItems.append(targetPath+item)
                            else:
                                #if srcPath+_file==os.path.join(root,_file):#在top目录下
                                if not onlyFile:
                                    retItems.append(targetPath+item+'/')
                    else:
                        for item in os.listdir(path):
                            #debugOut('Try isfile',path+item)
                            if os.path.isfile(path+item):
                                ...
                                if not onlyFile:
                                    retItems.append(item+'/')
                    #consoleOut('matchThings',path+' LOAD FINISHED.','green','white')
                except Exception as e:
                    debugOut('matchThings',path+' LOAD FAILED.'+str(e),'red')
        return retItems
    targetName,targetPath=splitPath(target)
    if targetName!='$n0p':
        if targetName[0]=='$' and matchVars:
            for varName in __PublicVarPool__:
                try:
                    if targetName[1]=='(':
                        allItems.append('$('+varName+')')
                    else:
                        allItems.append('$'+varName)
                except:
                    allItems.append('$'+varName)
            ...
            if matchCurrentPath:
                if targetPath!='':
                    ...
                    searchPaths.append(realTargetPath)#path位置
                    targetName=target#保持原目标
                else:
                    realTargetPath=''
                    searchPaths.append(__SHELLPATH__)#默认shell当前位置
            if matchPaths!='$n0p':
                searchPaths+=matchPaths
            allItems+=searchDir(searchPaths)
            if matchWords!='$n0p':
                allItems+=matchWords
    #-----MATCH---------------------------
        #debugOut('allItems',allItems)
        newItems=[]
        for i in allItems:
...
def divide(string,TheChar,delESC=False):
    ...
    if not RIGHT:
        ...
        return left,right
def _getPart(id,parts,default='$n0p'):
    try:
        if parts[id]=='':
            return '$n0p'
        else:
