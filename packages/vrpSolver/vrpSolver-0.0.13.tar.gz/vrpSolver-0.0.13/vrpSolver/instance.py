...
def rndPlainNodes(
    ...
    if (distr == "uniformSquare"):
        ...
        lstTriangle = []
        for p in polys:
            lstTriangle.append(tripy.earclip(p))
        ...
        for i in range(N):
            ...
            x = distrArgs['centerLoc'][0] + distrArgs['radius'] * math.sin(math.radians(deg))
            y = distrArgs['centerLoc'][1] + distrArgs['radius'] * math.cos(math.radians(deg))
