        ...
        for network in networks:
            ...
            try:
                ...
                if not dry:
                    ...
                    kwargs = {
                        field: value
                        for field, value in network_json.items()
                        if field not in ("flagsToDisable", "flagsToEnable")
                    }
                    # Verified is its own field
                    kwargs["flagsToSet"] = [f for f in network_json["flagsToEnable"] if f != "UNVERIFIED"]
                    kwargs["verified"] = "UNVERIFIED" not in network_json["flagsToEnable"]

                    log.debug(json.dumps(kwargs))
                    add_customer_network(**kwargs)
