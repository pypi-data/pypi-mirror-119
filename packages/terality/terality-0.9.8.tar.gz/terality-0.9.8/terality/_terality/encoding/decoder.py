...
def _download_df(
    aws_credentials: AwsCredentials, transfer_id: str, is_col_json: List[bool]
) -> pd.DataFrame:
    df: pd.DataFrame = pd.read_parquet(
        DataTransfer.download_to_bytes(
            global_client().get_download_config(), aws_credentials, transfer_id  # type: ignore
        )
    )
...
def decode(credentials_fetcher: AwsCredentials, o):  # pylint: disable=invalid-name
