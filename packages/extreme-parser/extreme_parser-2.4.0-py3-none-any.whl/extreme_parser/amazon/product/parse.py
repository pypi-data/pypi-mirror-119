...
def parse(html: str, p: Product, include: Sequence = None):
    ...
    "product_asin" in include and parse_product_asin(p, selector=sel)
    ...
    "price" in include and parse_price(p, selector=sel)
    "in_stock" in include and parse_in_stock(p, selector=sel)
    "stock" in include and parse_stock(p, selector=sel)
    "ship_day" in include and parse_ship_day(p, selector=sel)
    ...
    "size" in include and parse_size(p, selector=sel)
    "color" in include and parse_color(p, selector=sel)
    "variation" in include and parse_variation(p, selector=sel)
...
def parse_ship(p: Product, selector: parsel.Selector = None):
    def s1() -> str:
        return tabular_buy_box_container(selector, "Ships from")
...
def parse_sold(p: Product, selector: parsel.Selector = None):
    sold = selector.xpath(
        "normalize-space(//span[contains(text(), 'Sold by:')]/following-sibling::*[1]/text())"
    ).get()
    sold = sold or tabular_buy_box_container(selector, "Sold by")
    sold = sold or selector.xpath("//a[@id='sellerProfileTriggerId']/text()").get("")
    if sold == "":
        p.sold = None
    else:
        p.sold = sold
...
def parse_rank(p: Product, selector: parsel.Selector = None):
    rank = product_details_detail_bullets(selector, "Best Sellers Rank")
    if rank == "":
        p.rank = None
        return
