...
SENT_EMAILS=[]
EMAIL_BLACKLIST=APvht()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",APvhW(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=APvha,from_name=APvha,smtp_host=APvha,smtp_user=APvha,smtp_pass=APvha,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if APvhy(recipients,APvhe)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=APvha):
 ...
 result["From"]=formataddr((APvhW(Header(from_name,"utf-8")),from_email))
 return result
