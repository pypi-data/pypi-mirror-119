...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return JKSHO if e else JKSHT
 def attach(self,cli:LocalstackCli)->JKSHD:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(JKSHD)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:JKSHE):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,JKSHD)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:JKSHE):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,JKSHD)
def print_table(columns:Dict[JKSHE,JKSHE],rows:List[Dict[JKSHE,Any]],formatters:Dict[JKSHE,Callable[[Any],JKSHE]]=JKSHD):
 from rich.table import Table
 if formatters is JKSHD:
  formatters=JKSHd()
 t=Table()
