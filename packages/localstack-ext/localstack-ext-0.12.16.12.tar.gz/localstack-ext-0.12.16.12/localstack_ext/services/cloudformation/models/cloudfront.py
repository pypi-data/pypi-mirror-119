...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or aIMpf
 @aIMpv
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return aIMpV(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @aIMpB
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if aIMpG(origins,aIMpc)else origins.get("Items")
  origins_to_create=aIMpx([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if aIMpG(aliases,aIMpc)else aliases.get("Items")
  aliases_to_create=aIMpx(aliases)
  for distr in distrs:
   origins=aIMpx([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=aIMpx(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",aIMps)
   config["Enabled"]=aIMpJ(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if aIMpG(custom_errors,aIMpt)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=aIMpJ(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":aIMps}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=aIMpJ(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=aIMpf):
    parent=config if parent is aIMpf else parent
    items=parent.get(key)or[]
    if aIMpG(items,aIMpt)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":aIMpO(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if aIMpG(cache_behavs,aIMpt)else cache_behavs)
