...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=WRjKz(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not WRjKh
   return result
  except WRjKJ:
   props=self.props
   ...
   if parts[0]in candidates and WRjKx(parts)>1:
