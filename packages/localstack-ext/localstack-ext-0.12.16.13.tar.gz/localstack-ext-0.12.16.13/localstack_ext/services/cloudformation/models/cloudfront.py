...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or NJVgU
 @NJVgv
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return NJVgy(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @NJVgC
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if NJVgr(origins,NJVgw)else origins.get("Items")
  origins_to_create=NJVgd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if NJVgr(aliases,NJVgw)else aliases.get("Items")
  aliases_to_create=NJVgd(aliases)
  for distr in distrs:
   origins=NJVgd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=NJVgd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",NJVgO)
   config["Enabled"]=NJVgz(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if NJVgr(custom_errors,NJVgM)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=NJVgz(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":NJVgO}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=NJVgz(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=NJVgU):
    parent=config if parent is NJVgU else parent
    items=parent.get(key)or[]
    if NJVgr(items,NJVgM)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":NJVgD(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if NJVgr(cache_behavs,NJVgM)else cache_behavs)
