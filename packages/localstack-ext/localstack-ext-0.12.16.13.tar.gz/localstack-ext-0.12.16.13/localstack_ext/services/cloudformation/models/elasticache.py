...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=uvxSy(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not uvxSR
   return result
  except uvxSa:
   props=self.props
   ...
   if parts[0]in candidates and uvxSq(parts)>1:
