...
SENT_EMAILS=[]
EMAIL_BLACKLIST=klyuY()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",klyuV(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=klyuT,from_name=klyuT,smtp_host=klyuT,smtp_user=klyuT,smtp_pass=klyuT,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if klyuj(recipients,klyus)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=klyuT):
 ...
 result["From"]=formataddr((klyuV(Header(from_name,"utf-8")),from_email))
 return result
