...
from localstack_ext.bootstrap import cli,install,licensing,local_daemon
...
def register_localstack_plugins():
 ...
 if noYOG([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if "No module named" not in noYOD(e):
...
def api_key_configured():
 return noYOe(os.environ.get("LOCALSTACK_API_KEY"))
