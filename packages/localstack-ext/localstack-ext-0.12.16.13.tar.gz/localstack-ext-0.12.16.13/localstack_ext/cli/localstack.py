...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return QuXFq if e else QuXFP
 def attach(self,cli:LocalstackCli)->QuXFR:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(QuXFR)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:QuXFf):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,QuXFR)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:QuXFf):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,QuXFR)
def print_table(columns:Dict[QuXFf,QuXFf],rows:List[Dict[QuXFf,Any]],formatters:Dict[QuXFf,Callable[[Any],QuXFf]]=QuXFR):
 from rich.table import Table
 if formatters is QuXFR:
  formatters=QuXFj()
 t=Table()
