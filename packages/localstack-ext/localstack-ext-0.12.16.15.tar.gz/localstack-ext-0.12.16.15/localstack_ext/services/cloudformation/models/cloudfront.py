...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or GsHYJ
 @GsHYS
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return GsHYO(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @GsHYI
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if GsHYb(origins,GsHYn)else origins.get("Items")
  origins_to_create=GsHYv([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if GsHYb(aliases,GsHYn)else aliases.get("Items")
  aliases_to_create=GsHYv(aliases)
  for distr in distrs:
   origins=GsHYv([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=GsHYv(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",GsHYD)
   config["Enabled"]=GsHYu(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if GsHYb(custom_errors,GsHYf)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=GsHYu(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":GsHYD}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=GsHYu(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=GsHYJ):
    parent=config if parent is GsHYJ else parent
    items=parent.get(key)or[]
    if GsHYb(items,GsHYf)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":GsHYW(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if GsHYb(cache_behavs,GsHYf)else cache_behavs)
