...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=HhexF(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not HhexW
   return result
  except Hhexy:
   props=self.props
   ...
   if parts[0]in candidates and Hhexr(parts)>1:
