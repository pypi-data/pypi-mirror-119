...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return Mowuz if e else Mowua
 def attach(self,cli:LocalstackCli)->MowuB:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(MowuB)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:MowuN):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,MowuB)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:MowuN):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,MowuB)
def print_table(columns:Dict[MowuN,MowuN],rows:List[Dict[MowuN,Any]],formatters:Dict[MowuN,Callable[[Any],MowuN]]=MowuB):
 from rich.table import Table
 if formatters is MowuB:
  formatters=MowuR()
 t=Table()
