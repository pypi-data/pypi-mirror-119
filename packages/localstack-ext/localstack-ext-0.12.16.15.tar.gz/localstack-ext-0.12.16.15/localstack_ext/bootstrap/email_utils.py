...
SENT_EMAILS=[]
EMAIL_BLACKLIST=YHwDv()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",YHwDl(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=YHwDI,from_name=YHwDI,smtp_host=YHwDI,smtp_user=YHwDI,smtp_pass=YHwDI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if YHwDB(recipients,YHwDf)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=YHwDI):
 ...
 result["From"]=formataddr((YHwDl(Header(from_name,"utf-8")),from_email))
 return result
