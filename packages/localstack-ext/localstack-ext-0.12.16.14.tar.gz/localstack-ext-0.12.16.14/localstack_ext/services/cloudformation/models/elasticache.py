...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=LrjmS(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not LrjmM
   return result
  except LrjmV:
   props=self.props
   ...
   if parts[0]in candidates and LrjmI(parts)>1:
