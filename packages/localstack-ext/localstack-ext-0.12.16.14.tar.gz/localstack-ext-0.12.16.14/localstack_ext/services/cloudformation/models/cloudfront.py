...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or EHxFQ
 @EHxFp
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return EHxFm(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @EHxFh
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if EHxFW(origins,EHxFc)else origins.get("Items")
  origins_to_create=EHxFT([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if EHxFW(aliases,EHxFc)else aliases.get("Items")
  aliases_to_create=EHxFT(aliases)
  for distr in distrs:
   origins=EHxFT([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=EHxFT(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",EHxFV)
   config["Enabled"]=EHxFD(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if EHxFW(custom_errors,EHxFG)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=EHxFD(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":EHxFV}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=EHxFD(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=EHxFQ):
    parent=config if parent is EHxFQ else parent
    items=parent.get(key)or[]
    if EHxFW(items,EHxFG)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":EHxFq(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if EHxFW(cache_behavs,EHxFG)else cache_behavs)
