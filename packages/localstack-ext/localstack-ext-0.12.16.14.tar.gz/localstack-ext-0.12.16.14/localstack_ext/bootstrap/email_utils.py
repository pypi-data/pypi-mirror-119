...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rmiXg()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rmiXF(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rmiXH,from_name=rmiXH,smtp_host=rmiXH,smtp_user=rmiXH,smtp_pass=rmiXH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rmiXd(recipients,rmiXM)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rmiXH):
 ...
 result["From"]=formataddr((rmiXF(Header(from_name,"utf-8")),from_email))
 return result
