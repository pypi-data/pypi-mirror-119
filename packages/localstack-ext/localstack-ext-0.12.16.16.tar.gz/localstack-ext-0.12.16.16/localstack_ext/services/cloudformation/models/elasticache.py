...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=uKjRA(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not uKjRQ
   return result
  except uKjRg:
   props=self.props
   ...
   if parts[0]in candidates and uKjRb(parts)>1:
