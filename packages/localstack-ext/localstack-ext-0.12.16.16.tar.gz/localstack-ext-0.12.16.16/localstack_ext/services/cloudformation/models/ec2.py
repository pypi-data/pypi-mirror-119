...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=SITOa,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if SITON(perm["IpProtocol"])!=SITON(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",SITOa)
   vpc_id=SITOa
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or SITOa
    vpc_id=groups and groups[0].get("VpcId")or SITOa
   dst_group_id=result.pop("DestinationSecurityGroupId",SITOa)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=SITON(result.get("IpProtocol"))
   description=result.pop("Description",SITOa)
   cidr_ipv6=result.pop("CidrIpv6",SITOa)
