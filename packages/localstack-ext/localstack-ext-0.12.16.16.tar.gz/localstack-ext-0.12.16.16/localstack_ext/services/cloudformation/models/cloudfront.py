...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or VvSDB
 @VvSDQ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return VvSDa(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @VvSDR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if VvSDc(origins,VvSDx)else origins.get("Items")
  origins_to_create=VvSDu([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if VvSDc(aliases,VvSDx)else aliases.get("Items")
  aliases_to_create=VvSDu(aliases)
  for distr in distrs:
   origins=VvSDu([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=VvSDu(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",VvSDH)
   config["Enabled"]=VvSDT(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if VvSDc(custom_errors,VvSDF)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=VvSDT(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":VvSDH}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=VvSDT(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=VvSDB):
    parent=config if parent is VvSDB else parent
    items=parent.get(key)or[]
    if VvSDc(items,VvSDF)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":VvSDK(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if VvSDc(cache_behavs,VvSDF)else cache_behavs)
