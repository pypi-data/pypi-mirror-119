...
SENT_EMAILS=[]
EMAIL_BLACKLIST=eVwIH()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",eVwIp(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=eVwIM,from_name=eVwIM,smtp_host=eVwIM,smtp_user=eVwIM,smtp_pass=eVwIM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if eVwIh(recipients,eVwIq)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=eVwIM):
 ...
 result["From"]=formataddr((eVwIp(Header(from_name,"utf-8")),from_email))
 return result
