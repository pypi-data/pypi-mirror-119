...
function updateAllDateTimes() {
  ...
  $('.js-format-date').each((_, el) => {
    el.innerHTML = moment(el.innerHTML, 'YYYY-MM-DD').isValid() ? formatDateTime(el.innerHTML) : el.innerHTML;
  });
}
...
$('#next-run').on('mouseover', () => {
  $('#next-run').attr('data-original-title', () => {
    let newTitle = '';
    newTitle += `<strong>Run After:</strong> ${Object(_datetime_utils__WEBPACK_IMPORTED_MODULE_1__[/* formatDateTime */ "c"])(nextRun.createAfter)}<br><br>`;
...
/* harmony import */ var _meta_value__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(1);
...
const urlRoot = Object(_meta_value__WEBPACK_IMPORTED_MODULE_3__[/* default */ "a"])('root');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const getActiveRuns = () => data.instances.filter(run => runActiveStates.includes(run.state)).length > 0;
  ...
  function update(source, showTransition = true) {
    ...
    d3.select(window.frameElement).transition().duration(duration).style('height', `${height}px`); // Compute the "layout".
    ...
    const node = svg.selectAll('g.node').data(updateNodes, d => {
      if (!d.id) {
        i += 1;
        d.id = i;
      }
  ...
  function handleRefresh() {
    ...
    $.get(`${treeDataUrl}?dag_id=${dagId}&num_runs=${numRuns}&root=${urlRoot}`).done(runs => {
      const newData = { ...data,
        ...JSON.parse(runs)
