...
class SearchDialog(Toplevel):
    ...
    def search(self,event=None,mark=True,bell=True):
        ...
        if pos:
            ...
            try: 
                import winsound
                winsound.PlaySound('.',winsound.SND_ASYNC)# -*****- 1.2.5版更新
            except (ImportError, RuntimeError):
                self.bell()
...
class Editor(Tk):
    ...
    def create_menu(self):
        ...
        self.is_autowrap=IntVar(self.contents)
...
##        self.is_autowrap.set(int(not self.is_autowrap.get()))
    ...
    def update_offset(self,event=None):
        if self.isbinary:
            ...
            try:
                data=to_bytes(prev) # to_bytes_long(prev,master=self)
            except SyntaxError:
                sep='\\'
                # self.update()
    ...
    def load(self,filename,binary=False):
        ...
        else:msgbox.showinfo("Error","文件未找到:"+repr(filename),parent=self)
