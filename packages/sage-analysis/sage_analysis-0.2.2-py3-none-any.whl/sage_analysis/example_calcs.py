...
def calc_bh_bulge(model, gals, snapshot: int) -> None:
    """
    ...
    if not len(my_gals):
        logger.info(
            "Could not find any galaxies with bulge mass >1e8 Msun and black hole mass >1e5 Msun when trying to "
            "compute black hole bulge relationship."
        )
        return
