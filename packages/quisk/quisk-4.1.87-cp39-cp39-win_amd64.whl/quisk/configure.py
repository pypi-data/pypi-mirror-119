...
class ChoiceCombo(wx.Choice):
  ...
  def SetSelection(self, n):
    if 0 <= n < len(self.choices):
      ...
      print("Failed to set choice index to", n)
    self.GetValue()
...
class ComboCtrl(wxcombo.ComboCtrl):
  def __init__(self, parent, value, choices, no_edit=False):
    ...
    self.dirty = False
    self.bgColor = wx.Choice.GetClassDefaultAttributes().colBg
    if no_edit:
...
class QuiskControl(wx.Control):
  ...
  def MakeMenu(self):
    ...
    for idName in application.idName2Button:
      ...
      elif idName in ("CW U/L", "SSB U/L", "AM", "FM", "DGT", "FDV", "IMD", ):
        item = modes.Append(-1, idName)
      elif idName in ("Graph", "WFall", "Scope", "Config", "Audio FFT", "Bscope", "RX Filter", "Help"):
      ...
      if item:
        ...
        self.menuId2idName[item.GetId()] = idName
    main_names.sort()
    self.menu = wx.Menu()
    main1 = wx.Menu()
    main2 = wx.Menu()
    for name in main_names:
      ...
      self.Bind(wx.EVT_MENU, self.OnMenu, item)
      self.menuId2idName[item.GetId()] = name
    self.menu.AppendSubMenu(main1, "A-N")
    self.menu.AppendSubMenu(main2, "O-Z")
  ...
  def OnMenu(self, event):
    self.value = self.menuId2idName[event.GetId()]
    if self.handler:
      self.handler(self)
...
class ControlMixin:
  ...
  def AddTextEditHelp(self, col, text1, text2, help_text, border=2, span1=1, span2=1, no_edit=True):
    ...
    return txt, edt, btn
  def AddPopupMenuHelp(self, col, text, btn_text, help_text, span1=1, span2=1, border=1):
    if text:
      txt = wx.StaticText(self, -1, text)
      self.gbs.Add(txt, (self.row, col), span=(1, span1), flag=wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, border=self.charx)
      col += span1
    ctrl = QuiskControl(self, btn_text, self.quisk_height - 2, style=wx.BORDER_RAISED)
...
class RadioSection(BaseWindow):		# The pages for each section in the second-level notebook for each radio
  ...
  def MakeControls(self):
    ...
    if self.section == "Keys":
      charx = self.charx
      self.list_ctrl = list_ctrl = wx.ListCtrl(self, size=(charx * 15 + 5, self.quisk_height * 8),
                     style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.BORDER_SIMPLE)
      list_ctrl.InsertColumn(0, 'Note', width=charx*6)
      list_ctrl.InsertColumn(1, 'Button', width=charx*9)
      list_ctrl.SetBackgroundColour(wx.Colour(0xeaeaea))
      index = 0
      for txt_note in local_conf.MidiNoteDict:		# txt_note is a string
        int_note = int(txt_note, base=10)
      ...
      'These MIDI devices generate note on/off messages with a note number that depends on the device and the key. '\
      'You can assign each note number to a Quisk button. '\
      'Look on the Config/Status screen to see the note number received by Quisk. '\
      'Then enter the note number here and press "Enter".'
      txt, edt, btn = self.AddTextEditHelp(2, "Add Midi note", '', hlp, border=2, span1=1, span2=1, no_edit=False)
      self.Bind(wx.EVT_TEXT_ENTER, self.OnMidiAddNote, source=edt)
      self.NextRow()
      hlp = 'Select a row and then choose a Quisk button to assign to that note.'
      ctrl = self.AddPopupMenuHelp(2, "Assign Midi note to", "Button", hlp)
      ctrl.handler = self.OnMidiMenu
      self.NextRow()
      hlp = "To delete a row, select it and press this button."
      self.AddTextButtonHelp(2, "Delete Midi note", "Delete", self.OnMidiDelete, hlp)
      self.NextRow()
  ...
  def OnMidiAddNote(self, event):
    note = event.GetString()
    win = event.GetEventObject()
    win.Clear()
    ctrl = self.list_ctrl
    try:
      int_note = int(note, base=10)
      txt_note = str(int_note)
    except:
      return
    ...
    if index >= 0:
      ctrl.Select(index)
      ctrl.EnsureVisible(index)
  def OnMidiMenu(self, ctrl):
    txt = ctrl.value
    index = self.list_ctrl.GetFirstSelected()
    if txt and index >= 0:
      self.list_ctrl.SetItem(index, 1, txt)
      txt_note = self.list_ctrl.GetItem(index, 0).GetText()
      local_conf.MidiNoteDict[txt_note] = txt
      local_conf.settings_changed = True
