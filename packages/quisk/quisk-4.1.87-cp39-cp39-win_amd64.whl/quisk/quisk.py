...
class ConfigStatus(wx.ScrolledWindow):
  ...
  def MakeBitmap(self):
    ...
    if application.midi_note:
      self.MakeRow2("Midi received", application.midi_note)
    else:
      self.MakeRow2("Midi received", '-')
    self.mem_y += self.dy
...
class App(wx.App):
  ...
  def OnReadSound(self):	# called at frequent intervals
    ...
    if self.midi_note:
      if self.timer - self.midi_time0 > 0.4:
        self.midi_note = None
  ...
  def OnReadMIDI(self, byts):
    for byt in byts:
      ...
      if len(self.midi_message) == 3:
        self.midi_note = "0x%2X %d %d" % tuple(self.midi_message)
        self.midi_time0 = time.time()
