...
class Mongo:
    def __init__(
        ...
        mongo_cluster="dev-data-team-cluster",
        max_pool_size=64,
    ):
        ...
        uri = "mongodb+srv://%s:%s@%s.drpqa.mongodb.net/?authSource=$external&authMechanism=MONGODB-AWS&retryWrites=true&w=majority" % (
            access_key_id,
            secret_access_key,
            cluster,
        )
    ...
    def insert_items(self, database, collection, queries):
        try:
            ...
            for query in queries:
                query["created_at"] = datetime.today()
                query["updated_at"] = datetime.today()

            writer = self.client[database][collection]
            result = writer.insert_many(queries)
            print(
                f"Successfully inserted {len(queries)} documents to '{database}'-'{collection}'"
            )
    ...
    def update_item(self, database, collection, id, value):
        try:
            value["updated_at"] = datetime.today()
