...
class Settings(object):
    def __init__(self, setting=None):
        self.downloader = DownloaderSetting()
        self.request = RequestSetting()
        if setting: self.load(setting)

    def load(self, setting):
        if isinstance(setting, Settings):
            self.request.__dict__.update(setting.request.__dict__)
            self.downloader.__dict__.update(setting.downloader.__dict__)
        elif isinstance(setting, dict):
            if 'request' in setting.keys():
                self.request.__dict__.update(setting.get('request'))
            else:
