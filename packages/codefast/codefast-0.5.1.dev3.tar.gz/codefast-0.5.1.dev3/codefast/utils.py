...
class Network:
    ...
    @classmethod
    def _resume(cls, url: str, name: str, resume_byte_pos: int = 0) -> None:
    ...
    @classmethod
    def download(cls, url: str, name=None) -> None:
        name = name or url.split('/').pop()

        if not FileIO.exists(name):
            cls._resume(url, name)

        resume_size_in_bytes = os.path.getsize(name)
        total_size_in_bytes = int(
            requests.get(url, stream=True).headers.get('content-length', resume_size_in_bytes))

        while abs(resume_size_in_bytes - total_size_in_bytes) > 8:
            print(resume_size_in_bytes, total_size_in_bytes)
            Logger().info('resume downloading {}'.format(name))
            try:
                cls._resume(url, name, resume_size_in_bytes)
