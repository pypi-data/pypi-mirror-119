    ...
    for ser in COMS:
        ...
        if pico_serial in hardware_id_channels_map.keys():
            for channel in hardware_id_channels_map[pico_serial]:
                response = requests.get('http://192.168.3.101:8000/channels/' + str(channel.serial_no) + '/io')
                data = response.json()
                pico_config_element = {}
...
def load_hub_config(hub):
    ...
    response = http.get('http://192.168.3.101:8000/hubs/' + hub_serial_no)
...
def load_hardware_config(hardware):
    response = http.get('http://192.168.3.101:8000/hub/' + hub_serial_no + '/hardwares')
    data = json.dumps(response.json())
...
def load_channels(channels, id_channels_map):
    for hard in id_hardware_map.values():
        response = requests.get('http://192.168.3.101:8000/hardwares/' + str(hard.serial_no) + '/channels')
        data = json.dumps(response.json())
        t = []
...
def load_homekit_accessory_config(accessories):
    from OpenHub.homekit_accessories.json import homekit_decoder
    response = http.get('http://192.168.3.101:8000/hub/' + hub_serial_no + '/accessories')
    data = json.dumps(response.json())
