...
def send_image_to_bisque(session, args, root, image_path,   tagmap=None, fixedtags=None):
    ...
    for tag, value in [ x.split(':') for x in args.tag if '$' not in x ]:
        ...
        resource_tags[tag] = value
    # Add special tags for path and filename (if referenced)
    ...
    for tag, value in resource_tags.items():
        ...
        new_tags[tag]=value  # Save old tag also
    ...
    for tagkey, tagtable in fixedtags.items():
        ...
        if tagkey == 'filename':
            ...
            key =  resource_tags.get(tagkey)
        if key is None:
            args.log.warn("Lookup for fixed table: key %s  was not found", tagkey)
            continue
        if key not in tagtable:
            args.log.warn("Key %s : %s was not present in fixedtable", tagkey, key) # KGK: Too strict  for multple fixedtables
            continue
        for tag, value in tagtable[key].items():
            resource_tags[tag]=value
    ...
    ######################
    #  move tags to xml if note removed
    upload_tags = {}
    for tag,value in resource_tags.items():
        if tag and value and tag not in args.tagremove:
            etree.SubElement(tags, 'tag', name=tag, value=value)
            upload_tags[tag] = value
    ...
    if not args.dry_run:
        ...
        etree.SubElement(response, 'image', name=image_path,
                         ...
                         value=image_path)
    if not args.quiet:
        args.log.info("Uploaded %s with %s", image_path, upload_tags)
    return response
...
def main():
    ...
    parser.add_argument('--path-tags', help='tag names for a parsable path i.e. /root/project/date//subject/ or \\root\\project\\data\subject', default="")
    ...
    parser.add_argument('--tagremove', help="list of tags to remove from annotated upload. Can be repeated and/or readfrom @tagfile) ", default=[], action="append")
    ...
    parser.add_argument("-r", "--replace-uploaded", help="Force upload of metadata even if file exists on server", action="store_true")
    ...
    for entry in args.tagmap:
        ...
        if entry.startswith('@'):
            ...
            with open(entry[1:], 'r') as csvfile:
                tagitems.setdefault(context, {}).update((row[0].strip(), row[1].strip()) for row in csv.reader(decomment(csvfile)))
        else:
