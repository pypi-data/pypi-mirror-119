...
def extract_data(p, encoding='utf-8', start_func=lambda x: x.startswith('create table'), end_func=lambda x: x.startswith('tablespace')):
    flag = False
    data_map = {}
...
def main(ipath, opath=None, encoding='utf-8'):
    p = Path(ipath)
    if opath is None:
