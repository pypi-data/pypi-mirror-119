...
from rdflib.namespace import Namespace, ClosedNamespace, URIPattern
...
class Entity(object, metaclass=EntityMetaclass):
    ...
    @staticmethod
    def getEntityFromRDFType(rdf_type):
    ...
    def getEntityClassFromURI(uri):
        ...
        if not hasattr(Entity, "_z_domain"): setattr(Entity, "_z_domain", Entity().entity_domain)

        my_uri = str(uri)
        if my_uri.startswith(Entity._z_domain):
            my_uri = my_uri.split('/')
            if my_uri[3] in Entity._z_cache_scopes: return Entity._z_cache_scopes[my_uri[3]]
        return None
    ...
    def load(data, format="turtle"):
        ...
        rows = graph.query(SPARQL_LOAD_GET_ENTITIES)
        for row in rows:
            #classname = mapper.unmap_class(graph.namespace_manager.qname(row[1]))
        ...
        for uri in entities:
            ...
            for row in rows:
                ...
                if propertyName is not None and hasattr(entities[uri], propertyName):
                    my_object = row[1]
        ...
        #P-3 filter result
        entities = [entities[e] for e in entities if 0 == entities[e]._in ]
    ...
    def _register_namespaces(self, graph):
        ...
        for a in namespace.__dict__:
            if isinstance(namespace.__dict__[a],Namespace) or isinstance(namespace.__dict__[a],ClosedNamespace):
                graph.bind(a.lower(), namespace.__dict__[a])
...
class EntityMapper(object):
    ...
    def __load_mapping_definition(self, scope=None, schema=None):
        ...
        try:
            url = "{}/{}".format(self.config.get("mapping_definition_url", default="http://example.com"), mapping_file)
        ...
        # definition_file = pkg_resources.resource_filename(__name__, "..{sep}data{sep}{scope}_entity_mapping.json".format(sep=os.sep, scope=scope.lower()))
        # if os.path.exists(definition_file):
...
class EntityMappingSchema(object):
    ...
    def unmap_class(self, classname):
        ...
        keys = [key for key, value in self.classes.items() if str(value) == classname]
        if len(keys): 
            return keys[0]
