...
_linode_cli()
{
    ...
    case "${prev}" in
        linode-cli)
            COMPREPLY=( $(compgen -W "account default events payment-methods service-transfers users domains images linodes kernels stackscripts lke longview managed networking firewalls vlans nodebalancers object-storage profile sshkeys regions tickets tags volumes --help" -- ${cur}) )
            return 0
            ;;
        account)
            COMPREPLY=( $(compgen -W "view update cancel update-card invoices-list invoice-view invoice-items logins-list login-view maintenance-list notifications-list clients-list client-create client-view client-update client-delete client-reset-secret payments-list payment-create payment-view paypal-start paypal-execute promo-add settings settings-update enable-managed transfer --help" -- ${cur}) )
            return 0
            ;;
        default)
        ...
        users)
            ...
            ;;
        domains)
            COMPREPLY=( $(compgen -W "list create view update delete zone-file clone records-list records-create records-view records-update records-delete --help" -- ${cur}) )
            return 0
            ;;
        images)
        ...
        stackscripts)
            ...
            ;;
        lke)
            COMPREPLY=( $(compgen -W "clusters-list cluster-create cluster-view cluster-update cluster-delete pools-list pool-create cluster-nodes-recycle pool-view pool-update pool-delete pool-recycle node-view node-delete node-recycle api-endpoints-list kubeconfig-view kubeconfig-delete versions-list version-view --help" -- ${cur}) )
            return 0
            ;;
        longview)
