...
def convert_field_into_input(model_field: Field = None, graphene_field: TGrapheneReturnType = None, graphene_type: Union[TGrapheneQuery, TGrapheneMutation] = None) -> TGrapheneReturnType:
    """
    ...
    :param model_field: if graphene_field is None, the django model to convert

    """
    if graphene_field is not None:
        t = graphene_field
    elif graphene_type is not None:
        t = graphene_type._meta.fields[model_field.attname]
    else:
...
def create_graphene_tuple_input_type(name: str, graphene_input_type: TGrapheneQuery) -> TGrapheneInputType:
    return None
...
def create_graphene_tuple_input_type(name: str, it: Iterable[Tuple[str, TGrapheneInputType]]) -> TGrapheneInputType:
    ...
    properties = dict()
    for key, type in it:
        properties[key] = convert_field_into_input(type)
    result = type(
        name,
...
def create_graphene_tuple_type(name: str, it: Iterable[Tuple[str, type]]) -> type:
    """
    Programmatically create a type in graphene repersenting a tuple of elements.
