...
def convert_field_into_input(field_specifier: Union[str, Field], graphene_field: TGrapheneReturnType = None, graphene_type: Union[TGrapheneQuery, TGrapheneMutation] = None) -> TGrapheneReturnType:
    """
    ...
    :param field_specifier: if graphene_field is None, either the django model to convert or the field name in the graphene type
        to convert
...
def create_graphene_tuple_input_type_from_graphene_type(graphene_tuple_type: TGrapheneQuery, name: str = None, description: str = None) -> TGrapheneInputType:
    properties = {}

    if name is None:
        name = f"{graphene_tuple_type.__name__}Meta"
    if description is None:
        description = f"Input representation fo the graphene tuple type {graphene_tuple_type.__name__}"

    for field_name, field in graphene_tuple_type._meta.fields:
        properties[field_name] = convert_field_into_input(
            graphene_type=graphene_tuple_type,
            field_specifier=field_name
        )
