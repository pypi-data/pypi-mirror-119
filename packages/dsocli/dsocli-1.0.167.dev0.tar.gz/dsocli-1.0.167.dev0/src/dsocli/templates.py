...
class TemplateService():
    ...
    def get_template_render_path(self, key):
        ...
        return f'.{os.sep}' + os.path.relpath(os.path.join(Templates.default_render_path, key), self.config.working_dir) 

    def list(self, config, uninherited=False, include_contents=False, filter=None):
        self.config = config
        provider = Providers.TemplateProvider()
        Logger.info(f"Start listing templates: namespace={config.get_namespace(ContextMode.Requested)}, project={config.get_project(ContextMode.Requested)} application={config.get_application(ContextMode.Requested)}, stage={config.get_short_stage(ContextMode.Requested)}, scope={config.context.scope}")
    ...
    def add(self, config, key, contents, render_path):
        ...
        self.config = config
        provider = Providers.TemplateProvider()
        Logger.info(f"Start adding template '{key}': namespace={config.get_namespace(ContextMode.Requested)}, project={config.get_project(ContextMode.Requested)} application={config.get_application(ContextMode.Requested)}, stage={config.get_short_stage(ContextMode.Requested)}, scope={config.context.scope}")
    ...
    def get(self, config, key, revision=None):
        self.config = config
        provider = Providers.TemplateProvider()
        Logger.info(f"Start getting template '{key}': namespace={config.get_namespace(ContextMode.Requested)}, project={config.get_project(ContextMode.Requested)} application={config.get_application(ContextMode.Requested)}, stage={config.get_short_stage(ContextMode.Requested)}, scope={config.context.scope}")
    ...
    def history(self, config, key, include_contents=False):
        self.config = config
        provider = Providers.TemplateProvider()
        Logger.info(f"Start getting the history of template '{key}': namespace={config.get_namespace(ContextMode.Requested)}, project={config.get_project(ContextMode.Requested)} application={config.get_application(ContextMode.Requested)}, stage={config.get_short_stage(ContextMode.Requested)}, scope={config.context.scope}")
        return provider.history(config, key, include_contents)

    def delete(self, config, key):
        self.config = config
        provider = Providers.TemplateProvider()
        Logger.info(f"Start deleting template '{key}': namespace={config.get_namespace(ContextMode.Requested)}, project={config.get_project(ContextMode.Requested)} application={config.get_application(ContextMode.Requested)}, stage={config.get_short_stage(ContextMode.Requested)}, scope={config.context.scope}")
    ...
    def render(self, config, filter=None):
        ...
        Logger.info(f"Start rendering templates: namespace={config.get_namespace(ContextMode.Requested)}, project={config.get_project(ContextMode.Requested)} application={config.get_application(ContextMode.Requested)}, stage={config.get_short_stage(ContextMode.Requested)}, scope={config.context.scope}")
