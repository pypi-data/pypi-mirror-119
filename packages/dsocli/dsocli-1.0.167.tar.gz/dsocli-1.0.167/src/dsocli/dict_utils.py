...
def set_dict_value(dic, keys, value, overwrite_parent=False, overwrite_children=False):
    ...
    if not isinstance(parent_item, dict):
        if overwrite_parent:
            ...
            raise DSOException("Failed to set '{0}' becasue it would overwrite the parent item '{1}' which is not a dictionary but of type '{2}'.".format('.'.join(keys), '.'.join(keys[:-1]), type(parent_item)))
...
def del_dict_item(dic, keys):
    ...
    if not item or not keys[-1] in item:
        return False
