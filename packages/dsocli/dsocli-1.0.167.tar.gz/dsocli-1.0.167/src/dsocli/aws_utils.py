...
logging.getLogger('botocore').setLevel(Logger.mapped_log_level)
logging.getLogger('boto').setLevel(Logger.mapped_log_level)
logging.getLogger('boto3').setLevel(Logger.mapped_log_level)
...
def load_ssm_path(parameters, path, parameter_type, prefix=None, decrypt=False, filter=None):
...
def load_context_ssm_parameters(project, application, stage, parameter_type, prefix='', decrypt=False, uninherited=False, filter=None):
    ...
    for path in paths:
        ...
        load_ssm_path(parameters, path, parameter_type, prefix, decrypt, filter=filter)
...
def locate_ssm_parameter_in_context_hierachy(project, application, stage, key, prefix=None, uninherited=False):
    result = {}
    paths = Contexts.get_hierachy_context_paths(project, application, stage, key, prefix=prefix, allow_stages=(not stage is None), uninherited=uninherited, reverse=True)
    ssm = boto3.session.Session().client(service_name='ssm')
    for path in paths:
