...
class Handler:
    ...
    def _send_notify(self, body: str, medium):
    ...
    def send_notify(self, body: str, medium):
        try:
            self._send_notify(body, medium)
        except (
            ConnectionClosed,
            ChannelClosed,
            ChannelWrongStateError,
            ...
            LOGGER.info(
                f"Rabbitmq reported error {type(e)} while publishing, trying to reconnect",
            )
            self.channel = None
            self._send_notify(body, medium)
