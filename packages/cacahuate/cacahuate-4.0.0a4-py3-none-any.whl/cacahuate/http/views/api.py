...
@app.route('/v1/user/<user_identifier>', methods=['GET'])
def fetch_user_info(user_identifier):
    user = User.get_by_or_exception('identifier', user_identifier)
...
def execution_patch(id):
    ...
    collection = mongo.db[app.config['EXECUTION_COLLECTION']]
    execution_state = next(collection.find({'id': id, 'status': 'ongoing'}))
    ...
    for i, field in enumerate(request.json['inputs']):
        ...
        if 'value' in field:
            try:
                input_obj = make_input(element=input_el)
...
def execution_add_user(exe_id):
    ...
    # get actual pointer
    try:
        pointer = next(execution.pointers.q().filter(node_id=node_id, status='ongoing'))
    except StopIteration:
        raise BadRequest([{
...
def continue_process():
    ...
    try:
        pointer = next(execution.pointers.q().filter(
            node_id=node_id,
            status='ongoing',
        ))
    except StopIteration:
...
@requires_auth
def list_activities():
    activities = g.user.activities.q().filter(status='ongoing')
...
@requires_auth
def task_list():
    tasks = g.user.tasks.q().filter(status='ongoing')
