...
class Command:
    def __init__(self):
        ...
        """
        self.addr = (data.SERVER_ADDR, data.UDPPORT)
        print(f'Set server address : {self.addr}')

    def account(self) -> data.AccountData:
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(data.ACC.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
    ...
    def _code_manager(self, code: str, cmd: str):
        ...
        try:
            if cmd not in data.CMANAGER_CMD:
                raise ValueError
            udp_sock.sendto(f'{data.CMANAGER}/{code}/{cmd}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
    ...
    def cprice(self, code) -> data.CurrentPriceData:
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{data.CPRICE}/{code}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
    ...
    def inquire_order(self) -> list:
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(data.IORDER.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
    ...
    def buy_order(self, code: str, amount: int, price: int):
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{data.BUY}/{code}/{amount}/{price}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
    ...
    def sell_order(self, code: str, amount: int, price: int):
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{data.SELL}/{code}/{amount}/{price}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def cancel_one(self, ordernum: str):
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{data.CANCEL}/one/{ordernum}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def cancel_all(self, code: str):
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{data.CANCEL}/all/{code}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(data.BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
