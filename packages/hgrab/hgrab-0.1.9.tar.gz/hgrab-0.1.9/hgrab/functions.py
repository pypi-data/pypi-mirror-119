...
class Downloader:
    ...
    # aiohttp异步
    async def fetch(self, session, url, headers=None, timeout=9, binary=False, debug=None):
        if not headers:
    ...
    async def post_fetch(self, session, url, data, headers=None, timeout=10, binary=False, debug=None):
        if not headers:
            _headers = self.headers
        if headers:
            _headers = headers
